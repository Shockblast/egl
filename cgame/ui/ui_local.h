...
#define UI_HSCALE			((float)(cg.glConfig.vidWidth / 640.0))
#define UI_VSCALE			((float)(cg.glConfig.vidHeight / 480.0))

#define UI_SCALE			((float)(cg.glConfig.vidWidth / 640.0))
...
typedef struct uiState_s {
	...
	struct sfx_s	*(*keyFunc) (uiFrameWork_t *fw, int keyNum);
} uiState_t;
...
void		UI_PushInterface (uiFrameWork_t *fw, void (*drawFunc) (void), struct sfx_s *(*closeFunc) (void), struct sfx_s *(*keyFunc) (uiFrameWork_t *fw, int keyNum));
...
struct sfx_s *UI_DefaultKeyFunc (uiFrameWork_t *fw, int keyNum);
