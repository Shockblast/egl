...
static void StartServerMenu_Init (void)
{
	...
	if (!buffer || length <= 0) {
		...
		ps = Com_BeginParseSession (&list);
		for (i=0 ; i<m_startServerMenu.numMaps ; i++) {
			char	scratch[200];
			int		j, l;

			Q_strncpyz (shortname, Com_ParseExt (ps, qTrue), sizeof (shortname));
			l = (int)strlen (shortname);
			for (j=0 ; j<l ; j++)
				shortname[j] = toupper(shortname[j]);

			Q_strncpyz (longname, Com_ParseExt (ps, qTrue), sizeof (longname));
...
static void StartServerMenu_Draw (void)
{
	...
	m_startServerMenu.frameWork.x			= cg.glConfig.vidWidth * 0.5f;
