...
static void KeyCursorDrawFunc (uiFrameWork_t *menu)
{
	...
	if (item->flags & UIF_LEFT_JUSTIFY)
		CG_DrawChar (menu->x + item->x + LCOLUMN_OFFSET + item->cursorOffset, menu->y + item->y, 0, UISCALE_TYPE (item->flags), cursor, colorWhite);
	else if (item->flags & UIF_CENTERED)
		CG_DrawChar (menu->x + item->x - (((Q_ColorCharCount (item->name, (int)strlen (item->name)) + 4) * UISIZE_TYPE (item->flags))*0.5) + item->cursorOffset, menu->y + item->y, 0, UISCALE_TYPE (item->flags), cursor, colorWhite);
	else
		CG_DrawChar (menu->x + item->x + item->cursorOffset, menu->y + item->y, 0, UISCALE_TYPE (item->flags), cursor, colorWhite);
}
...
static void DrawKeyBindingFunc (void *self)
{
	...
	M_FindKeysForCommand (m_bindNames[a->generic.localData[0]][0], keys);
		
	if (keys[0] == -1) {
		CG_DrawString (a->generic.x + a->generic.parent->x + (UISIZE_TYPE (a->generic.flags)*2), a->generic.y + a->generic.parent->y,
		...
		name = cgi.Key_KeynumToString (keys[0]);
		...
		if (keys[1] != -1) {
			CG_DrawString (a->generic.x + a->generic.parent->x + (UISIZE_TYPE (a->generic.flags)*3) + x, a->generic.y + a->generic.parent->y,
				0, UISCALE_TYPE (a->generic.flags), "or", colorWhite);
			CG_DrawString (a->generic.x + a->generic.parent->x + (UISIZE_TYPE (a->generic.flags)*6) + x, a->generic.y + a->generic.parent->y,
				0, UISCALE_TYPE (a->generic.flags), cgi.Key_KeynumToString (keys[1]), colorWhite);

			a->generic.botRight[0] = a->generic.x + a->generic.parent->x + ((int)strlen (cgi.Key_KeynumToString (keys[1])) * UISIZE_TYPE (a->generic.flags)) + (UISIZE_TYPE (a->generic.flags)*6) + x;
			a->generic.botRight[1] = a->generic.y + a->generic.parent->y + UISIZE_TYPE (a->generic.flags);
		}
	}
}
...
static void ControlsMenu_Draw (void)
{
	...
	m_controlsMenu.frameWork.x				= cg.glConfig.vidWidth * 0.5f;
...
=============
*/
static struct sfx_s *ControlsMenu_Key (uiFrameWork_t *fw, int key)
{
	char	cmd[1024];
	...
	if (uiState.cursorLock) {
		...
		default:
			Q_snprintfz (cmd, sizeof (cmd), "bind \"%s\" \"%s\"\n", cgi.Key_KeynumToString(key), m_bindNames[item->generic.localData[0]][0]);
	...
	return UI_DefaultKeyFunc (fw, key);
}
