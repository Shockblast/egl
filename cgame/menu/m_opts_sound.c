...
void Sound_RestartBox (void)
{
	...
	CG_DrawString (midrow + (UIFT_SIZEMED*2), midcol + (UIFT_SIZEMED*2), 0, UIFT_SCALEMED,	"The sound system is currently res-",	colorGreen);
	CG_DrawString (midrow + (UIFT_SIZEMED*2), midcol + (UIFT_SIZEMED*3), 0, UIFT_SCALEMED,	"tarting, this could take up to a",		colorGreen);
	CG_DrawString (midrow + (UIFT_SIZEMED*2), midcol + (UIFT_SIZEMED*4), 0, UIFT_SCALEMED,	"minute so, please be patient.",		colorGreen);
...
static void SoundMenu_Draw (void)
{
	...
	m_soundMenu.frameWork.x		= cg.glConfig.vidWidth * 0.5f;
