...
void PlayerConfigMenu_Draw (void)
{
	...
	m_playerConfigMenu.frameWork.x				= cg.glConfig.vidWidth * 0.5f;
	...
	if (!m_playerConfigMenu.modelsFound) {
		...
		CG_DrawString (
			(cg.glConfig.vidWidth-((strlen(str)-4)*UIFT_SIZELG)) * 0.5f,
	...
	refDef.x = (cg.glConfig.vidWidth * 0.5f) - (UIFT_SIZE * 10);
...
{
	float	midrow = (cg.glConfig.vidWidth*0.5) - (18*UIFT_SIZEMED);
	float	midcol = (cg.glConfig.vidHeight*0.5) - (3*UIFT_SIZEMED);

	// show box
	UI_DrawTextBox (midrow, midcol, UIFT_SCALEMED, 36, 4);
	CG_DrawString (midrow + (UIFT_SIZEMED*2), midcol + UIFT_SIZEMED, 0, UIFT_SCALEMED,		"       --- PLEASE WAIT! ---       ",	colorGreen);
	CG_DrawString (midrow + (UIFT_SIZEMED*2), midcol + (UIFT_SIZEMED*2), 0, UIFT_SCALEMED,	"Player models, skins and icons are",	colorGreen);
	CG_DrawString (midrow + (UIFT_SIZEMED*2), midcol + (UIFT_SIZEMED*3), 0, UIFT_SCALEMED,	"being listed, and this might take ",		colorGreen);
	CG_DrawString (midrow + (UIFT_SIZEMED*2), midcol + (UIFT_SIZEMED*4), 0, UIFT_SCALEMED,	"a minute so, please be patient.   ",		colorGreen);
