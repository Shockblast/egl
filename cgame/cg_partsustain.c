...
void CG_Nukeblast (cgSustainPfx_t *self)
{
	...
	if (cgs.currGameMod == GAME_MOD_GLOOM) {
		ratio = 1.0f - (((float)self->endtime - (float)cgs.realTime) / 1000.0f);

		cgi.R_AddLight (self->org, 600 * ratio * (0.9f + (frand () * 0.1f)), 1, 1, 1);

		if ((int)(ratio*10) & 1)
			CG_ExplosionParticles (self->org, 3.5f + (crand () * 0.5f), qTrue, qFalse);

		for (i=0 ; i<pIncDegree (self->org, 100, 0.5f, qTrue) ; i++) {
			VectorSet (dir, crand (), crand (), crand ());
			VectorNormalize (dir, dir);
			VectorMA (self->org, (125.0f * ratio), dir, porg);
		...
		ratio = 1.0f - (((float)self->endtime - (float)cgs.realTime)/1000.0f);
		for (i=0 ; i<pIncDegree (self->org, 600, 0.1f, qTrue) ; i++) {
			VectorSet (dir, crand (), crand (), crand ());
			VectorNormalize (dir, dir);
			VectorMA(self->org, (200.0f * ratio), dir, porg);
...
void __fastcall CG_ParticleSteamEffect (vec3_t org, vec3_t dir, int color, int count, int magnitude)
{
	...
	for (i=0 ; i<pIncDegree (org, (float)count, 0.5f, qTrue) ; i++) {
...
void CG_ParticleSteamEffect2 (cgSustainPfx_t *self)
{
	...
	for (i=0 ; i<(int)pIncDegree (self->org, (float)self->count, 0.5f, qTrue) ; i++) {
...
void CG_Widowbeamout (cgSustainPfx_t *self)
{
	...
	ratio = 1.0f - (((float)self->endtime - (float)cgs.realTime)/ 2100.0f);

	for (i=0 ; i<pIncDegree (self->org, 300, 0.5, qTrue) ; i++) {
		VectorSet (dir, crand (), crand (), crand ());
		VectorNormalize (dir, dir);
	
		VectorMA (self->org, (45.0f * ratio), dir, porg);
