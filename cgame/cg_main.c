...
/*
==================
CG_FixGloomCvarCheats
==================
*/
static cheatVar_t glmcheatVars[] = {
	{"cl_noskins",			0},
	{"cl_testblend",		0},
	{"gl_nobind",			0},
	{"gl_polyblend",		1},
...
static void CG_FixGloomCvarCheats (void) {
	int			i;
	cheatVar_t	*var;
	...
	if (!numGlmCheatVars) {
		while (glmcheatVars[numGlmCheatVars].name) {
			glmcheatVars[numGlmCheatVars].var = cgi.Cvar_Get (glmcheatVars[numGlmCheatVars].name, Q_VarArgs ("%d", glmcheatVars[numGlmCheatVars].value), 0);
			numGlmCheatVars++;
		}
	}
...
void CG_CopyConfigStrings (void) {
	...
	for (num=0 ; num<MAX_CONFIGSTRINGS ; num++) {
		...
		if ((num >= CS_LIGHTS) && (num < CS_LIGHTS+MAX_CS_LIGHTSTYLES)) {
			...
			// image
			cgi.Img_RegisterPic (cg.configStrings[num]);
		}
	}
}
...
static void CG_RegisterCmds (void) {
	cgi.Cmd_AddCommand ("skins",		CG_Skins_f,		"Lists skins of players connected");
}
