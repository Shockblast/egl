...
void CG_LoadClientinfo (clientInfo_t *ci, char *s) {
	...
	if (cl_noskins->integer || (*s == 0)) {
		...
		Q_snprintfz (skin_filename, sizeof (skin_filename),		"players/male/grunt.pcx");
		...
		ci->skin = cgi.Img_RegisterSkin (skin_filename);
		ci->icon = cgi.Img_RegisterPic (ci->iconName);
	} else {
		...
		*t = 0;
		...
		// skin file
		Q_snprintfz (skin_filename, sizeof (skin_filename), "players/%s/%s.pcx", model_name, skin_name);
		ci->skin = cgi.Img_RegisterSkin (skin_filename);
		...
		if (!ci->skin) {
			// see if the skin exists for the male model
			Q_snprintfz (skin_filename, sizeof (skin_filename), "players/%s/grunt.pcx", model_name, skin_name);
			ci->skin = cgi.Img_RegisterSkin (skin_filename);
		}
		...
		// icon file
		Q_snprintfz (ci->iconName, sizeof (ci->iconName), "/players/%s/%s_i.pcx", model_name, skin_name);
		ci->icon = cgi.Img_RegisterPic (ci->iconName);
	}
...
void CG_FixUpGender (void) {
	if (gender_auto->value) {
		...
		if (!Q_stricmp (sk, "male") || !Q_stricmp (sk, "cyborg"))
			cgi.Cvar_VariableSet (gender, "male");
		else if (!Q_stricmp (sk, "female") || !Q_stricmp (sk, "crackhor"))
			cgi.Cvar_VariableSet (gender, "female");
		else
			cgi.Cvar_VariableSet (gender, "none");
		gender->modified = qFalse;
	}
}
