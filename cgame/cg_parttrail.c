...
void CG_BfgTrail (refEntity_t *ent)
{
	...
	for (i=0 ; i<(NUMVERTEXNORMALS/2.0f) ; i++) {
		...
		org[1] = ent->origin[1] + (m_byteDirs[i][1] * (float)sin (ltime + i) * 64) + (forward[1] * BEAMLENGTH);
		org[2] = ent->origin[2] + (m_byteDirs[i][2] * (float)sin (ltime + i) * 64) + (forward[2] * BEAMLENGTH);
...
void CG_GrenadeTrail (vec3_t start, vec3_t end)
{
	...
	for (; len>0 ; Vec3Add (move, vec, move)) {
		...
		CG_SpawnParticle (
			...
			rnum,							rnum,							rnum,
			rnum2,							rnum2,							rnum2,
			0.8f + (crand () * 0.1f),		-3.5f / (1.0f + cg_particleSmokeLinger->floatVal + (crand () * 0.15f)),
...
void CG_RocketTrail (vec3_t start, vec3_t end)
{
	...
	for (; len>0 ; Vec3Add (move, vec, move)) {
		...
		CG_SpawnParticle (
			...
			rnum,								rnum,								rnum,
			rnum2,								rnum2,								rnum2,
			0.4f + (crand () * 0.1f),			-1.5f / (1.0f + cg_particleSmokeLinger->floatVal + (crand () * 0.15f)),
