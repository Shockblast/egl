...
void __fastcall CG_BeamTrail (vec3_t start, vec3_t end, int color, float size, float alpha, float alphaVel)
{
	...
	dec = pDecDegree (start, 35.0f - (rand () % 27), 0.2f, qFalse);
...
void CG_BfgTrail (entity_t *ent)
{
	...
	for (i=0 ; i<(pIncDegree (ent->origin, NUMVERTEXNORMALS/2.0, 0.5, qTrue)) ; i++) {
		...
		// sparks
		if ((!(rand () % 7)) && (((frand () * 20 + 1) / 10.0) <= cg_particleQuality->value)) {
			makePart (
				...
				PT_BFG_DOT,							PF_SCALED|PF_DIRECTION,
				pSplashThink,						qTrue,
				PMAXSPLASHLEN);
		}
	}
...
void CG_BlasterGoldTrail (vec3_t start, vec3_t end)
{
	...
	dec = (int)pDecDegree (start, 5.0f, 0.3f, qTrue);
	VectorScale (vec, dec, vec);

	orgscale = (cgs.currGameMod == GAME_MOD_GLOOM) ? 0.5f : 1.0f;
	velscale = (cgs.currGameMod == GAME_MOD_GLOOM) ? 3.0f : 5.0f;
...
void CG_BlasterGreenTrail (vec3_t start, vec3_t end)
{
	...
	dec = (int)pDecDegree (start, 5, 0.3f, qTrue);
	VectorScale (vec, dec, vec);
...
void CG_BubbleTrail (vec3_t start, vec3_t end)
{
	...
	dec = (int)pDecDegree (start, 32, 0.5, qFalse);
	VectorScale (vec, dec, vec);
...
void CG_BubbleTrail2 (vec3_t start, vec3_t end, int dist)
{
	...
	dec = (int)pDecDegree (start, (float)dist, 0.5f, qFalse);
	VectorScale (vec, dec, vec);
	...
	for (i=0 ; i<(clamp (cg_particleGore->value + 1, 1, 11) / 5.0) ; i++) {
		if (!(rand () % 26)) {
			...
			makePart (
				...
				1.5f + frand (),					2 + frand (),
				isGreen ? PT_BLDDRIP_GRN : PT_BLDDRIP,
				PF_GRAVITY|PF_DIRECTION|PF_ALPHACOLOR|PF_NOSFX,
...
void CG_GrenadeTrail (vec3_t start, vec3_t end)
{
	...
	for (; len>0 ; VectorAdd (move, vec, move)) {
		...
		makePart (
			...
			rnum,							rnum,							rnum,
			rnum2,							rnum2,							rnum2,
			0.8f + (crand () * 0.1f),		-2 / (1 + (cg_particleSmokeLinger->value * 0.1f) + (crand () * 0.2f)),
			20 + (crand () * 3),			30 + (crand () * 5),
...
void CG_Heatbeam (vec3_t start, vec3_t forward)
{
	...
	VectorCopy (start, move);
	VectorSubtract (end, start, vec);
	len = (int)VectorNormalize (vec, vec);
...
void CG_RailTrail (vec3_t start, vec3_t end)
{
	...
	if (cl_railtrail->integer >= 1) {
		...
		VectorSubtract (end, start, vec);
		len = VectorNormalize (vec, vec);
		dec = pDecDegree (start, 30, 0.4f, qFalse);
		VectorScale (vec, dec, vec);
		VectorCopy (start, move);
		...
		dec = pDecDegree (start, 3, 0.4f, qFalse);
		VectorScale (vec, dec, vec);
		VectorCopy (start, move);
		...
		for (; len>0 ; VectorAdd (move, vec, move)) {
			...
			makePart (
				move[0],						move[1],						move[2],
				0,								0,								0,
				crand () * 0.5f,				crand () * 0.5f,				crand () * 0.5f,
				0,								0,								0,
				cl_railred->value * 255,		cl_railgreen->value * 255,		cl_railblue->value * 255,
				cl_railred->value * 255,		cl_railgreen->value * 255,		cl_railblue->value * 255,
				1.0f,							-1.0f / (0.6f + (frand () * 0.2f)),
				3,								5,
				pRandSmoke (),					PF_SHADE|PF_NOCLOSECULL,
				pSmokeThink,					qTrue,
				frand () * 360);
	...
	if (tr.fraction < 1) {
		...
		makePart (
			...
			PT_RAIL_WAVE,						PF_ANGLED|PF_SCALED,
			NULL,								qFalse,
			0);
...
void CG_RocketTrail (vec3_t start, vec3_t end)
{
	...
	dec = pDecDegree (start, 2.5f + (inWater ? 1 : 0), 0.33f, qTrue);
	VectorScale (vec, dec, vec);
	...
	for (; len>0 ; VectorAdd (move, vec, move)) {
		...
		makePart (
			...
			rnum,								rnum,								rnum,
			rnum2,								rnum2,								rnum2,
			0.6f + (crand () * 0.1f),			-1.0f / (1.5f + (cg_particleSmokeLinger->value * 0.4f) + (crand () * 0.2f)),
			9 + (crand () * 4),					55 + (frand () * 30),
			pRandSmoke (),						PF_SHADE,
			NULL,								qFalse,
			frand () * 360.0f);
	}
