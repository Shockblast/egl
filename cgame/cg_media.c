...
void CG_InitBaseMedia (void)
{
	...
	// CGame media
	cgMedia.conCharsShader			= cgi.R_RegisterPic (Q_VarArgs ("fonts/%s.tga", cgi.Cvar_GetStringValue ("con_font")));
	...
	// Menu image media
	uiMedia.blackTexture			= cgi.R_RegisterTexture ("***r_blackTexture***", -1);
	uiMedia.noTexture				= cgi.R_RegisterTexture ("***r_noTexture***", -1);
	uiMedia.whiteTexture			= cgi.R_RegisterTexture ("***r_whiteTexture***", -1);

	uiMedia.conCharsShader			= cgi.R_RegisterPic (Q_VarArgs ("fonts/%s.tga", cgi.Cvar_GetStringValue ("con_font")));
	if (!uiMedia.conCharsShader)
		uiMedia.conCharsShader		= cgi.R_RegisterPic ("pics/conchars.tga");
	uiMedia.uiCharsShader			= cgi.R_RegisterPic ("egl/ui/uichars.tga");
...
static void CG_MapMediaInit (void)
{
	...
	CG_IncLoadPercent (CG_MAPMEDIA_RANGE*0.125);

	// Register map effects
	CG_LoadingString ("Loading map fx...");
	CG_MapFXInit (cg.configStrings[CS_MODELS+1]);

	CG_IncLoadPercent (CG_MAPMEDIA_RANGE*0.025);
	...
	for (i=1 ; i<MAX_CS_MODELS && cg.configStrings[CS_MODELS+i][0] ; i++) {
		...
		if (name[0] == '#') {
			...
			if (cg_numWeaponModels < MAX_CLIENTWEAPONMODELS) {
				Q_strncpyz (cg_weaponModels[cg_numWeaponModels], cg.configStrings[CS_MODELS+i]+1, sizeof (cg_weaponModels[cg_numWeaponModels]));
				cg_numWeaponModels++;
			}
		}
		else {
...
=================
*/
static void CG_GloomMediaInit (void)
{
