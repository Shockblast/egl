...
void CG_DrawChar (float x, float y, int flags, float scale, int num, vec4_t color)
{
	...
	if (flags & FS_SHADOW)
		cgi.R_DrawPic (shader, 0, x+FT_SHAOFFSET, y+FT_SHAOFFSET, ftWidth, ftHeight, fcol, frow, fcol+(1.0f/16.0f), frow+(1.0f/16.0f), shdColor);

	cgi.R_DrawPic (shader, 0, x, y, ftWidth, ftHeight, fcol, frow, fcol+(1.0f/16.0f), frow+(1.0f/16.0f), color);
}
...
int CG_DrawString (float x, float y, int flags, float scale, char *string, vec4_t color)
{
	...
	for (i=0 ; *string ; ) {
		...
		if ((num&127) != 32) {
			...
			cgi.R_DrawPic (shader, 0, x, y, ftWidth, ftHeight, fcol, frow, fcol+(1.0f/16.0f), frow+(1.0f/16.0f), strColor);
		}
