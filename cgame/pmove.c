...
static void PM_CheckJump (void) {
	...
	if (!cgi.Cvar_VariableInteger ("dedicated") && glm_jumppred->value && !cgi.Com_ServerState () && cgi.FS_CurrGame ("gloom")) {
		switch (glmClassType) {
...
static qBool PM_GoodPosition (void) {
	...
	origin[0] = end[0] = pm->state.origin[0]*ONEDIV8;
	origin[1] = end[1] = pm->state.origin[1]*ONEDIV8;
	origin[2] = end[2] = pm->state.origin[2]*ONEDIV8;
...
static void PM_SnapPosition (void) {
	...
	for (i=0 ; i<3 ; i++) {
		...
		if (pm->state.origin[i]*ONEDIV8 == pml.origin[i])
			sign[i] = 0;
	}
	VectorCopy (pm->state.origin, base);

	// try all combinations
	for (j=0 ; j<8 ; j++) {
		bits = jitterbits[j];
		VectorCopy (base, pm->state.origin);
		for (i=0 ; i<3 ; i++)
			if (bits & (1<<i))
	...
	VectorCopy (pml.previousOrigin, pm->state.origin);
}
...
static void PM_InitialSnapPosition (void) {
	...
	for (z=0 ; z<3 ; z++) {
		...
		for (y=0 ; y<3 ; y++) {
			...
			for (x=0 ; x<3 ; x++) {
				...
				if (PM_GoodPosition ()) {
					pml.origin[0] = pm->state.origin[0]*ONEDIV8;
					pml.origin[1] = pm->state.origin[1]*ONEDIV8;
					pml.origin[2] = pm->state.origin[2]*ONEDIV8;
					VectorCopy (pm->state.origin, pml.previousOrigin);
					return;
				}
			}
		}
	...
	Com_Printf (PRNT_DEV, S_COLOR_YELLOW "Bad InitialSnapPosition\n");
}
...
void Pmove (pMove_t *pMove, float airAcceleration) {
	...
	pml.velocity[0] = pm->state.velocity[0]*ONEDIV8;
	pml.velocity[1] = pm->state.velocity[1]*ONEDIV8;
	pml.velocity[2] = pm->state.velocity[2]*ONEDIV8;
