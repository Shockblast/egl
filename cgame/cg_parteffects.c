...
===============
*/
qBool CG_FindExplosionDir (vec3_t origin, float radius, vec3_t endPos, vec3_t dir)
{
	...
	for (i=0 ; i<6 ; i++) {
		...
		CG_PMTrace (&tr, tempOrg, NULL, NULL, tempDir, qFalse, qTrue);
...
void CG_BlasterGoldParticles (vec3_t org, vec3_t dir)
{
	...
	CG_SpawnParticle (
		...
		palRed (0xe0 + rnum),				palGreen (0xe0 + rnum),				palBlue (0xe0 + rnum),
		palRed (0xe0 + rnum2),				palGreen (0xe0 + rnum2),			palBlue (0xe0 + rnum2),
		0.9f + (frand() * 0.1f),			-1.0f / (0.6f + (cg_particleSmokeLinger->floatVal * 0.01f) + (frand() * 0.1f)),
		5 + crand (),						16 + (crand () * 8),
		pRandGlowSmoke (),					PF_ALPHACOLOR,
		pBlasterGlowThink,					qTrue,
		PART_STYLE_QUAD,
		frand () * 360);

	// Smoke
	count = 5 + (cg_particleSmokeLinger->floatVal * 0.25f);
	for (i=0 ; i<count ; i++) {
		d = 3 + (frand () * 6);
		rnum = rand () % 5;
		rnum2 = rand () % 5;
...
void CG_BlasterTip (vec3_t start, vec3_t end)
{
	...
	// Smoke
	dec = 1 + (cg_particleSmokeLinger->floatVal * 0.1f);
	for (i=0 ; i<dec ; i++) {
		rnum = rand () % 5;
		rnum2 = rand () % 5;
...
void CG_ExplosionParticles (vec3_t org, float scale, qBool exploOnly, qBool inWater)
{
	...
	if (!exploOnly) {
		...
		for (i=0 ; i<j ; i++) {
			...
			CG_SpawnParticle (
				...
				0.9f + (crand () * 0.1f),		-1.0f / (2.0f + (cg_particleSmokeLinger->floatVal * 0.3f) + (crand () * 0.2f)),
	...
	for (i=1 ; i<j ; i++) {
		...
		CG_SpawnParticle (
			...
			20 + rnum,						10 + rnum,						rnum,
			10 + rnum2,						5 + rnum2,						rnum2,
			0.85f + (crand () * 0.1f),		-1.0f / (1.5f + (cg_particleSmokeLinger->floatVal * 0.1f) + (crand () * 0.2f)),
