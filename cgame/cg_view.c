...
static void V_TestParticles (void) {
	...
	for (i=0 ; i<MAX_PARTICLES ; i++) {
		...
		origin[0] = cg.refDef.viewOrigin[0] + cg.v_Forward[0]*d + cg.v_Right[0]*r + cg.v_Up[0]*u;
		origin[1] = cg.refDef.viewOrigin[1] + cg.v_Forward[1]*d + cg.v_Right[1]*r + cg.v_Up[1]*u;
		origin[2] = cg.refDef.viewOrigin[2] + cg.v_Forward[2]*d + cg.v_Right[2]*r + cg.v_Up[2]*u;
...
static void V_TestLights (void) {
	...
	for (i=0 ; i<32 ; i++) {
		...
		origin[0] = cg.refDef.viewOrigin[0] + cg.v_Forward[0]*f + cg.v_Right[0]*r;
		origin[1] = cg.refDef.viewOrigin[1] + cg.v_Forward[1]*f + cg.v_Right[1]*r;
		origin[2] = cg.refDef.viewOrigin[2] + cg.v_Forward[2]*f + cg.v_Right[2]*r;
...
static void V_CalcViewValues (void) {
	...
	if (cgi.Cvar_VariableInteger ("timedemo"))
		...
		cg.lerpFrac = 1.0 - (cg.frame.serverTime - cg.refreshTime)*0.01;
	...
	if (cl_predict->integer && !(cg.frame.playerState.pMove.pmFlags & PMF_NO_PREDICTION)) {
		...
		if (delta < 100)
			cg.refDef.viewOrigin[2] -= cg.predicted.step * (100 - delta) * 0.01;
	}
	else {
...
void V_RenderView (int vidWidth, int vidHeight, int realTime, float netFrameTime, float refreshFrameTime, float stereoSeparation, qBool forceRefresh) {
	...
	cg.netTime += netFrameTime * 1000.0;
	cgs.refreshFrameTime = refreshFrameTime;
	cg.refreshTime += refreshFrameTime * 1000.0;
	...
	if (cg.frame.valid && (forceRefresh || !cgi.Cvar_VariableInteger ("paused"))) {
		...
		cg.refDef.time		= cg.refreshTime * 0.001;
...
void V_Register (void) {
	...
	CGI_Cmd_AddCommand ("sizeup",		V_SizeUp_f,		"Increases viewport size");
	CGI_Cmd_AddCommand ("sizedown",		V_SizeDown_f,	"Decreases viewport size");

	CGI_Cmd_AddCommand ("viewpos",		V_Viewpos_f,	"Prints view position and yaw");
}
...
void V_Unregister (void) {
	CGI_Cmd_RemoveCommand ("sizeup");
	CGI_Cmd_RemoveCommand ("sizedown");

	CGI_Cmd_RemoveCommand ("viewpos");
}
