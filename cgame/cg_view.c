...
static void V_TileClear (void)
{
	...
	V_TileRect (0, 0, cg.glConfig.vidWidth, top);

	// Clear below view screen
	V_TileRect (0, bottom, cg.glConfig.vidWidth, cg.glConfig.vidHeight - bottom);

	// Clear left of view screen
	V_TileRect (0, top, left, bottom - top + 1);

	// Clear right of view screen
	V_TileRect (right, top, cg.glConfig.vidWidth - right, bottom - top + 1);
}
...
static void V_CalcVrect (void)
{
	...
	cg.refDef.width = cg.glConfig.vidWidth*size/100;
	cg.refDef.width &= ~7;

	cg.refDef.height = cg.glConfig.vidHeight*size/100;
	cg.refDef.height &= ~1;

	cg.refDef.x = (cg.glConfig.vidWidth - cg.refDef.width)/2;
	cg.refDef.y = (cg.glConfig.vidHeight - cg.refDef.height)/2;
...
{
	cgi.Cvar_SetValue ("viewsize", viewsize->integer + 10.0f, qTrue);
}
...
static void V_TestParticles (void)
{
	...
	for (i=0 ; i<PT_PICTOTAL ; i++) {
		...
		// Center
		origin[0] = cg.refDef.viewOrigin[0] + cg.refDef.forwardVec[0]*d + cg.refDef.rightVec[0]*r + cg.refDef.upVec[0]*u;
		origin[1] = cg.refDef.viewOrigin[1] + cg.refDef.forwardVec[1]*d + cg.refDef.rightVec[1]*r + cg.refDef.upVec[1]*u;
		origin[2] = cg.refDef.viewOrigin[2] + cg.refDef.forwardVec[2]*d + cg.refDef.rightVec[2]*r + cg.refDef.upVec[2]*u;

		// Top left
		*(int *)v_testParticleList[i].outColor[0] = *(int *)outColor;
		Vector2Set (v_testParticleList[i].outCoords[0], 0, 0);
		VectorSet (v_testParticleList[i].outVertices[0],	origin[0] + cg.refDef.upVec[0]*scale - cg.refDef.rightVec[0]*scale,
															origin[1] + cg.refDef.upVec[1]*scale - cg.refDef.rightVec[1]*scale,
															origin[2] + cg.refDef.upVec[2]*scale - cg.refDef.rightVec[2]*scale);

		// Bottom left
		*(int *)v_testParticleList[i].outColor[0] = *(int *)outColor;
		Vector2Set (v_testParticleList[i].outCoords[1], 0, 1);
		VectorSet (v_testParticleList[i].outVertices[1],	origin[0] - cg.refDef.upVec[0]*scale - cg.refDef.rightVec[0]*scale,
															origin[1] - cg.refDef.upVec[1]*scale - cg.refDef.rightVec[1]*scale,
															origin[2] - cg.refDef.upVec[2]*scale - cg.refDef.rightVec[2]*scale);
		...
		*(int *)v_testParticleList[i].outColor[0] = *(int *)outColor;
		...
		VectorSet (v_testParticleList[i].outVertices[2],	origin[0] - cg.refDef.upVec[0]*scale + cg.refDef.rightVec[0]*scale,
															origin[1] - cg.refDef.upVec[1]*scale + cg.refDef.rightVec[1]*scale,
															origin[2] - cg.refDef.upVec[2]*scale + cg.refDef.rightVec[2]*scale);
		...
		*(int *)v_testParticleList[i].outColor[0] = *(int *)outColor;
		...
		VectorSet (v_testParticleList[i].outVertices[3],	origin[0] + cg.refDef.upVec[0]*scale + cg.refDef.rightVec[0]*scale,
															origin[1] + cg.refDef.upVec[1]*scale + cg.refDef.rightVec[1]*scale,
															origin[2] + cg.refDef.upVec[2]*scale + cg.refDef.rightVec[2]*scale);
...
static void V_TestEntities (void)
{
	...
	for (i=0 ; i<32 ; i++) {
		...
		for (j=0 ; j<3 ; j++) {
			ent.origin[j] = cg.refDef.viewOrigin[j] + cg.refDef.forwardVec[j]*f + cg.refDef.rightVec[j]*r;
...
static void V_TestLights (void)
{
	...
	for (i=0 ; i<32 ; i++) {
		...
		origin[0] = cg.refDef.viewOrigin[0] + cg.refDef.forwardVec[0]*f + cg.refDef.rightVec[0]*r;
		origin[1] = cg.refDef.viewOrigin[1] + cg.refDef.forwardVec[1]*f + cg.refDef.rightVec[1]*r;
		origin[2] = cg.refDef.viewOrigin[2] + cg.refDef.forwardVec[2]*f + cg.refDef.rightVec[2]*r;
...
static void V_CalcThirdPersonView (void)
{
	...
	// Move up
	VectorMA (cg.refDef.viewOrigin, -backDist, cg.refDef.forwardVec, end);
	VectorMA (end, upDist, cg.refDef.upVec, end);
	...
	// Clip and adjust aim
	if (cg_thirdPersonClip->integer) {
		VectorMA (cg.refDef.viewOrigin, 8192, cg.refDef.forwardVec, dir);
		ClipCam (cg.refDef.viewOrigin, dir, newAngles);

		VectorSubtract (newAngles, camPosition, dir);
		VectorNormalizef (dir, dir);
		VecToAngles (dir, newAngles);

		// Apply
		VectorCopy (newAngles, cg.refDef.viewAngles);
		Angles_Vectors (cg.refDef.viewAngles, cg.refDef.forwardVec, cg.refDef.rightVec, cg.refDef.upVec);
...
static void V_CalcViewValues (void)
{
	...
	Vector4Copy (ps->viewBlend, cg.viewBlend);
...
void V_RenderView (int realTime, float netFrameTime, float refreshFrameTime, float stereoSeparation, qBool refreshPrepped)
{
	...
	if (cg.frame.valid && (cg.forceRefDef || !cgi.Cvar_GetIntegerValue ("paused"))) {
		...
		if (stereoSeparation != 0)
			VectorMA (cg.refDef.viewOrigin, stereoSeparation, cg.refDef.rightVec, cg.refDef.viewOrigin);
		...
		cg.refDef.fovY			= CalcFovY (cg.refDef.fovX, (float)cg.refDef.width, (float)cg.refDef.height);
...
static void V_Benchmark_f (void)
{
	...
	refDef.width = cg.glConfig.vidWidth;
	refDef.height = cg.glConfig.vidHeight;
	refDef.fovX = 90;
	refDef.fovY = CalcFovY (refDef.fovX, refDef.width, refDef.height);
	VectorMA (cg.frame.playerState.viewOffset, (1.0f/8.0f), cg.frame.playerState.pMove.origin, refDef.viewOrigin);
	...
	for (j=0, result=0, timeinc=0 ; j<times ; j++) {
		...
		if (cgi.Cmd_Argc () >= 3) {
			...
			for (i=0 ; i<128 ; i++) {
				...
				Angles_Vectors (refDef.viewAngles, refDef.forwardVec, refDef.rightVec, refDef.upVec);
