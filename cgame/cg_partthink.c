...
static trace_t pTrace (vec3_t start, vec3_t end, float size) {
	return CGI_CM_Trace (start, end, size, 1);
}
...
void pBloodThink (struct particle_s *p, vec3_t org, vec3_t angle, vec4_t color, float *size, float *orient, float *time)
{
	...
	if (p->shader == cgMedia.particleTable[PT_GRNBLOOD]
	|| p->shader == cgMedia.particleTable[PT_GRNBLOOD2]
	|| p->shader == cgMedia.particleTable[PT_GRNBLOOD3]
	|| p->shader == cgMedia.particleTable[PT_GRNBLOOD4]
	|| p->shader == cgMedia.particleTable[PT_GRNBLOOD5]
	|| p->shader == cgMedia.particleTable[PT_GRNBLOOD6]
	|| p->shader == cgMedia.particleTable[PT_BLDDRIP_GRN])
		isGreen = qTrue;
	else
		isGreen = qFalse;
	...
	if (tr.startSolid || tr.allSolid || tr.fraction < 1) {
		if (!(p->flags & PF_NODECAL)) {
			...
			makeDecal (
				...
				0,
				(13 + (crand()*3)) * sizescale,
				isGreen ? dRandGrnBloodMark () : dRandBloodMark (),
				DF_ALPHACOLOR,
				0,								qFalse,
				0,								frand () * 360.0f);

			if (!(p->flags & PF_NOSFX) && (cgs.realTime > sfxDelay)) {
				sfxDelay = cgs.realTime + 300;
