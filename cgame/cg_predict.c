...
void CG_CheckPredictionError (void) {
	...
	if (len > 640) {
		...
		VectorScale (delta, ONEDIV8, cg.predicted.error);
	}
}
...
void CG_PredictMovement (void) {
	...
	if (!cl_predict->value || (cg.frame.playerState.pMove.pmFlags & PMF_NO_PREDICTION)) {
		...
		VectorScale (cg.frame.playerState.pMove.velocity, ONEDIV8, cg.predicted.velocity);
	...
	if ((step > 63) && (step < 160) && (pm.state.pmFlags & PMF_ON_GROUND) && (current != stepFrameLast)) {
		...
		cg.predicted.step = step * ONEDIV8;
		cg.predicted.stepTime = cgs.realTime - cgs.netFrameTime * 500;
	}

	VectorScale (pm.state.velocity, ONEDIV8, cg.predicted.velocity);
	VectorScale (pm.state.origin, ONEDIV8, cg.predicted.origin);
