...
void CG_ExploRattle (vec3_t org, float scale) {
	...
	for (i=0 ; i<MAX_TENT_EXPLORATTLES ; i++) {
		...
		// calculate distance
		dist = VectorDistFast (cg.refDef.viewOrigin, org) * 0.1;
		max = (20 * scale, 20, 50);
...
static void CG_AddExploRattles (void) {
	...
	scale = clamp (cl_explorattle_scale->value, 0, 0.95);
	for (i=0 ; i<MAX_TENT_EXPLORATTLES ; i++) {
		if (cgExploRattles[i] <= 0)
			continue;
	...
	for (i=0, b=cgPlayerBeams ; i<MAX_TENT_BEAMS ; i++, b++) {
		...
		if (cgMedia.heatBeamMOD && (b->model == cgMedia.heatBeamMOD)) {
			...
			if (b->entity == cg.playerNum+1) {	
				...
				VectorMA (b->start,	(hand_multiplier * b->offset[0]),	cg.rightVec,	org);
				VectorMA (org,		b->offset[1],						cg.forwardVec,	org);
				VectorMA (org,		b->offset[2],						cg.upVec,		org);

				if (hand->integer == 2)
					VectorMA (org, -1, cg.upVec, org);

				// FIXME - take these out when final
				VectorCopy (cg.rightVec, right);
				VectorCopy (cg.forwardVec, fwd);
				VectorCopy (cg.upVec, up);
...
void CG_ParseTempEnt (void) {
	...
	case TE_BULLET_SPARKS:
		...
		if (type != TE_SPARKS) {
			...
			cnt = rand()&15;
			switch (cnt) {
			case 1:	cgi.Snd_StartSound (pos, 0, CHAN_AUTO, cgMedia.ricochetSfx[0], 1, ATTN_NORM, 0);	break;
			case 2:	cgi.Snd_StartSound (pos, 0, CHAN_AUTO, cgMedia.ricochetSfx[1], 1, ATTN_NORM, 0);	break;
			case 3:	cgi.Snd_StartSound (pos, 0, CHAN_AUTO, cgMedia.ricochetSfx[2], 1, ATTN_NORM, 0);	break;
			default:
				break;
			}
		}
	...
	case TE_SHIELD_SPARKS:
		...
		cgi.Snd_StartSound (pos, 0, CHAN_AUTO, cgMedia.laserHitSfx, 1, ATTN_NORM, 0);
		break;
	...
	case TE_SPLASH:		// bullet hitting water
		...
		if (r == SPLASH_SPARKS) {
			r = (rand()%3);
			switch (r) {
			case 0:		cgi.Snd_StartSound (pos, 0, CHAN_AUTO, cgMedia.sparkSfx[4], 1, ATTN_STATIC, 0);	break;
			case 1:		cgi.Snd_StartSound (pos, 0, CHAN_AUTO, cgMedia.sparkSfx[5], 1, ATTN_STATIC, 0);	break;
			default:	cgi.Snd_StartSound (pos, 0, CHAN_AUTO, cgMedia.sparkSfx[6], 1, ATTN_STATIC, 0);	break;
			}
		}
		break;
	...
	case TE_RAILTRAIL:	// railgun effect
		...
		// start
		cgi.Snd_StartSound (pos, 0, CHAN_AUTO, cgMedia.mz.railgunFireSfx, 1, ATTN_NORM, 0);

		// end
		cgi.Snd_StartSound (pos2, 0, CHAN_AUTO, cgMedia.mz.railgunFireSfx, 1, ATTN_NORM, 0);
		break;
	...
	case TE_GRENADE_EXPLOSION_WATER:
		...
		if (type == TE_GRENADE_EXPLOSION_WATER) {
			cgi.Snd_StartSound (pos, 0, CHAN_AUTO, cgMedia.waterExploSfx, 1, ATTN_NORM, 0);
			CG_ExplosionParticles (pos, 1, qFalse, qTrue);
		}
		else {
			cgi.Snd_StartSound (pos, 0, CHAN_AUTO, cgMedia.grenadeExploSfx, 1, ATTN_NORM, 0);
	...
	case TE_PLASMA_EXPLOSION:
		cgi.MSG_ReadPos (pos);
		CG_ExplosionParticles (pos, 1, qFalse, qFalse);
		cgi.Snd_StartSound (pos, 0, CHAN_AUTO, cgMedia.rocketExploSfx, 1, ATTN_NORM, 0);
		break;
	...
	case TE_BOSSTPORT:	// boss teleporting to station
		...
		cgi.Snd_StartSound (pos, 0, CHAN_AUTO, cgMedia.bigTeleport, 1, ATTN_NONE, 0);
		break;
	...
	case TE_TRACKER_EXPLOSION:
		...
		cgi.Snd_StartSound (pos, 0, CHAN_AUTO, cgMedia.disruptExploSfx, 1, ATTN_NORM, 0);
		break;
