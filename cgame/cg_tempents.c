	...
	for (i=0, b=cgPlayerBeams ; i<MAX_TENT_BEAMS ; i++, b++) {
		...
		if (cgMedia.heatBeamMOD && (b->model == cgMedia.heatBeamMOD)) {
			...
			if (b->entity == cg.playerNum+1) {	
				...
				VectorMA (b->start,	(hand_multiplier * b->offset[0]),	cg.refDef.rightVec,		org);
				VectorMA (org,		b->offset[1],						cg.refDef.forwardVec,	org);
				VectorMA (org,		b->offset[2],						cg.refDef.upVec,		org);

				if (hand->integer == 2)
					VectorMA (org, -1, cg.refDef.upVec, org);

				// FIXME - take these out when final
				VectorCopy (cg.refDef.rightVec, right);
				VectorCopy (cg.refDef.forwardVec, fwd);
