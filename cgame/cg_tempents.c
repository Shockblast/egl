	...
	for (i=0, l=cgLasers ; i<MAX_TENT_LASERS ; i++, l++) {
		if (l->endtime >= cgs.realTime) {
			...
			makePart (
				...
				palRed (clr),					palGreen (clr),					palBlue (clr),
				0.30,							PART_INSTANT,
				l->ent.frame*0.9 + ((l->ent.frame * 0.05) * (rand () & 1)),
				l->ent.frame*0.9 + ((l->ent.frame * 0.05) * (rand () & 1)),
				PT_BEAM,						PF_BEAM,
			...
			makePart (
				...
				palRed (clr),					palGreen (clr),					palBlue (clr),
				0.30,							PART_INSTANT,
				l->ent.frame*0.3 + ((l->ent.frame*0.05) * (rand () & 1)),
				l->ent.frame*0.3 + ((l->ent.frame*0.05) * (rand () & 1)),
				PT_BEAM,						PF_BEAM,
				GL_SRC_ALPHA,					GL_ONE,
				0,								qFalse,
...
static void CG_ParseLaser (int colors) {
	...
	for (i=0, l=cgLasers ; i<MAX_TENT_LASERS ; i++, l++) {
		if (l->endtime < cgs.realTime) {
			...
			for (j=0 ; j<3 ; j++)
				CG_BeamTrail (start, end, clr, 2 + (rand () & 1), 0.30, -2);
			...
			makePart (
				...
				palRed (clr),						palGreen (clr),						palBlue (clr),
				0.30,								-2.1,
				(4*0.9)/3 + ((4*0.05)*(rand()&1)),	(4*0.9)/3 + ((4*0.05)*(rand()&1)),
				PT_BEAM,							PF_BEAM,
				GL_SRC_ALPHA,						GL_ONE,
				0,									qFalse,
				0);
