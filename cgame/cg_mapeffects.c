...
void CG_ShutdownMapFX (void)
{
	...
	CGI_Cmd_RemoveCommand ("addmfx", cmd_addMFX);
...
void CG_MapFXInit (char *mapName)
{
	...
	cmd_addMFX = CGI_Cmd_AddCommand ("addmfx",	CG_AddMFX_f,		"Appends a generic effect to this map's mfx file");
	...
	Q_strncpyz (cg_mfxMapName, mapName, sizeof (cg_mfxMapName));
	Q_snprintfz (cg_mfxFileName, sizeof (cg_mfxFileName), "mfx/%s.mfx", mapName);
	...
	while (token)  {
		if (stageNum == 0) {
			if (cg_numMapEffects >= MAPFX_MAXFX) {
				Com_Printf (0, S_COLOR_RED "CG_MapFXInit: MAPFX_MAXFX (%d) mapfx hit\n", cg_numMapEffects);
				break;
			}
	...
	if (stageNum != 0) {
		Com_Printf (0, S_COLOR_RED "CG_MapFXInit: Bad file '%s'\n", cg_mfxFileName);
		CG_ClearMapFX ();
	}
