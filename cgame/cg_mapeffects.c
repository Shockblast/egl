...
static void CG_ClearMapFX (void) {
	...
	memset (&mfxMapName, 0, sizeof (mfxMapName));
	memset (&mfxFileName, 0, sizeof (mfxFileName));
}
...
void CG_MapFXInit (char *mapName) {
	...
	CGI_Cmd_AddCommand ("addmfx",	CG_AddMFX_f,		"Appends a generic effect to this map's mfx file");
