...
static void CG_AddEntityShells (entity_t *ent)
{
	// Double
...
void CG_AddPacketEntities (void)
{
	...
	VectorSet (autoRotate, 0, AngleMod (cgs.realTime * 0.1f), 0);
	AnglesToAxis (autoRotate, autoRotateAxis);

	autoAnim = cgs.realTime / 1000;	// brush models can auto animate their frames
	...
	for (pNum=0 ; pNum<cg.frame.numEntities ; pNum++) {
		...
		if (ent.model) {
			...
			else if (cgs.currGameMod == GAME_MOD_XATRIX) {
				// Ugly phalanx tip
		...
		if (effects & EF_ROTATE) {
			...
			angles[0] = 0;
			angles[1] = AngleMod (cgs.realTime/2.0f) + state->angles[1];
			angles[2] = 180;
			...
			if (isSelf && isPred) { // FIXME fix for when you die k
				...
				angles[0] = InterpolateAngle (cent->prev.angles[0], cent->current.angles[0], cg.lerpFrac);
				angles[1] = InterpolateAngle (cent->prev.angles[1], cent->current.angles[1], cg.lerpFrac);
				angles[2] = InterpolateAngle (cent->prev.angles[2], cent->current.angles[2], cg.lerpFrac);
			}
		...
		if (effects & EF_COLOR_SHELL) {
			if (ent.flags & RF_SHELL_HALF_DAM) {
				if (cgs.currGameMod == GAME_MOD_ROGUE) {
		...
		if (isSelf) {
			if (cg_thirdPerson->integer && cl_predict->value && !(cg.frame.playerState.pMove.pmFlags & PMF_NO_PREDICTION) && !cg.attractLoop) {
		...
		if ((cg.refDef.rdFlags & RDF_IRGOGGLES) && (ent.flags & RF_IR_VISIBLE)) {
			...
			VectorSet (ent.color, 255, 0, 0);
		}
		else if (cg.refDef.rdFlags & RDF_UVGOGGLES) {
			ent.flags |= RF_FULLBRIGHT;
			VectorSet (ent.color, 0, 255, 0);
		}
		...
		if (effects &~ EF_ROTATE) {
			...
			// PGM - Do not reorder EF_BLASTER and EF_HYPERBLASTER
				...
				if (effects & EF_TRACKER)						// PGM	overloaded for blaster2.
					...
					if (ent.model && cgs.currGameMod == GAME_MOD_GLOOM && !Q_stricmp ((char *)ent.model, "sprites/s_shine.sp2")) {
						...
						if (glm_flashpred->integer && flPred && flPredLastTime != cgs.realTime) {
							playerStateNew_t	*ps;
							vec3_t				org, forward;
							trace_t				tr;
							...
							tr = CGI_CM_BoxTrace (org, forward, cg_gloomFLightMins, cg_gloomFLightMaxs, 0,
												CONTENTS_SOLID|CONTENTS_MONSTER|CONTENTS_DEADMONSTER);

							// Check if it's close
							VectorSubtract (tr.endPos, ent.origin, forward); 
							if (VectorLengthFast (forward) > 256) {
								VectorCopy (ent.origin, flOrg);
							}
							else {
								flPredLastTime = cgs.realTime;

								// Project a flashlight
								VectorScale (cg.forwardVec, 2048, forward);
								VectorAdd (forward, cg.refDef.viewOrigin, forward);
								tr = CGI_CM_BoxTrace (cg.refDef.viewOrigin, forward, cg_gloomFLightMins, cg_gloomFLightMaxs, 0,
													CONTENTS_SOLID|CONTENTS_MONSTER|CONTENTS_DEADMONSTER);

								VectorCopy (tr.endPos, flOrg);
							}
						}
						else
							VectorCopy (ent.origin, flOrg);
			...
			//ROGUE
				...
				if (effects & EF_TRACKER) {
					...
					intensity = 50 + (500 * ((float)sin (cgs.realTime/500.0f) + 1.0f));
