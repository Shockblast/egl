...
void CG_AddPacketEntities (void)
{
	...
	VectorSet (autoRotate, 0, AngleModf (cg.realTime * 0.1f), 0);
			...
			// PGM - Do not reorder EF_BLASTER and EF_HYPERBLASTER
				...
				if (effects & EF_TRACKER)						// PGM	overloaded for blaster2.
					...
					if (ent.model && cg.currGameMod == GAME_MOD_GLOOM && !Q_stricmp ((char *)ent.model, "sprites/s_shine.sp2")) {
						...
						if (glm_flashpred->integer && flPred && flPredLastTime != cg.realTime) {
							playerStateNew_t	*ps;
							vec3_t				org, forward;
							trace_t				tr;
							...
							if (VectorLengthFast (forward) > 256) {
								...
								VectorScale (cg.refDef.forwardVec, 2048, forward);
...
void CG_GetEntitySoundOrigin (int entNum, vec3_t origin, vec3_t velocity)
{
	...
	if (ent->current.renderFx & (RF_FRAMELERP|RF_BEAM)) {
		...
		VectorSubtract (ent->current.origin, ent->current.oldOrigin, velocity);
		...
		VectorSubtract (ent->current.origin, ent->prev.origin, velocity);
		VectorScale (velocity, 10, velocity);
	}
