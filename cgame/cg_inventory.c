...
void Inv_DrawInventory (void) {
	...
	x = (cg.vidWidth - (256 * HUD_SCALE)) * 0.5;
	y = (cg.vidHeight - (240 * HUD_SCALE)) * 0.5;

	// draw backsplash image
	cgi.Img_GetSize (cgMedia.hudInventoryImage, &w, &h);
	cgi.Draw_Pic (
		cgMedia.hudInventoryImage, x, y + HUD_FTSIZE,
	...
	// head of list
	cgi.Draw_String (NULL, x, y, 0, HUD_SCALE, S_STYLE_SHADOW "hotkey ### item", color);
	cgi.Draw_String (NULL, x, y+HUD_FTSIZE, 0, HUD_SCALE, S_STYLE_SHADOW "------ --- ----", color);
	...
	for (i=top ; i<num && i<top+INV_DISPLAY_ITEMS ; i++) {
		...
		if (item == selected) {
			cgi.Draw_Fill (x, y, 26*HUD_FTSIZE, HUD_FTSIZE, selBarColor);

			// draw blinky cursors by the selected item
			if ((cg.frame.serverFrame>>2) & 1) {
				cgi.Draw_Char (NULL, x-HUD_FTSIZE, y, FS_SHADOW, HUD_SCALE, 15, colorRed);
				cgi.Draw_Char (NULL, x+(26*HUD_FTSIZE), y, FS_SHADOW, HUD_SCALE, 15, colorRed);
			}
		}
		cgi.Draw_StringLen (NULL, x, y, (item == selected) ? 0 : FS_SECONDARY, HUD_SCALE, string, 26, color);
		y += HUD_FTSIZE;
	}
}
