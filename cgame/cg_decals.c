...
static uint32	cg_numDecals;

static byte		cg_decalCullGroups[MAX_DECALS];
static uint32	cg_numDecalCullGroups;
						...
						float lifeTime,				 float angle)
{
	...
	// Decal toggling
	if (!cg_decals->integer || !cg_decalLife->value || !cg_freeDecals)
		return NULL;
	...
	if (!numFragments)
		return NULL;
	if (cg_numDecals + numFragments >= cg_decalMax->value)
		return NULL;
	if (cg_numDecals + numFragments >= MAX_DECALS)
		return NULL;
	...
	for (i=0, fr=fragments ; i<numFragments ; i++, fr++) {
		...
		d->node = fr->node;
		d->cullGroup = cg_numDecalCullGroups;

		// Allocate coords and vertices
		buffer = CG_AllocTag ((fr->numVerts * sizeof (float) * 5) + (fr->numVerts * sizeof (bvec4_t)), qFalse, CGTAG_DECALS);
		coords = d->coords = (float *)buffer;

		buffer += fr->numVerts * sizeof (float) * 2;
		vertices = d->vertices = (float *)buffer;
	...
	cg_numDecalCullGroups = (cg_numDecalCullGroups + 1) % MAX_DECALS;
	return d;
}
...
void CG_ClearDecals (void)
{
	...
	memset (cg_decalList, 0, sizeof (cg_decalList));
...
void CG_AddDecals (void)
{
	...
	// Clear cull status
	for (j=0 ; j<cg_numDecalCullGroups ; j++)
		cg_decalCullGroups[j] = 0;
	...
	for (d=cg_activeDecals ; d ; d=next) {
		...
		if (d->colorVel[3] > DECAL_INSTANT) {
			...
			if (!(d->flags & DF_NOTIMESCALE))
				lifeTime += cg_decalLife->value;

			// Calculate alpha ramp
			color[3] = clamp (((lifeTime - ((cg.realTime - d->time) * 0.001f)) / lifeTime), -1, 1);
		}
		else {
			color[3] = d->color[3];
		}
		...
		// Skip if this group is already culled
		switch (cg_decalCullGroups[d->cullGroup]) {
		case 0:
			// Skip if not in current visibility set
			if (cgi.R_CullVisNode (d->node)) {
				cg_decalCullGroups[d->cullGroup] = 2;
				goto nextDecal;
			}
			...
			cg_decalCullGroups[d->cullGroup] = 1;
			break;

		case 1:
			// Already tried, it wasn't cullable
			break;

		case 2:
			// Tried and was culled
			goto nextDecal;
		}

		// Small decal lod
		if (cg_decalLOD->integer && (d->size < 12)) {
		...
		if (d->flags & DF_AIRONLY) {
			...
			if (cgi.CM_PointContents (d->org, 0) & pointBits) {
				color[3] = 0;
				d->color[3] = 0;
				d->colorVel[3] = 0;
				goto nextDecal;
