...
=============
*/
static qBool R_CullAliasModel (refEntity_t *ent)
{
	cBspPlane_t		*p;
	...
	if (r_sphereCull->intVal) {
		if (R_CullSphere (ent->origin, r_aliasRadius, 31))
			return qTrue;
	}
	else {
		vec3_t	bbox[8];
		...
		for (i=0 ; i<8 ; i++) {
			switch (i) {
			case 0:	tmp[0] = r_aliasMaxs[0];	tmp[1] = r_aliasMaxs[1];	tmp[2] = r_aliasMaxs[2];	break;
			case 1:	tmp[0] = r_aliasMins[0];	tmp[1] = r_aliasMaxs[1];	tmp[2] = r_aliasMaxs[2];	break;
			case 2:	tmp[0] = r_aliasMaxs[0];	tmp[1] = r_aliasMins[1];	tmp[2] = r_aliasMaxs[2];	break;
			case 3:	tmp[0] = r_aliasMins[0];	tmp[1] = r_aliasMins[1];	tmp[2] = r_aliasMaxs[2];	break;
			case 4:	tmp[0] = r_aliasMaxs[0];	tmp[1] = r_aliasMaxs[1];	tmp[2] = r_aliasMins[2];	break;
			case 5:	tmp[0] = r_aliasMins[0];	tmp[1] = r_aliasMaxs[1];	tmp[2] = r_aliasMins[2];	break;
			case 6:	tmp[0] = r_aliasMaxs[0];	tmp[1] = r_aliasMins[1];	tmp[2] = r_aliasMins[2];	break;
			case 7:	tmp[0] = r_aliasMins[0];	tmp[1] = r_aliasMins[1];	tmp[2] = r_aliasMins[2];	break;
			}
	...
	if (ri.scn.mirrorView || ri.scn.portalView) {
		if (PlaneDiff (ent->origin, &ri.scn.clipPlane) < -r_aliasRadius) {
...
void R_AddAliasModelToList (refEntity_t *ent)
{
	...
	if (!gl_shadows->intVal && R_CullAliasModel (ent))
		return;
	...
	for (meshNum=0 ; meshNum<model->numMeshes ; meshNum++) {
		...
		R_AddMeshToList (shader, ent->shaderTime, ent, R_FogForSphere (ent->origin, r_aliasRadius), MBT_ALIAS, aliasMesh);
	}
}
