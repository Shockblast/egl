...
static void R_UpdateCvars (void)
{
	...
	// Clamp shadowing
#ifndef _DEBUG
	if (gl_shadows->modified) {
		gl_shadows->modified = qFalse;
		if (gl_shadows->intVal < 0)
			Cvar_VariableSetValue (gl_shadows, 0, qTrue);
		else if (gl_shadows->intVal > 2)
			Cvar_VariableSetValue (gl_shadows, 2, qTrue);
	}
#endif
