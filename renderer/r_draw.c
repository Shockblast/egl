...
=============
*/
void Draw_ImagePic (image_t *image, float x, float y, int w, int h, float s1, float t1, float s2, float t2, vec4_t color) {
	if (!image)
		return;
...
void Draw_CheckFont (void) {
	...
	glMedia.charsImage = Img_RegisterImage (Q_VarArgs ("fonts/%s.tga", con_font->string), IF_NOMIPMAP|IF_NOPICMIP|IF_NOGAMMA|IF_NOINTENS|IF_CLAMP);
	if (!glMedia.charsImage) {
		glMedia.charsImage = Img_RegisterImage ("pics/conchars.tga", IF_NOMIPMAP|IF_NOPICMIP|IF_NOGAMMA|IF_NOINTENS|IF_CLAMP);
		if (!glMedia.charsImage)
