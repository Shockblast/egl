...
void Draw_Pic (image_t *image, float x, float y, int w, int h, float s1, float t1, float s2, float t2, vec4_t color) {
	...
	shader = RS_RegisterShader (image->bareName);
...
void Draw_StretchPic (char *pic, float x, float y, int w, int h, vec4_t color) {
	...
	if (!img) {
		Com_Printf (PRINT_ALL, S_COLOR_YELLOW "ERROR: Can't find pic: %s\n", pic);
		return;
	}
...
void Draw_CheckFont (void) {
	...
	glMedia.charsImage = Img_FindImage (va ("fonts/%s.pcx", con_font->string), IF_NOFLUSH|IF_NOMIPMAP|IF_NOPICMIP|IF_NOGAMMA|IF_NOINTENS|IF_CLAMP);
...
int Draw_String (float x, float y, int flags, float scale, char *string, vec4_t color) {
	...
	if (flags & FS_UIFONT)
		charsImg = glMedia.uiCharsImage;
	else
		charsImg = glMedia.charsImage;
	...
	for (i=0 ; *string ; ) {
		...
		if ((num&127) != 32) {
			...
			if (isShadowed)
				Draw_ImagePic (charsImg, x+FT_SHAOFFSET, y+FT_SHAOFFSET, ftWidth, ftHeight, fcol, frow, fcol+ONEDIV16, frow+ONEDIV16, shdColor);

			Draw_ImagePic (charsImg, x, y, ftWidth, ftHeight, fcol, frow, fcol+ONEDIV16, frow+ONEDIV16, strColor);
		}
...
int Draw_StringLen (float x, float y, int flags, float scale, char *string, int len, vec4_t color) {
	...
	if (len < 0)
		return Draw_String (x, y, flags, scale, string, color);

	swap = string[len];
	string[len] = 0;
	length = Draw_String (x, y, flags, scale, string, color);
	string[len] = swap;
...
void Draw_Char (float x, float y, int flags, float scale, int num, vec4_t color) {
	...
	Draw_ImagePic (charsImg, x, y, ftWidth, ftHeight, fcol, frow, fcol+ONEDIV16, frow+ONEDIV16, color);
}
