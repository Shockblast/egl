...
static qBool	triangleFacingLight[VERTARRAY_MAX_TRIANGLES];
...
static void R_DrawShadowVolume (void)
{
	...
	if (glConfig.extDrawRangeElements)
		qglDrawRangeElementsEXT (GL_TRIANGLES, 0, rb_numVerts * 2, numShadowVolumeTris * 3, GL_UNSIGNED_INT, shadowVolumeIndexes);
	else
...
static void R_CastShadowVolume (entity_t *ent, vec3_t mins, vec3_t maxs, float radius, vec3_t lightOrigin, float intensity)
{
	...
	if (!Matrix_Compare (ent->axis, axisIdentity))
		Matrix_TransformVector (ent->axis, lightDist2, lightDist);
	else
...
{
	if (gl_shadows->integer != SHADOW_VOLUMES || !glConfig.useStencil)
		return;
