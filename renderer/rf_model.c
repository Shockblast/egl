...
static int			r_q2_numEdges;
static uint16		*r_q2_edges;

static int			r_q2_numSurfEdges;
static int			*r_q2_surfEdges;

static int			r_q2_numVertexes;
...
static qBool R_BuildQ2BSPSurface (refModel_t *model, mBspSurface_t *surf)
{
	...
	// Check mesh validity
	if (RB_InvalidMesh (outMesh->numVerts, outMesh->numIndexes)) {
...
{
	if (!surf->mesh || RB_InvalidMesh (surf->mesh->numVerts, surf->mesh->numIndexes))
		return qFalse;
...
static qBool R_LoadQ2BSPVertexes (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	r_q2_numVertexes = lump->fileLen / sizeof (*in);
	r_q2_vertexes = out = R_ModAllocZero (model, sizeof(vec3_t) * r_q2_numVertexes);

	//
	// Byte swap
	//
	for (i=0 ; i<r_q2_numVertexes ; i++, in++, out+=3) {
...
static qBool R_LoadQ2BSPEdges (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	r_q2_numEdges = lump->fileLen / sizeof(*in);
	r_q2_edges = out = R_ModAllocZero (model, sizeof(uint16) * 2 * (r_q2_numEdges + 1));

	//
	// Byte swap
	//
	for (i=0 ; i<r_q2_numEdges ; i++, in++, out+=2) {
...
{	
	...
	r_q2_numSurfEdges = lump->fileLen / sizeof (*in);
	if (r_q2_numSurfEdges < 1 || r_q2_numSurfEdges >= Q2BSP_MAX_SURFEDGES) {
		Com_Printf (PRNT_ERROR, "R_LoadQ2BSPSurfEdges: invalid surfEdges count in %s: %i (min: 1; max: %d)\n", model->name, r_q2_numSurfEdges, Q2BSP_MAX_SURFEDGES);
		return qFalse;
	}

	r_q2_surfEdges = out = R_ModAllocZero (model, sizeof (*out) * r_q2_numSurfEdges);

	//
	// Byte swap
	//
	for (i=0 ; i<r_q2_numSurfEdges ; i++)
...
static qBool R_LoadQ2BSPFaces (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	// Byte swap
	//
	for (i=0 ; i<model->bspModel.numSurfaces ; i++, in++, out++) {
		...
		if (j < 0 || j >= model->q2BspModel.numTexInfo) {
			Com_Printf (PRNT_ERROR, "R_LoadQ2BSPFaces: bad texInfo number: %i\n", j);
			return qFalse;
		}
		out->q2_texInfo = model->q2BspModel.texInfo + j;
		...
		out->q2_lmSamples = (j == -1) ? NULL : model->q2BspModel.lightData + j;
		...
		//else if (!(out->q2_texInfo->flags & SURF_TEXINFO_SKY)) {
...
static qBool R_LoadQ2BSPMarkSurfaces (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	for (i=0 ; i<model->q2BspModel.numMarkSurfaces ; i++) {
		...
		if (index < 0 || index >= model->bspModel.numSurfaces) {
			Com_Printf (PRNT_ERROR, "R_LoadQ2BSPMarkSurfaces: bad surface number: %i\n", index);
			return qFalse;
		}
		out[i] = model->bspModel.surfaces + index;
	}
...
=================
*/
static qBool R_SetupQ2BSPSubModels (refModel_t *model)
{
	mBspHeader_t	*sm;
	refModel_t		*out;
	uint32			i;

	sm = &model->bspModel.subModels[0];
	out = &model->bspModel.inlineModels[0];
	for (i=0 ; i<model->bspModel.numSubModels ; i++, sm++, out++) {
		*out = *model;

		out->bspModel.firstModelSurface = out->bspModel.surfaces + sm->firstFace;
		out->bspModel.numModelSurfaces = sm->numFaces;
		out->q2BspModel.firstNode = sm->headNode;

		if (out->q2BspModel.firstNode >= model->bspModel.numNodes) {
			Com_Printf (PRNT_ERROR, "R_LoadQ2BSPModel: Inline model number '%i' has a bad firstNode (%d >= %d)\n", i, out->q2BspModel.firstNode, model->bspModel.numNodes);
			return qFalse;
		}
		...
		out->bspModel.numLeafs = sm->visLeafs;
	}
...
=================
*/
static qBool R_FinishQ2BSPModel (refModel_t *model)
{
	mBspLeaf_t		*leaf;
	mBspNode_t		*node;
	mBspSurface_t	*surf;
	...
	uint32			totalLitRemoved, totalVisRemoved, totalFragRemoved;
	int				i, j;
	...
	*/
	...
	//
	for (surf=model->bspModel.surfaces, i=0 ; i<model->bspModel.numSurfaces ; i++, surf++) {
		if (surf->q2_texInfo->shader->flags & SHADER_SUBDIVIDE) {
			if (surf->q2_texInfo->flags & (SURF_TEXINFO_SKY|SURF_TEXINFO_WARP)) {
				if (!R_SubdivideQ2BSPSurface (model, surf, surf->q2_texInfo->shader->subdivide)
				|| !R_ConvertQ2BSPSurface (model, surf))
					return qFalse;
			}
			else {
				if (!R_SubdivideQ2BSPLMSurface (model, surf, surf->q2_texInfo->shader->subdivide)
				|| !R_ConvertQ2BSPSurface (model, surf))
					return qFalse;
			}
		}
	...
	for (node=model->bspModel.nodes, i=0 ; i<model->bspModel.numNodes ; i++, node++) {
	}
	...
	for (node=model->bspModel.nodes, i=0 ; i<model->bspModel.numNodes ; i++, node++) {
		...
		for (j=0 ; j<node->numSurfaces ; j++) {
			surf = model->bspModel.surfaces + node->firstSurface + j;
		...
		totalLitRemoved += node->numSurfaces - numLitSurfs;
		totalVisRemoved += node->numSurfaces - numVisSurfs;
	}
	...
	R_Q2BSP_BeginBuildingLightmaps ();
	totalFragRemoved = 0;
	for (leaf=model->bspModel.leafs, i=0 ; i<model->bspModel.numLeafs ; i++, leaf++) {
		if (!leaf->q2_numMarkSurfaces)
			continue;

		// Calculate the total amount of fragmentable surfaces
		numFragSurfaces = 0;
		for (j=0 ; j<leaf->q2_numMarkSurfaces ; j++) {
			surf = leaf->q2_firstMarkSurface[j];
			if (!surf)
				continue;
			if (R_Q2BSP_SurfPotentiallyVisible (surf)) {
		...
		if (!numFragSurfaces) {
			leaf->q2_firstDecalSurface = NULL;
			continue;
		}

		leaf->q2_firstDecalSurface = R_ModAllocZero (model, sizeof (mBspSurface_t *) * (numFragSurfaces + 1));
		...
		for (j=0 ; j<leaf->q2_numMarkSurfaces ; j++) {
			...
			if (R_Q2BSP_SurfPotentiallyVisible (surf)) {
				if (R_Q2BSP_SurfPotentiallyFragmented (surf))
					leaf->q2_firstDecalSurface[numFragSurfaces++] = surf;
				if (R_Q2BSP_SurfPotentiallyLit (surf))
					R_Q2BSP_CreateSurfaceLightmap (surf);
			}
		}

		totalFragRemoved += leaf->q2_numMarkSurfaces - numFragSurfaces;
	}
	...
	//
	// Batch surfaces with identical planes, shaders, and lightmap texture
	//
...
static qBool R_LoadQ2BSPModel (refModel_t *model, byte *buffer)
{
	...
	// Setup submodels
	//
	if (!R_SetupQ2BSPSubModels	(model))
		return qFalse;
...
static qBool R_LoadQ3BSPShaderRefs (refModel_t *model, byte *byteBase, const dQ3BspLump_t *lump)
{
	...
	in = (void *)(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
		Com_Printf (PRNT_ERROR, "R_LoadQ3BSPShaderRefs: funny lump size in %s\n", model->name);	
		return qFalse;
	}
...
static qBool R_LoadQ3BSPFaces (refModel_t *model, byte *byteBase, const dQ3BspLump_t *lump)
{
	...
	in = (void *)(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
		Com_Printf (PRNT_ERROR, "Mod_LoadFaces: funny lump size in %s\n", model->name);
		return qFalse;
	}
...
static qBool R_LoadQ3BSPLeafs (refModel_t *model, byte *byteBase, const dQ3BspLump_t *lump, const dQ3BspLump_t *msLump)
{
	...
	for (i=0 ; i<model->bspModel.numLeafs ; i++, in++, out++) {
		...
		if (firstMarkSurface < 0 || numMarkSurfaces + firstMarkSurface > countMarkSurfaces) {
			Com_Printf (PRNT_ERROR, "MOD_LoadBmodel: bad marksurfaces in leaf %i\n", i);
			return qFalse;
		}
