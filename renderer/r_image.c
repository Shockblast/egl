...
void GL_BindTexture (image_t *image)
{
	...
	if (gl_nobind->integer || !image)	// performance evaluation option
		...
		texNum = image->texNum;
	if (glState.texNumsBound[glState.texUnit] == texNum)
		return;

	glState.texNumsBound[glState.texUnit] = texNum;
...
void GL_TextureBits (qBool verbose, qBool verboseOnly)
{
	...
	switch (gl_texturebits->integer) {
	case 32:
		glConfig.texRGBFormat = GL_RGB8;
		glConfig.texRGBAFormat = GL_RGBA8;
		break;

	case 16:
		glConfig.texRGBFormat = GL_RGB5;
		glConfig.texRGBAFormat = GL_RGBA4;
		break;

	default:
		glConfig.texRGBFormat = GL_RGB;
		glConfig.texRGBAFormat = GL_RGBA;
		break;
	}
}
...
void R_LoadPCX (char *filename, byte **pic, byte **palette, int *width, int *height)
{
	...
	out = Mem_PoolAlloc (x * x, MEMPOOL_IMAGESYS, 0);
	if (pic)
		*pic = out;
	pix = out;
...
static void R_ScreenShot_f (void)
{
	...
	if (glConfig.extBGRA && type == 0) {
		qglReadPixels (0, 0, vidDef.width, vidDef.height, GL_BGR_EXT, GL_UNSIGNED_BYTE, buffer);
		...
		if (glConfig.hwGamma) {
			...
			for (i=0 ; i<size ; i+=3) {
				buffer[i+2] = glConfig.gammaRamp[buffer[i+2]] >> 8;
				buffer[i+1] = glConfig.gammaRamp[buffer[i+1] + 256] >> 8;
				buffer[i+0] = glConfig.gammaRamp[buffer[i+0] + 512] >> 8;
			}
		}
	}
	else {
	...
	switch (type) {
	case 0:
		R_WriteTGA (f, buffer, vidDef.width, vidDef.height, !glConfig.extBGRA);
		break;
