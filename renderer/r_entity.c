...
void R_AddEntities (qBool vWeapOnly) {
	...
	for (i=0 ; i<r_NumEntities ; i++) {
		rbCurrEntity = &r_Entities[i];
		if ((rbCurrEntity->flags & RF_TRANSLUCENT) || (rbCurrEntity->color[3] < 1.0f))
			continue;
		...
		else if (rbCurrEntity->flags & RF_WEAPONMODEL)
			continue;

		R_RotateForEntity (rbCurrEntity);

		rbCurrModel = rbCurrEntity->model;
		if (!rbCurrModel)
			R_DrawNullModel (rbCurrEntity);
		else {
			switch (rbCurrModel->type) {
			case MODEL_MD2:
			case MODEL_MD3:
				R_AddAliasModel (rbCurrEntity);
				break;

			case MODEL_MD5:
				R_AddSkeletalModel (rbCurrEntity);
				break;

			case MODEL_BSP:
				R_AddBrushModel (rbCurrEntity);
				break;

			case MODEL_SP2:
				R_AddSpriteModel (rbCurrEntity);
				break;

			case MODEL_BAD:
			default:
				R_DrawNullModel (rbCurrEntity);
				Com_Printf (0, S_COLOR_RED "R_AddEntities[op]: '%s' Bad modeltype (%i)\n", rbCurrModel->name, rbCurrModel->type);
				break;
			}
		}
	...
	for (i=0 ; i<r_NumEntities ; i++) {
		...
		if (!rbCurrModel)
			...
			default:
				R_DrawNullModel (rbCurrEntity);
				Com_Printf (0, S_COLOR_RED "R_AddEntities[tr]: '%s' Bad modeltype (%i)\n", rbCurrModel->name, rbCurrModel->type);
				break;
			}
		}
		glSpeeds.numEntities++;
