...
================
*/
static void R_AddWorldSurface (mBspSurface_t *surf, entity_t *entity)
{
	meshBuffer_t	*mb;
	shader_t		*shader;
	int				i;
...
================
*/
static inline qBool R_CullSurface (mBspSurface_t *surf, float dist, int clipFlags)
{
	shader_t	*shader;
	int			i;
	...
	// Side culling
	if (shader->cullType != SHADER_CULL_NONE) {
		// Cull sides
		if (shader->cullType == SHADER_CULL_BACK) {
...
static void R_RecursiveWorldNode (mBspNode_t *node, int clipFlags)
{
	...
	for (i=node->numSurfaces, surf=r_worldModel->surfaces+node->firstSurface ; i ; i--, surf++) {
		// Cull
		if (R_CullSurface (surf, dist, clipFlags))
			continue;
		...
		// World surface
		R_AddWorldSurface (surf, &r_worldEntity);
	}
...
void R_AddBrushModelToList (entity_t *ent)
{
	...
	for (i=0 ; i<ent->model->numModelSurfaces ; i++, surf++) {
		...
		// Cull
		if (R_CullSurface (surf, dist, 0))
			continue;

		// World surface
		R_AddWorldSurface (surf, ent);
	}
}
