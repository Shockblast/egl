	...
	for (i=0 ; i<node->numSurfaces ; i++, surf++) {
		...
		if (lightmap) {
			...
			for (map=0 ; map<surf->numStyles ; map++) {
				...
				for (i=0 ; i<3 ; i++)
					r_PointColor[i] += lightmap[i] * scale[i] * (1.0 / 255);
...
void R_LightForEntity (entity_t *ent, int numVerts, qBool hasShell, qBool hasShadow, vec3_t shadowspot) {
	...
	if (R_RecursiveLightPoint (ent->origin, end, lightColor)) {
		...
		if (hasShadow) {
			hasShadow = qFalse;
			VectorIdentity (shadowspot);
		}

		VectorIdentity (ambientLight);
		VectorIdentity (directedLight);
	}
	...
	if (ent->flags & RF_WEAPONMODEL) {
		...
		if (lightColor[0] > lightColor[1]) {
			if (lightColor[0] > lightColor[2])
				r_lightlevel->value = 150 * lightColor[0];
			else
				r_lightlevel->value = 150 * lightColor[2];
		} else {
			if (lightColor[1] > lightColor[2])
				r_lightlevel->value = 150 * lightColor[1];
			else
				r_lightlevel->value = 150 * lightColor[2];
		}
	...
	if (r_fullbright->integer || (r_RefDef.rdFlags & RDF_NOWORLDMODEL) || (ent->flags & RF_FULLBRIGHT)) {
