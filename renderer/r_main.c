...
void R_RenderScene (refDef_t *rd)
{
	...
	if (!r_noRefresh->integer) {
		...
		for (i=0 ; i<MAX_MESH_KEYS ; i++)
			r_worldList.numMeshes[i] = 0;
		for (i=0 ; i<MAX_ADDITIVE_KEYS ; i++)
			r_worldList.numAdditiveMeshes[i] = 0;
...
static void GL_InitExtensions (void)
{
	...
	*/
	if (gl_ext_bgra->integer) {
		if (ExtensionFound (glConfig.extensionString, "GL_EXT_bgra")) {
	...
	*/
	if (gl_ext_texture3D->integer) {
		if (ExtensionFound (glConfig.extensionString, "GL_EXT_texture3D")) {
			...
			if (!qglUnlockArraysEXT) {
				Com_Printf (0, "..." S_COLOR_RED "GL_EXT_texture3D not properly supported!\n");
				qglTexImage3D		= NULL;
...
static void R_Register (void)
{
	...
	r_hwGamma			= Cvar_Get ("r_hwGamma",			"0",			CVAR_ARCHIVE|CVAR_LATCH_VIDEO);
