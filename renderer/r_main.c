...
#define R_AddScene(rd)		\
	...
	R_DrawWorld ();			\
	R_DrawDecals ();		\
	R_AddEntities (qFalse);	\
	R_RenderDLights ();		\
	R_DrawParticles ();		\
	R_DrawAlphaSurfaces ();	\
	R_AddEntities (qTrue);

#define R_BloomQuad(x,y,w,h,yScale)	\
	qglTexCoord2f (0, yScale);		\
	qglVertex2f (x, y);				\
	qglTexCoord2f (0, 0);			\
	qglVertex2f (x, y+h);			\
	qglTexCoord2f (1, 0);			\
	qglVertex2f (x+w, y+h);			\
	qglTexCoord2f (1, yScale);		\
	qglVertex2f (x+w, y);

void R_DrawBloom (refDef_t *rd) {
	uInt		bloomSize;
	refDef_t	oldRefDef, tempRefDef;
	int			oldWidth, oldHeight, i;
	float		bloomYScale;
	...
	bloomSize = r_bloom_size->integer;
	Q_NearestPow (&bloomSize, qTrue);
	// initialize if not already
	if (r_bloom_size->modified) {
		byte		*buffer;

		r_bloom_size->modified = qFalse;

		// allocate buffer
		buffer = Mem_Alloc (bloomSize * bloomSize * 3);

		// remove old texture
		qglDeleteTextures (1, &r_BloomTexture->texNum);

		// bloom texture
		GL_Bind (r_BloomTexture);
	...
	// save old values
	//
	memcpy (&oldRefDef, rd, sizeof (oldRefDef));
	memcpy (&tempRefDef, rd, sizeof (tempRefDef));
	oldWidth = vidDef.width;
	oldHeight = vidDef.height;

	//
	// render bloom scene
	//
	bloomYScale = (oldHeight / (float)oldWidth);
	tempRefDef.x = tempRefDef.y = 0;
	vidDef.width = tempRefDef.width = bloomSize;
	vidDef.height = tempRefDef.height = bloomSize * bloomYScale;

	R_AddScene (&tempRefDef);

	//
	// copy off screen image
	//
	GL_Bind (r_BloomTexture);

	qglCopyTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, tempRefDef.x, tempRefDef.y, bloomSize, bloomSize, 0);
	qglTexParameterf (GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	qglTexParameterf (GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	...
	// darken and blur
	//
	if (r_bloom_blur->integer || r_bloom_darken->integer) {
		GL_Setup2D ();
		qglEnable (GL_BLEND);

		// darken
		if (r_bloom_darken->integer) {
			qglColor4f (1.0, 1.0, 1.0, 1.0f);
			GL_BlendFunc (GL_DST_COLOR, GL_ZERO);

			qglDisable (GL_BLEND);

			qglBegin (GL_QUADS);
			R_BloomQuad (tempRefDef.x, tempRefDef.y, tempRefDef.width, tempRefDef.height, bloomYScale);
			qglEnd ();
		...
		// blur
		if (r_bloom_blur->integer > 0) {
			qglColor4f (1.0, 1.0, 1.0, r_bloom_blur_scale->value / (r_bloom_blur->value*2 + 1));
			...
			qglBegin (GL_QUADS);
			for (i=0 ; i<r_bloom_blur->integer ; i++) {
				R_BloomQuad (tempRefDef.x+(i*0.5f), tempRefDef.y, tempRefDef.width, tempRefDef.height, bloomYScale);
				R_BloomQuad (tempRefDef.x-(i*0.5f), tempRefDef.y, tempRefDef.width, tempRefDef.height, bloomYScale);
			}
			qglEnd ();
			...
			qglBegin (GL_QUADS);
			for (i=0 ; i<r_bloom_blur->integer ; i++) {
				R_BloomQuad (tempRefDef.x, tempRefDef.y+(i*0.5f), tempRefDef.width, tempRefDef.height, bloomYScale);
				R_BloomQuad (tempRefDef.x, tempRefDef.y-(i*0.5f), tempRefDef.width, tempRefDef.height, bloomYScale);
			}
			qglEnd ();
	...
	// clear screen
	//
	qglClearColor (0, 0, 0, 1.0);
	if (glConfig.useStencil && (gl_shadows->integer > 1)) {
		qglClearStencil (1);
		qglClear (GL_STENCIL_BUFFER_BIT|GL_DEPTH_BUFFER_BIT|GL_COLOR_BUFFER_BIT);
	}
	else {
	...
	// render normal scene
	//
	vidDef.width = oldWidth;
	vidDef.height = oldHeight;

	R_AddScene (&oldRefDef);
	...
	qglColor4f (1, 1, 1, clamp (r_bloom_alpha->value, 0.0f, 1.0f));
	qglEnable (GL_BLEND);
	qglDisable (GL_DEPTH_TEST);

	qglBegin (GL_QUADS);
	R_BloomQuad (oldRefDef.x, oldRefDef.y, oldRefDef.width, oldRefDef.height, bloomYScale);
	qglEnd ();
...
qBool R_ExtensionAvailable (const char *extensions, const char *extension) {
	...
	for ( ; ; ) {
		...
		if (!where)
			break;
		terminator = where + Q_strlen (extension);
...
void R_EndRegistration (void) {
	Mod_EndRegistration (); // unregister first so skins/shaders are touched
	RS_EndRegistration ();	// unregister first so shader images are touched
	Img_EndRegistration ();
}
...
void R_MediaInit (void) {
	...
	*/
	glMedia.modelShellTexture = Img_RegisterImage ("egl/shell_god.tga", IF_NOGAMMA|IF_NOINTENS);
	if (!glMedia.modelShellTexture)
		glMedia.modelShellTexture = r_WhiteTexture;

	/*
	** model_Shell shaders
	*/
	glMedia.modelShellGod		= RS_RegisterShader ("shell_god");
	glMedia.modelShellHalfDam	= RS_RegisterShader ("shell_halfdam");
	glMedia.modelShellDouble	= RS_RegisterShader ("shell_double");
	glMedia.modelShellRed		= RS_RegisterShader ("shell_red");
	glMedia.modelShellGreen		= RS_RegisterShader ("shell_green");
	glMedia.modelShellBlue		= RS_RegisterShader ("shell_blue");
	glMedia.modelShellIR		= RS_RegisterShader ("shell_ir");

	/*
	** world_Caustic shaders
	*/
	glMedia.worldLavaCaustics	= RS_RegisterShader ("egl/lavacaustics");
	glMedia.worldSlimeCaustics	= RS_RegisterShader ("egl/slimecaustics");
	glMedia.worldWaterCaustics	= RS_RegisterShader ("egl/watercaustics");
}
	...
	*/
	...
	if (glConfig.extSGISMultiTexture || glConfig.extArbMultitexture) {
		...
		if (glConfig.maxTexUnits < 1)
			...
			if (glConfig.maxTexUnits > TU_MAX)
				glConfig.maxTexUnits = TU_MAX;
		}
	}
	else
		glConfig.maxTexUnits = 1;
...
==================
*/
static inline void R_Register (void) {
	...
	r_bloom_blur		= Cvar_Get ("r_bloom_blur",			"8",			CVAR_ARCHIVE);
	r_bloom_blur_scale	= Cvar_Get ("r_bloom_blur_scale",	"1.5",			CVAR_ARCHIVE);
	r_bloom_darken		= Cvar_Get ("r_bloom_darken",		"4",			CVAR_ARCHIVE);
	r_bloom_size		= Cvar_Get ("r_bloom_size",			"256",			CVAR_ARCHIVE);
...
qBool R_Init (void *hInstance, void *hWnd) {
	...
	Com_Printf (0, "GL_VENDOR: %s\n", glConfig.vendString);
	vendorbuf = Mem_StrDup (glConfig.vendString);
	Q_strlwr (vendorbuf);

	glConfig.rendString = qglGetString (GL_RENDERER);
	Com_Printf (0, "GL_RENDERER: %s\n", glConfig.rendString);
	renderbuf = Mem_StrDup (glConfig.rendString);
	Q_strlwr (renderbuf);
