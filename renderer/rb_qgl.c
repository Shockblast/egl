...
# define LOGPROC	(sys_winInfo.oglLogFP)
# define GL_GPA(a)	(void *)GetProcAddress (sys_winInfo.oglLibrary, a)
# define SIG(x)		fprintf (sys_winInfo.oglLogFP, x "\n")
...
{
#ifdef WIN32
	if (sys_winInfo.oglLibrary) {
		FreeLibrary (sys_winInfo.oglLibrary);
		sys_winInfo.oglLibrary = NULL;
	}
#elif defined __unix__
...
#ifdef WIN32
	if (!(sys_winInfo.oglLibrary = LoadLibrary (dllName))) {
		char *buf = NULL;
		...
		if (!(sys_winInfo.oglLibrary = LoadLibrary (GL_DRIVERNAME))) {
			char *buf = NULL;
