	...
	if (surf->flags & SURF_DRAWTURB) {
		...
		for (bp=surf->polys ; bp ; bp=bp->next) {
			...
			for (i=0, v=p->verts[0] ; i<p->numverts ; i++, v+=VERTEXSIZE) {
				s = v[3] + r_WarpSinTable[Q_ftol (((v[4] * ONEDIV8 + r_RefDef.time) * TURBSCALE)) & 255];
				t = v[4] + r_WarpSinTable[Q_ftol (((v[3] * ONEDIV8 + r_RefDef.time) * TURBSCALE)) & 255];
...
void R_DrawBrushModel (entity_t *ent) {
	...
	if (!Matrix_Compare (ent->axis, axisIdentity)) {
		...
		if (R_CullSphere (ent->origin, ent->model->radius))
			return;
	} else {
		rotated = qFalse;
	...
	for (i=0 ; i<ent->model->numModelSurfaces ; i++, surf++) {
		...
		// draw the polygon
		if (((surf->flags & SURF_PLANEBACK) && (dot < -SMALL_EPSILON)) ||
			(!(surf->flags & SURF_PLANEBACK) && (dot > SMALL_EPSILON))) {
...
void LM_BeginBuildingLightmaps (void) {
	int		i;
	uInt	dummy[LIGHTMAP_WIDTH*LIGHTMAP_HEIGHT];
	...
	*/
	...
	for (i=0 ; i<MAX_LIGHTMAPS ; i++) {
		...
		lms.lmTextures[i].upFlags = lms.lmTextures[0].flags = 0;
