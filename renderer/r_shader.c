...
static shader_t	*sh_Shaders	= NULL;
...
static shader_t *RS_NewShader (char *name, char *fileName) {
	...
	if (!sh_Shaders) {
		sh_Shaders = (shader_t *) Mem_PoolAlloc (sizeof (shader_t), MEMPOOL_SHADERSYS, 0);
		shader = sh_Shaders;
	} else {
		shader = sh_Shaders;
		...
		shader->next = (shader_t *) Mem_PoolAlloc (sizeof (shader_t), MEMPOOL_SHADERSYS, 0);
		shader = shader->next;
	}
...
void RS_LoadShader (char *fileName, qBool recursed, qBool silent) {
	...
	while (shader != NULL) {
		if (!Q_stricmp (shader->fileName, fileName)) {
			if (shader->stage) {
				if (!silent)
					Com_Printf (0, "..." S_COLOR_YELLOW "shader '%s' already loaded\n", fileName);
				return;
			}
		}
...
static inline void RS_ToggleLightmap (qBool toggle) {
...
void RS_DrawSurface (mBspSurface_t *surf, shader_t *shader, qBool lightmap, int lmTexNum) {
	...
	do {
		if (lightmap && lmTexNum && stage->lightMap)
			RS_ToggleLightmap (qTrue);
		else if (!stage->lightMap && lightmap)
			RS_ToggleLightmap (qFalse);
		...
		if (shader->subDivide || (surf->flags & SURF_DRAWTURB)) {
			...
			for (bp=surf->polys ; bp ; bp=bp->next) {
				...
				for (i=0, v=p->verts[0] ; i<p->numverts ; i++, v+=VERTEXSIZE) {
					if (stage->envMap)
						RS_SetEnvmap (vec3Origin, axisIdentity, v, surf->plane->normal, coords);
					else {
						coords[0] = v[3];
						coords[1] = v[4];

						if (surf->flags & SURF_DRAWTURB) {
							warpCoords[0] = coords[0] + r_WarpSinTable[Q_ftol (((coords[1]*ONEDIV8 + r_RefDef.time) * TURBSCALE)) & 255];
							warpCoords[1] = coords[1] + r_WarpSinTable[Q_ftol (((coords[0]*ONEDIV8 + r_RefDef.time) * TURBSCALE)) & 255];

							if (surf->texInfo->flags & SURF_FLOWING)
								warpCoords[0] += -64 * ((r_RefDef.time * 0.5) - (int)(r_RefDef.time * 0.5));

							coords[0] = warpCoords[0] * ONEDIV64;
							coords[1] = warpCoords[1] * ONEDIV64;
						}
					}
...
static void RS_ShaderList_f (void) {
	...
	Com_Printf (0, "----------------------------------------\n");
	Com_Printf (0, "Total shaders/passes: %d/%d\n", totShaders, totPasses);
	Com_Printf (0, "----------------------------------------\n");
}
