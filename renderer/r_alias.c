	...
	if (frame == oldFrame) {
		...
		if (aliasModel->numMeshes > 1) {
			for (meshNum=0 ; meshNum<aliasModel->numMeshes ; meshNum++) {
				...
				VectorCopy (aliasMesh->mins[ent->frame], r_aliasMeshMins[meshNum]);
				VectorCopy (aliasMesh->maxs[ent->frame], r_aliasMeshMaxs[meshNum]);
	...
	if (ent->scale != 1.0f) {
		...
		if (aliasModel->numMeshes > 1) {
			for (meshNum=0 ; meshNum<aliasModel->numMeshes ; meshNum++) {
				...
				VectorScale (r_aliasMeshMins[meshNum], ent->scale, r_aliasMeshMins[meshNum]);
				VectorScale (r_aliasMeshMaxs[meshNum], ent->scale, r_aliasMeshMaxs[meshNum]);
...
=============
*/
static qBool R_CullAliasModel (entity_t *ent, vec3_t origin, vec3_t mins, vec3_t maxs, float radius)
{
	if (ent->flags & RF_WEAPONMODEL)
		return qFalse;
	if (ent->flags & RF_VIEWERMODEL)
		return !(r_refScene.mirrorView || r_refScene.portalView);
	...
	if (r_sphereCull->integer) {
		if (R_CullSphere (origin, radius, 15))
			return qTrue;
	}
	else {
		vec3_t	bbox[8];
		...
		for (p=0 ; p<8 ; p++) {
			...
			for (f=0 ; f<4 ; f++) {
				if (DotProduct (r_refScene.viewFrustum[f].normal, bbox[p]) < r_refScene.viewFrustum[f].dist)
					mask |= (1 << f);
			}
	...
	if (r_refScene.mirrorView || r_refScene.portalView) {
		if (PlaneDiff (origin, &r_refScene.clipPlane) < -radius) {
...
void R_AddAliasModelToList (entity_t *ent)
{
	...
	if (!gl_shadows->integer && R_CullAliasModel (ent, ent->origin, r_aliasMins, r_aliasMaxs, r_aliasRadius))
		return;
	...
	for (meshNum=0 ; meshNum<model->numMeshes ; meshNum++) {
		...
		VectorAverage (r_aliasMeshMins[meshNum], r_aliasMeshMaxs[meshNum], meshOrigin);
		VectorAdd (meshOrigin, ent->origin, meshOrigin);

		// Cull this mesh
		if (!gl_shadows->integer && model->numMeshes > 1) {
...
void R_DrawAliasModel (meshBuffer_t *mb, qBool shadowPass)
{
	...
	if (!shadowPass && ent->flags & RF_VIEWERMODEL && !r_refScene.mirrorView && !r_refScene.portalView)
		return;
	...
	if (!r_lerpmodels->integer || mb->shader->flags & SHADER_NOLERP)
		backLerp = 0;
	...
	if (ent->frame == ent->oldFrame) {
		...
		if (calcNormals) {
			...
			for (i=0 ; i<aliasMesh->numVerts ; i++, verts++, oldVerts++) {
				...
				VectorNormalizeFastf (rb_normalsArray[i]);
			}
		}
	}
