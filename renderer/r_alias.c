...
// r_alias.c
// - triangle model functions and rendering
...
=============
*/
qBool R_LerpAliasModelTag (orientation_t *orient, int frameNum, int oldFrameNum, float backLerp, char *name) {
	...
	//
	for (i=0 ; i<alias_Model->numTags ; i++) {
		if (!Q_stricmp (alias_Model->tags[i].tagName, name))
			break;
	}

	if (i == alias_Model->numTags) {
		Com_Printf (PRINT_DEV, "R_LerpAliasModelTag: no such tag '%s'\n", name);
		return qFalse;
	}

	tag = alias_Model->tags + (frameNum * alias_Model->numTags + i);
	oldTag = alias_Model->tags + (oldFrameNum * alias_Model->numTags + i);

	//
	// interpolate axis and origin
	//
	Quat_Lerp (oldTag->quat, tag->quat, backLerp, quat);
	Quat_Matrix (quat, orient->axis);

	orient->origin[0] = oldTag->origin[0] + (tag->origin[0] - oldTag->origin[0]) * backLerp;
	orient->origin[1] = oldTag->origin[1] + (tag->origin[1] - oldTag->origin[1]) * backLerp;
	orient->origin[2] = oldTag->origin[2] + (tag->origin[2] - oldTag->origin[2]) * backLerp;
...
=============
*/
void R_MD3Tag_Laser (entity_t *ent) {
...
=============
*/
void R_MD3Tag_Muzzle (orientation_t *orient) {
	vec3_t	origin, dir;
	int		i;

	VectorSubtract (r_CurrEntity->origin, orient->origin, dir);

	Matrix_TransformVector (orient->axis, dir, origin);
	for (i=0 ; i<3 ; i++)
		VectorMA (r_CurrEntity->origin, orient->origin[i], r_CurrEntity->axis[i], origin);
...
qBool R_CullAliasModel (void) {
	...
	if (!Matrix_Compare (r_CurrEntity->axis, axisIdentity)) {
		if (R_CullSphere (r_CurrEntity->origin, alias_modRadius, 15))
			return qTrue;
	} else {
		vec3_t	bbox[8];
...
void R_DrawAliasModel (entity_t *ent)
{
	...
	if ((ent->frame >= alias_Model->numFrames) || (ent->frame < 0)) {
		Com_Printf (PRINT_DEV, S_COLOR_YELLOW "R_DrawAliasModel: '%s' no such frame '%d'\n", ent->model->name, ent->frame);
		ent->frame = 0;
	}

	if ((ent->oldFrame >= alias_Model->numFrames) || (ent->oldFrame < 0)) {
		Com_Printf (PRINT_DEV, S_COLOR_YELLOW "R_DrawAliasModel: '%s' no such oldFrame '%d'\n", ent->model->name, ent->oldFrame);
		ent->oldFrame = 0;
	}
	...
	for (meshNum=0; meshNum<alias_Model->numMeshes; meshNum++) {
		...
		// render tags
		for (j=0 ; j<alias_Model->numTags ; j++) {
			alias_Tag = alias_Model->tags[j];

			continue; // kthx

			if (!alias_Tag.tagName[0])
				continue;

			if (!strcmp (alias_Tag.tagName, "tag_laser")) {
				R_MD3Tag_Laser (ent);
				continue;
			}

			if (ent->muzzleOn && !strcmp (alias_Tag.tagName, "tag_muzzle")) {
				if (R_LerpAliasModelTag (&tagOrientation, ent->frame, ent->oldFrame, backLerp, "tag_muzzle"))
					R_MD3Tag_Muzzle (&tagOrientation);
