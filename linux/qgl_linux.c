...
// When doing a port of EGL you must implement the following functions:
//
...
#include "glimp_linux.h"

#define GPA(a) (void *)dlsym (glxState.OpenGLLib, a)
...
static void APIENTRY logColor4f (GLfloat red, GLfloat green, GLfloat blue, GLfloat alpha) {
	fprintf (glxState.oglLogFP, "glColor4f (%f,%f,%f,%f)\n", red, green, blue, alpha);
	dllColor4f (red, green, blue, alpha);
}

static void APIENTRY logColor4fv (const GLfloat *v) {
	fprintf (glxState.oglLogFP, "glColor4fv (%f,%f,%f,%f)\n", v[0], v[1], v[2], v[3]);
	dllColor4fv (v);
}
...
static void APIENTRY logDisable (GLenum cap) {
	fprintf (glxState.oglLogFP, "glDisable (0x%x)\n", cap);
	dllDisable (cap);
}
...
static void APIENTRY logEnable (GLenum cap) {
	fprintf (glxState.oglLogFP, "glEnable (0x%x)\n", cap);
	dllEnable (cap);
}
...
static void APIENTRY logHint (GLenum target, GLenum mode) {
	fprintf (glxState.oglLogFP, "glHint (0x%x, 0x%x)\n", target, mode);
	dllHint (target, mode);
}
...
static void APIENTRY logPolygonMode (GLenum face, GLenum mode) {
	fprintf (glxState.oglLogFP, "glPolygonMode (0x%x, 0x%x)\n", face, mode);
...
static void APIENTRY logTexEnvf (GLenum target, GLenum pname, GLfloat param) {
	fprintf (glxState.oglLogFP, "glTexEnvf (0x%x, 0x%x, %f)\n", target, pname, param);
...
static void APIENTRY logTexEnvi (GLenum target, GLenum pname, GLint param) {
	fprintf (glxState.oglLogFP, "glTexEnvi (0x%x, 0x%x, 0x%x)\n", target, pname, param);
...
static void APIENTRY logTexParameterf (GLenum target, GLenum pname, GLfloat param) {
	fprintf (glxState.oglLogFP, "glTexParameterf (0x%x, 0x%x, %f)\n", target, pname, param);
...
static void APIENTRY logTexParameteri (GLenum target, GLenum pname, GLint param) {
	fprintf (glxState.oglLogFP, "glTexParameteri (0x%x, 0x%x, 0x%x)\n", target, pname, param);
...
qBool QGL_Init (const char *dllName)
{
	...
	if ((glxState.OpenGLLib = dlopen (dllName, RTLD_LAZY)) == 0) {
		cVar_t	*basedir;
		char	fn[MAX_OSPATH];
		char	*path;
	...
	qglXCreateContext			= GPA ("qglXCreateContext");
	qglXDestroyContext			= GPA ("qglXDestroyContext");
	qglXMakeCurrent				= GPA ("qglXMakeCurrent");
	qglXCopyContext				= GPA ("qglXCopyContext");
	qglXSwapBuffers				= GPA ("qglXSwapBuffers");
...
void QGL_ToggleLogging (void)
{
	if (gl_log->integer) {
		if (!glxState.oglLogFP) {
			...
			glxState.oglLogFP = fopen (buffer, "wt");

			fprintf (glxState.oglLogFP, "%s\n", asctime (newtime));
		}
...
void QGL_LogBeginFrame (void)
{
	...
	fprintf (glxState.oglLogFP, "*** R_BeginFrame ***\n");
}
...
void QGL_LogEndFrame (void)
{
	...
	fprintf (glxState.oglLogFP, "*** R_EndFrame ***\n");
}
