  ...
  if (ent->d_name[0] != '.' && ll+(dl=strlen(ent->d_name)+1) < sizeof(f->path)) {
    ...
    if (!stat(f->path, &st)) {
      if ((f->addmask & 1 && S_ISREG(st.st_mode) || f->addmask & 2 && S_ISDIR(st.st_mode)) && Q_WildcardMatch (f->pattern, f->path, 1))
        *(f->list++) = Mem_StrDup(f->path), f->max--;
...
int Sys_FindFiles (char *path, char *pattern, char **fileList, int maxFiles, int fileCount, qBool recurse, qBool addFiles, qBool addDirs)
{
  ...
  if (!(f.pathlen = strlen(path))) return 0;
  if (f.pathlen > sizeof(f.path)-2) return 0;
  strcpy(f.path, path);
  if (f.path[f.pathlen-1] != '/') f.path[f.pathlen++] = '/', f.path[f.pathlen] = 0;
  f.pattern = pattern;
...
void Sys_UnloadLibrary (libType_t libType)
{
	...
	default:
		Com_Error (ERR_FATAL, "Sys_UnloadLibrary: Bad libType\n");
		return;
	}
...
#define ARCH	"UNKNOW"
	...
	default:
		Com_Error (ERR_FATAL, "Sys_LoadLibrary: Bad libType\n");
		return NULL;
	}
...
static void signal_handler (int sig)
{
  ...
  signal(sig, SIG_DFL); /* avoid possible infinite loops if signal is triggered again here */
