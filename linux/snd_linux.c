...
int				audio_fd;
static	qBool	sndInitialized = false;
...
qBoolean SndImp_Init (void) {
    ...
    if (rc < 0) {
		perror (snddevice->string);
        Com_Printf (0, "Could not set %s to stereo=%d", snddevice->string, dma.channels);
		close (audio_fd);
        return 0;
    }
	if (tmp)
    ...
    if (rc < 0) {
		perror (snddevice->string);
        Com_Printf (0, "Could not set %s speed to %d", snddevice->string, dma.speed);
		close (audio_fd);
        return 0;
    }
    ...
    if (dma.samplebits == 16) {
		...
		perror (snddevice->string);
		Com_Printf (0, "%d-bit sound not supported.", dma.samplebits);
		close (audio_fd);
		return 0;
	}
