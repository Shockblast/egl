...
static void IN_ActivateMouse (void)
{
	...
	GetWindowRect (sys_winInfo.hWnd, &windowRect);
...
void IN_MouseEvent (int state)
{
	...
	for (i=0 ; i<mouseNumButtons ; i++) {
		if ((state & (1<<i)) && !(mouseOldButtonState & (1<<i)))
...
void IN_MouseMove (userCmd_t *cmd)
{
	...
	if (CL_StrafeState () || (lookstrafe->value && mLooking))
		cmd->sideMove += m_side->value * mouseXY[0];
	else
		cl.viewAngles[YAW] -= m_yaw->value * mouseXY[0];

	if ((mLooking || freelook->value) && !CL_StrafeState ())
...
static void	*cmd_plusMLook;
static void	*cmd_minusMLook;
...
void IN_Init (void)
{
	...
	cmd_plusMLook			= Cmd_AddCommand ("+mlook",				IN_MLookDown_f,			"");
	cmd_minusMLook			= Cmd_AddCommand ("-mlook",				IN_MLookUp_f,			"");
...
void IN_Shutdown (void)
{
	...
	Cmd_RemoveCommand ("+mlook", cmd_plusMLook);
	Cmd_RemoveCommand ("-mlook", cmd_minusMLook);
