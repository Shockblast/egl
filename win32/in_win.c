...
qBool		mouse_Initialized;
qBool		mouse_Active;		// qFalse when not focus app

int			mouse_NumButtons;
int			mouse_OldButtonState;
POINT		mouse_CurrentPos;

ivec2_t		mouse_XY;
ivec2_t		mouse_OldXY;

qBool		mouse_RestoreSPI;
qBool		mouse_ValidSPI;
ivec3_t		mouse_OrigParams;
ivec3_t		mouse_NoAccelParms = {0, 0, 0};
ivec3_t		mouse_AccelParms = {0, 0, 1};

ivec2_t		window_CenterXY;
RECT		window_Rect;
...
void IN_StartupMouse (void) {
	...
	Com_Printf (PRINT_ALL, "Mouse initialization\n");
	cv = Cvar_Get ("in_initmouse",	"1",	CVAR_NOSET);
	if (!cv->value) {
		Com_Printf (PRINT_ALL, "...skipped\n");
		return;
	}
	...
	if (fResult)
		Com_Printf (PRINT_DEV, "...mouse found\n");
	else
		Com_Printf (PRINT_ALL, "..." S_COLOR_RED "mouse not found\n");
}
...
void IN_MouseEvent (int mstate) {
	...
	for (i=0 ; i<mouse_NumButtons ; i++) {
		if ((mstate & (1<<i)) && !(mouse_OldButtonState & (1<<i)))
			Key_Event (K_MOUSE1 + i, qTrue, sys_MsgTime);

		if (!(mstate & (1<<i)) && (mouse_OldButtonState & (1<<i)))
			Key_Event (K_MOUSE1 + i, qFalse, sys_MsgTime);
	}
...
void IN_MouseMove (userCmd_t *cmd) {
	...
	if (Key_KeyDest (KD_MENU)) {
		CL_UIModule_MoveMouse (mx * ui_sensitivity->value, my * ui_sensitivity->value);
		return;
	}
...
void IN_StartupJoystick (void) {
	...
	Com_Printf (PRINT_ALL, "Joystick initialization\n");
	...
	if (mmr != JOYERR_NOERROR) {
		Com_Printf (PRINT_ALL, "...not found -- no valid joysticks (%x)\n", mmr);
		return;
	}
	...
	*/
	...
	if ((mmr = joyGetDevCaps (joy_ID, &jc, sizeof (jc))) != JOYERR_NOERROR) {
		Com_Printf (PRINT_ALL, "..." S_COLOR_YELLOW "not found -- invalid joystick capabilities (%x)\n", mmr); 
		return;
	}
