...
static void CDAudio_Eject (void) {
	...
	if (dwReturn)
		Com_Printf (PRNT_DEV, S_COLOR_YELLOW "MCI_SET_DOOR_OPEN failed (%i)\n", dwReturn);
}
...
static void CDAudio_CloseDoor (void) {
	...
	if (dwReturn)
		Com_Printf (PRNT_DEV, S_COLOR_YELLOW "MCI_SET_DOOR_CLOSED failed (%i)\n", dwReturn);
}
...
static int CDAudio_GetAudioDiskInfo (void) {
	...
	if (dwReturn) {
		Com_Printf (PRNT_DEV, S_COLOR_YELLOW "CDAudio: drive ready test - get status failed\n");
		return -1;
	}

	if (!mciStatusParms.dwReturn) {
		Com_Printf (PRNT_DEV, S_COLOR_YELLOW "CDAudio: drive not ready\n");
		return -1;
	}
	...
	dwReturn = mciSendCommand (wDeviceID, MCI_STATUS, MCI_STATUS_ITEM | MCI_WAIT, (DWORD) (LPVOID) &mciStatusParms);
	if (dwReturn) {
		Com_Printf (PRNT_DEV, S_COLOR_YELLOW "CDAudio: get tracks - status failed\n");
		return -1;
	}

	if (mciStatusParms.dwReturn < 1) {
		Com_Printf (PRNT_DEV, S_COLOR_YELLOW "CDAudio: no music tracks\n");
		return -1;
	}
...
void CDAudio_Pause (void) {
	...
	mciGenericParms.dwCallback = (DWORD) globalhWnd;
	dwReturn = mciSendCommand (wDeviceID, MCI_PAUSE, 0, (DWORD)(LPVOID) &mciGenericParms);
	if (dwReturn)
		Com_Printf (PRNT_DEV, S_COLOR_YELLOW "MCI_PAUSE failed (%i)", dwReturn);
...
void CDAudio_Play2 (int track, qBool looping) {
	...
	if (dwReturn) {
		Com_Printf (PRNT_DEV, S_COLOR_YELLOW "MCI_STATUS failed (%i)\n", dwReturn);
		return;
	}

	if (mciStatusParms.dwReturn != MCI_CDA_TRACK_AUDIO) {
		Com_Printf (0, S_COLOR_YELLOW "CDAudio: track %i is not audio\n", track);
		return;
	}
	...
	mciPlayParms.dwCallback = (DWORD) globalhWnd;
	dwReturn = mciSendCommand (wDeviceID, MCI_PLAY, MCI_NOTIFY | MCI_FROM | MCI_TO, (DWORD)(LPVOID) &mciPlayParms);
	if (dwReturn) {
		Com_Printf (PRNT_DEV, S_COLOR_YELLOW "CDAudio: MCI_PLAY failed (%i)\n", dwReturn);
		return;
	}
...
void CDAudio_Stop (void) {
	...
	if (dwReturn)
		Com_Printf (PRNT_DEV, S_COLOR_YELLOW "MCI_STOP failed (%i)", dwReturn);
...
static void CD_f (void) {
	...
	if (!cd_IsValid) {
		CDAudio_GetAudioDiskInfo ();
		if (!cd_IsValid) {
			Com_Printf (0, S_COLOR_YELLOW "No CD in player.\n");
			return;
		}
	}
...
LRESULT CALLBACK CDAudio_MessageHandler (HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case MCI_NOTIFY_FAILURE:
		Com_Printf (PRNT_DEV, S_COLOR_YELLOW "MCI_NOTIFY_FAILURE\n");
		CDAudio_Stop ();
		cd_IsValid = qFalse;
		break;

	default:
		Com_Printf (PRNT_DEV, S_COLOR_YELLOW "Unexpected MM_MCINOTIFY type (%i)\n", wParam);
		return 1;
	}
...
qBool CDAudio_Init (void) {
	...
	if (dwReturn) {
		Com_Printf (0, S_COLOR_YELLOW "CDAudio_Init: MCI_OPEN failed (%i)\n", dwReturn);
		return qFalse;
	}
	...
	if (dwReturn) {
		Com_Printf (0, S_COLOR_YELLOW "MCI_SET_TIME_FORMAT failed (%i)\n", dwReturn);
...
===========
*/
void CDAudio_Shutdown (qBool full) {
	if (!cd_Initialized)
		return;

	CDAudio_Stop ();
	if (mciSendCommand (wDeviceID, MCI_CLOSE, MCI_WAIT, (DWORD)NULL))
		Com_Printf (PRNT_DEV, S_COLOR_YELLOW "CDAudio_Shutdown: MCI_CLOSE failed\n");
