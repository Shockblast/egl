...
// qal_win.c
//
// This file implements the operating system binding of AL to QAL function pointers.
// When doing a port of EGL you must implement the following two functions:
//
// QAL_Init () - loads libraries, assigns function pointers, etc.
// QAL_Shutdown () - unloads libraries, NULLs function pointers
//
...
This is responsible for binding our qal function pointers to the appropriate AL stuff.
...
===========
*/
qBool QAL_Init (const char *dllName) {
	Com_Printf (0, "QAL_Init: LoadLibrary ( \"%s\" )\n", dllName);
	if ((alwState.hInstOpenAL = LoadLibrary (dllName)) == 0) {
		char *buf = NULL;
