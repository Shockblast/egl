...
static void APIENTRY logDisable (GLenum cap) {
	fprintf (glw.logFP, "glDisable (0x%x)\n", cap);
	dllDisable (cap);
}
...
static void APIENTRY logEnable (GLenum cap) {
	fprintf (glw.logFP, "glEnable (0x%x)\n", cap);
	dllEnable (cap);
}
...
static void APIENTRY logHint (GLenum target, GLenum mode) {
	fprintf (glw.logFP, "glHint (0x%x, 0x%x)\n", target, mode);
	dllHint (target, mode);
}
...
static void APIENTRY logPolygonMode (GLenum face, GLenum mode) {
	fprintf (glw.logFP, "glPolygonMode (0x%x, 0x%x)\n", face, mode);
...
static void APIENTRY logTexEnvf (GLenum target, GLenum pname, GLfloat param) {
	fprintf (glw.logFP, "glTexEnvf (0x%x, 0x%x, %f)\n", target, pname, param);
...
static void APIENTRY logTexEnvi (GLenum target, GLenum pname, GLint param) {
	fprintf (glw.logFP, "glTexEnvi (0x%x, 0x%x, 0x%x)\n", target, pname, param);
...
static void APIENTRY logTexParameterf (GLenum target, GLenum pname, GLfloat param) {
	fprintf (glw.logFP, "glTexParameterf (0x%x, 0x%x, %f)\n", target, pname, param);
...
static void APIENTRY logTexParameteri (GLenum target, GLenum pname, GLint param) {
	fprintf (glw.logFP, "glTexParameteri (0x%x, 0x%x, 0x%x)\n", target, pname, param);
...
===========
*/
#define GPA(a) GetProcAddress (glw.hInstOpenGL, a)
qBool QGL_Init (const char *dllname) {
	if ((glw.hInstOpenGL = LoadLibrary (dllname)) == 0) {
		char *buf = NULL;
...
void QGL_ToggleLogging (qBool enable) {
	...
	if (enable) {
		if (!glw.logFP) {
			...
			Com_sprintf (buffer, sizeof (buffer), "%s/glConfig.log", FS_Gamedir ()); 
			glw.logFP = fopen (buffer, "wt");
...
void QGL_LogBeginFrame (void) {
	fprintf (glw.logFP, "*** R_BeginFrame ***\n");
}
