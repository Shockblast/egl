...
static void VID_AppActivate (qBool fActive, qBool minimize)
{
	...
	SndImp_Activate (sys_activeApp);

	if (win_noalttab->value)
		WIN_ToggleAltTab (!sys_activeApp);

	GLimp_AppActivate (sys_activeApp); // FIXME was fActive, should it be this instead?
}
...
LRESULT CALLBACK MainWndProc (HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	// fall through
	case WM_KEYDOWN:
		Key_Event (In_MapKey (wParam, lParam), qTrue, sys_msgTime);
		break;

	case WM_SYSKEYUP:
	case WM_KEYUP:
		Key_Event (In_MapKey (wParam, lParam), qFalse, sys_msgTime);
		break;
