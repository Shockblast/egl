...
void GLimp_InitExtensions (HDC hDC) {
	...
	Com_Printf (0, "----------------------------------------\n");
	Com_Printf (0, "GL implementation extensions:\n");
...
qBool GLimp_InitWGLPFD (PIXELFORMATDESCRIPTOR *iPFD, int *iPixelFormat,
	...
	if (!RegisterClass (&wc_temp)) {
		...
		Com_Printf (0, "...WGL PFD temp class regsitered\n");
	}
	...
	*iPixelFormat = ChoosePixelFormat (hDC_temp, &tempPFD);
	...
	// check for software acceleration
	if (!VerifyDriver ()) {
		Com_Printf (0, "..." S_COLOR_RED "no hardware acceleration detected\n");
		goto fail;
	}
	...
	if (qwglGetPixelFormatAttribivARB (hDC_temp, 0, 0, 1, iAttribs, iResults) == GL_FALSE) {
		...
		Com_Printf (0, "...qwglGetPixelFormatAttribivARB succeeded\n");
	}
	...
	if ((validPFD == qFalse) || !numPFDS) {
		...
		Com_Printf (0, "...qwglChoosePixelFormatARB succeeded\n");
	...
	// set the pixel format
	if (SetPixelFormat (glwState.hDC, *iPixelFormat, &tempPFD) == qFalse) {
		Com_Printf (0, "..." S_COLOR_RED "SetPixelFormat failed\n");
		goto fail;
	}
	else {
...
qBool GLimp_InitGL (void) {
	...
	if (gl_nowglpfd->integer
	|| !glimp_extensions->integer
	|| !GLimp_InitWGLPFD (&iPFD, &iPixelFormat, alphaBits, colorBits, depthBits, stencilBits)) {
		Com_Printf (0, "...using standard pfd initialization\n");
		if (!GLimp_InitGLPFD (&iPFD, &iPixelFormat)) {
			Com_Printf (0, S_COLOR_RED "...standard pfd initialization failed\n");
			return qFalse;
		}
	}
	...
	Com_Printf (0, "----------------------------------------\n");
	Com_Printf (0, "%s_PFD #%i: c(%d-bits) a(%d-bits) z(%d-bit) s(%d-bit)\n",
				(glConfig.wglPFD) ? "WGL" : "GL", iPixelFormat,
