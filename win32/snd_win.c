...
static qBool	sndDSndInitialized;
static qBool	sndWavInitialized;

static qBool	sndFirstTime = qTrue;
static qBool	sndIsDSound;
static qBool	sndIsWave;
static qBool	snd_primaryFormatSet;
...
static qBool DS_CreateBuffers (void)
{
	...
	Com_Printf ((sndFirstTime) ? 0 : PRNT_DEV, "Creating DS buffers\n");
	...
	if (DS_OK == pDS->lpVtbl->CreateSoundBuffer (pDS, &dsbuf, &pDSPBuf, NULL)) {
		...
		if (DS_OK != pDSPBuf->lpVtbl->SetFormat (pDSPBuf, &pformat))
			...
			snd_primaryFormatSet = qTrue;
		}
	}
	else
		Com_Printf (0, S_COLOR_RED "FAILED!\n");

	if (!snd_primaryFormatSet || !s_primary->value) {
		...
		Com_Printf ((sndFirstTime) ? 0 : PRNT_DEV, "...using secondary sound buffer\n");
	}
	else {
		Com_Printf ((sndFirstTime) ? 0 : PRNT_DEV, "...using primary buffer\n");
	...
	Com_Printf ((sndFirstTime) ? 0 : PRNT_DEV,
								"   %d channel(s)\n"
								"   %d bits/sample\n"
								"   %d bytes/sec\n",
	...
	snd_audioDMA.samples = gSndBufSize / (snd_audioDMA.sampleBits / 8);
	snd_audioDMA.samplePos = 0;
	snd_audioDMA.submissionChunk = 1;
	snd_audioDMA.buffer = (byte *) lpData;
	sample16 = (snd_audioDMA.sampleBits/8) - 1;
...
static sndinitstat SndImp_InitDirect (void)
{
	...
	case 11:	snd_audioDMA.speed = 11025;	break;
	case 8:		snd_audioDMA.speed = 8000;	break;
	default:	snd_audioDMA.speed = 11025;
	}
...
static qBool SndImp_InitWav (void)
{
	...
	*/
	...
	snd_audioDMA.samples = gSndBufSize / (snd_audioDMA.sampleBits / 8);
	snd_audioDMA.samplePos = 0;
	snd_audioDMA.submissionChunk = 512;
	snd_audioDMA.buffer = (byte *) lpData;
	sample16 = (snd_audioDMA.sampleBits/8) - 1;

	sndWavInitialized = qTrue;
...
int SndImp_Init (void)
{
	...
	sndDSndInitialized = qFalse;
	sndWavInitialized = qFalse;

	stat = SIS_FAILURE;	// assume DirectSound won't initialize

	// Init DirectSound
	if (!s_wavonly->integer) {
		if (sndFirstTime || sndIsDSound) {
	...
	** to have sound)
	*/
	if (!sndDSndInitialized && (stat != SIS_NOTAVAIL)) {
		if (sndFirstTime || sndIsWave) {
			sndIsWave = SndImp_InitWav ();
	...
	if (!sndDSndInitialized && !sndWavInitialized) {
		if (sndFirstTime)
...
int SndImp_GetDMAPos (void)
{
	...
	if (sndDSndInitialized) {
		mmtime.wType = TIME_SAMPLES;
		pDSBuf->lpVtbl->GetCurrentPosition (pDSBuf, &mmtime.u.sample, &dwWrite);
		s = mmtime.u.sample - mmStartTime.u.sample;
	}
	else if (sndWavInitialized) {
		s = snd_sent * WAV_BUFFER_SIZE;
	}

	s >>= sample16;
	s &= (snd_audioDMA.samples-1);
...
void SndImp_Submit (void)
{
	...
	if (pDSBuf)
		pDSBuf->lpVtbl->Unlock (pDSBuf, snd_audioDMA.buffer, locksize, NULL, 0);

	if (!sndWavInitialized)
		return;
