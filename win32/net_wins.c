...
static int NET_IPSocket (char *netInterface, int port)
{
	...
	if ((newSocket = socket (PF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1) {
		...
		if (error != WSAEAFNOSUPPORT)
			Com_Printf (PRNT_WARNING, "WARNING: NET_IPSocket: socket: %s", NET_ErrorString (error));
	...
	if (ioctlsocket (newSocket, FIONBIO, &hasArgs) == -1) {
		Com_Printf (PRNT_WARNING, "WARNING: NET_IPSocket: ioctl FIONBIO: %s\n", NET_ErrorString (WSAGetLastError ()));
		return 0;
	}

	// Make it broadcast capable
	if (setsockopt (newSocket, SOL_SOCKET, SO_BROADCAST, (char *)&i, sizeof (i)) == -1) {
		Com_Printf (PRNT_WARNING, "WARNING: NET_IPSocket: setsockopt SO_BROADCAST: %s\n", NET_ErrorString (WSAGetLastError ()));
		return 0;
	}
	...
	if (bind (newSocket, (void *)&address, sizeof (address)) == -1) {
		Com_Printf (PRNT_WARNING, "WARNING: NET_IPSocket: bind: %s\n", NET_ErrorString (WSAGetLastError ()));
...
int NET_Config (int openFlags)
{
	...
	if (openFlags == NET_NONE) {
		...
		if (openFlags & NET_SERVER) {
			if (!ipSockets[NS_SERVER]) {
				port = Cvar_Register ("ip_hostport", "0", CVAR_READONLY)->integer;
				if (!port) {
					port = Cvar_Register ("hostport", "0", CVAR_READONLY)->integer;
					if (!port)
						port = Cvar_Register ("port", Q_VarArgs ("%i", PORT_SERVER), CVAR_READONLY)->integer;
				}
		...
		if (!dedicated->integer && openFlags & NET_CLIENT) {
			if (!ipSockets[NS_CLIENT]) {
				...
				port = Cvar_Register ("ip_clientport", Q_VarArgs ("%i", newport), CVAR_READONLY)->integer;
				if (!port) {
					port = Cvar_Register ("clientport", Q_VarArgs ("%i", newport), CVAR_READONLY)->integer;
					if (!port) {
						port = PORT_ANY;
