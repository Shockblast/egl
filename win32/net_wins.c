...
qBool NET_GetPacket (int sock, netAdr_t *fromAddr, netMsg_t *message) {
	...
	for (protocol=0 ; protocol<2 ; protocol++) {
		...
		if (ret == -1) {
			...
			if (err == WSAEMSGSIZE) {
				Com_Printf (PRINT_ALL, S_COLOR_YELLOW "WARNING: NET_GetPacket: Oversize packet from %s\n", NET_AdrToString (*fromAddr));
				continue;
			}

			if (dedicated->integer || (err == WSAECONNRESET))
				Com_Printf (PRINT_ALL, S_COLOR_YELLOW "NET_GetPacket: %s from %s\n", NET_ErrorString (), NET_AdrToString (*fromAddr));
			else
				Com_Printf (PRINT_ALL, S_COLOR_RED "NET_GetPacket: %s from %s", NET_ErrorString (), NET_AdrToString (*fromAddr));

			continue;
		}

		if (ret == message->maxSize) {
			Com_Printf (PRINT_ALL, S_COLOR_YELLOW "NET_GetPacket: Oversize packet from %s\n", NET_AdrToString (*fromAddr));
			continue;
		}
...
void NET_SendPacket (int sock, int length, void *data, netAdr_t to) {
	...
	if (ret == -1) {
		...
		Com_Printf (PRINT_ALL, S_COLOR_RED "NET_SendPacket: %s to %s\n", NET_ErrorString (), NET_AdrToString (to));
	}
}
...
int NET_IPSocket (char *net_interface, int port) {
	...
	if ((newsocket = socket (PF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1) {
		...
		if (err != WSAEAFNOSUPPORT)
			Com_Printf (PRINT_ALL, S_COLOR_YELLOW "WARNING: NET_IPSocket: socket: %s", NET_ErrorString ());
		return 0;
	}

	/* make it non-blocking */
	if (ioctlsocket (newsocket, FIONBIO, &_qTrue) == -1) {
		Com_Printf (PRINT_ALL, S_COLOR_YELLOW "WARNING: NET_IPSocket: ioctl FIONBIO: %s\n", NET_ErrorString ());
		return 0;
	}

	/* make it broadcast capable */
	if (setsockopt(newsocket, SOL_SOCKET, SO_BROADCAST, (char *)&i, sizeof (i)) == -1) {
		Com_Printf (PRINT_ALL, S_COLOR_YELLOW "WARNING: NET_IPSocket: setsockopt SO_BROADCAST: %s\n", NET_ErrorString ());
		return 0;
	}
	...
	if (bind (newsocket, (void *)&address, sizeof (address)) == -1) {
		Com_Printf (PRINT_ALL, S_COLOR_YELLOW "WARNING: NET_IPSocket: bind: %s\n", NET_ErrorString ());
...
int NET_IPXSocket (int port) {
	...
	if ((newsocket = socket (PF_IPX, SOCK_DGRAM, NSPROTO_IPX)) == -1) {
		err = WSAGetLastError();
		if (err != WSAEAFNOSUPPORT)
			Com_Printf (PRINT_ALL, S_COLOR_YELLOW "WARNING: NET_IPXSocket: socket: %s\n", NET_ErrorString ());
	...
	/* make it non-blocking */
	if (ioctlsocket (newsocket, FIONBIO, &_qTrue) == -1) {
		Com_Printf (PRINT_ALL, S_COLOR_YELLOW "WARNING: NET_IPXSocket: ioctl FIONBIO: %s\n", NET_ErrorString ());
		return 0;
	}

	/* make it broadcast capable */
	if (setsockopt(newsocket, SOL_SOCKET, SO_BROADCAST, (char *)&_qTrue, sizeof (_qTrue)) == -1) {
		Com_Printf (PRINT_ALL, S_COLOR_YELLOW "WARNING: NET_IPXSocket: setsockopt SO_BROADCAST: %s\n", NET_ErrorString ());
		return 0;
	}
	...
	if (bind (newsocket, (void *)&address, sizeof (address)) == -1) {
		Com_Printf (PRINT_ALL, S_COLOR_YELLOW "WARNING: NET_IPXSocket: bind: %s\n", NET_ErrorString ());
		closesocket (newsocket);
		return 0;
	}
...
void NET_ShowIP_f (void) {
	...
	if (!(h = gethostbyname (s))) {
		Com_Printf (PRINT_ALL, S_COLOR_RED "Can't get host\n");
		return;
	}

	Com_Printf (PRINT_ALL, "Hostname: %s\n", h->h_name);

	i = 0;
	while (h->h_addr_list[i]) {
		in.s_addr = *(int *)h->h_addr_list[i];
		Com_Printf(PRINT_ALL, "IP Address: %s\n", inet_ntoa(in));
		i++;
	}
}
