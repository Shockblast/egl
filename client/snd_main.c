...
sfxCache_t *Snd_LoadSound (sfx_t *s)
{
	...
	if (!data) {
		Com_DevPrintf (0, "Snd_LoadSound: Couldn't load %s\n", namebuffer);
		return NULL;
	}
...
static sfx_t *Snd_AliasName (char *aliasName, char *trueName)
{
	...
	sfx->trueName = Mem_StrDup (trueName);
	sfx->hashValue = Com_HashFileName (sfx->name, MAX_SFX_HASH);
...
void Snd_Shutdown (void)
{
	...
	// Free all memory
	size = Mem_PoolSize (MEMPOOL_SOUNDSYS);
	Com_Printf (0, "...releasing %i bytes\n", size);
...
void Snd_StartSound (vec3_t origin, int entNum, entChannel_t entChannel, struct sfx_s *sfx, float volume, float attenuation, float timeOffset)
{
	...
	if (entNum < 0 || entNum >= MAX_CS_EDICTS)
		Com_Error (ERR_DROP, "Snd_StartSound: invalid entNum (%i)", entNum);
	if (entChannel < 0)
		Com_Error (ERR_DROP, "Snd_StartSound: invalid entChannel (%i)", entChannel);
	if (attenuation < 0 || attenuation > ATTN_STATIC)
		Com_Error (ERR_DROP, "Snd_StartSound: invalid attenuation (%i)", attenuation);
