...
void Huff1TableInit (void) {
	...
	for (prev=0 ; prev<256 ; prev++) {
		...
		FS_Read (counts, sizeof (counts), cl.cin.file);
		for (j=0 ; j<256 ; j++)
...
byte *CIN_ReadNextFrame (void) {
	...
	// read the next frame
	r = fread (&command, 4, 1, cl.cin.file);
	...
	if (command == 1) {
		// read palette
		FS_Read (cl.cin.palette, sizeof (cl.cin.palette), cl.cin.file);
	}

	// decompress the next frame
	FS_Read (&size, 4, cl.cin.file);
	size = LittleLong(size);
	if (size > sizeof (compressed) || size < 1)
		Com_Error (ERR_DROP, "Bad compressed frame size");
	FS_Read (compressed, size, cl.cin.file);
	...
	FS_Read (samples, count*cin.sndWidth*cin.sndChannels, cl.cin.file);
...
void CIN_PlayCinematic (char *arg) {
	...
	FS_Read (&width, 4, cl.cin.file);
	FS_Read (&height, 4, cl.cin.file);
	cin.width = LittleLong (width);
	cin.height = LittleLong (height);

	FS_Read (&cin.sndRate, 4, cl.cin.file);
	cin.sndRate = LittleLong (cin.sndRate);
	FS_Read (&cin.sndWidth, 4, cl.cin.file);
	cin.sndWidth = LittleLong (cin.sndWidth);
	FS_Read (&cin.sndChannels, 4, cl.cin.file);
