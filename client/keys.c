...
static void Key_CompleteCommand (void) {
	...
	if ((aliasCnt + cmdCnt + cvarCnt) == 1) {
		...
		Q_strncpyz (keyConsoleBuffer[keyConsoleEditLine]+1, completed, sizeof (keyConsoleBuffer[keyConsoleEditLine])-1);
		keyConsoleLinePos = Q_strlen (completed)+1;
...
static void Key_Message (int key) {
	if ((toupper (key) == 'V' && keysDown[K_CTRL]) ||
		...
		if ((cbd = Sys_GetClipboardData ()) != 0) {
			...
			if (i > 0) {
				cbd[i] = 0;
				Q_strcat (keyChatBuffer[keyChatEditLine], cbd, sizeof (keyChatBuffer[keyChatEditLine]));
				keyChatLinePos += i;
			}
			Mem_Free (cbd);
		}
	...
	if (key == K_DEL) {
		if (keyChatLinePos < Q_strlen (keyChatBuffer[keyChatEditLine]))
	...
	if (key == K_RIGHTARROW) {
		if (Q_strlen (keyChatBuffer[keyChatEditLine]) == keyChatLinePos) {
			if (Q_strlen (keyChatBuffer[(keyChatEditLine + 31) & 31]) <= keyChatLinePos)
				return;
	...
	if ((key == K_UPARROW) || (key == K_KP_UPARROW) || ((key == 'p') && keysDown[K_CTRL])) {
		...
		keyChatLinePos = Q_strlen (keyChatBuffer[keyChatEditLine]);
		return;
	}
	...
	if (keyChatLinePos < MAXCMDLINE-1) {
		...
		if (keyInsertOn) {
			...
			i = Q_strlen (keyChatBuffer[keyChatEditLine]) - 1;
		...
		keyChatLinePos++;
		if (!i) {
			for (i=keyChatLinePos ; i<MAXCMDLINE ; i++)
				keyChatBuffer[keyChatEditLine][i] = '\0';
		}
	}
}
...
static void Key_Console (int key) {
	...
	if ((toupper (key) == 'V' && keysDown[K_CTRL]) ||
		...
		if ((cbd = Sys_GetClipboardData ()) != 0) {
			...
			if (i > 0) {
				cbd[i] = 0;
				Q_strcat (keyConsoleBuffer[keyConsoleEditLine], cbd, sizeof (keyConsoleBuffer[keyConsoleEditLine]));
				keyConsoleLinePos += i;
			}
			Mem_Free (cbd);
		}
	...
	if (key == K_DEL) {
		if (keyConsoleLinePos < Q_strlen (keyConsoleBuffer[keyConsoleEditLine]))
	...
	if (key == K_RIGHTARROW) {
		if (Q_strlen (keyConsoleBuffer[keyConsoleEditLine]) == keyConsoleLinePos) {
			if (Q_strlen (keyConsoleBuffer[(keyConsoleEditLine + 31) & 31]) <= keyConsoleLinePos)
				return;
	...
	if ((key == K_UPARROW) || (key == K_KP_UPARROW) || ((key == 'p') && keysDown[K_CTRL])) {
		...
		memset (&keyConsoleBuffer[keyConsoleEditLine], '\0', sizeof (keyConsoleBuffer[keyConsoleEditLine]));
		strcpy (keyConsoleBuffer[keyConsoleEditLine], keyConsoleBuffer[keyConsoleHistoryLine]);
		keyConsoleLinePos = Q_strlen (keyConsoleBuffer[keyConsoleEditLine]);
		return;
	}
	...
	if (keyConsoleLinePos < MAXCMDLINE-1) {
		...
		if (keyInsertOn) {
			// can't do strcpy to move string to right
			i = Q_strlen (keyConsoleBuffer[keyConsoleEditLine]) - 1;
		...
		keyConsoleLinePos++;
		if (!i) {
			for (i=keyConsoleLinePos ; i<MAXCMDLINE ; i++)
				keyConsoleBuffer[keyConsoleEditLine][i] = '\0';
		}
	}
}
...
void Key_Event (int keyNum, qBool isDown, uInt time) {
	...
	if (isDown) {
		...
		if ((keyNum >= 200) && (keyNum < K_MWHEELDOWN) && !keyBindings[keyNum])
	...
	if ((keyNum == K_SHIFT) || (keyNum == K_LSHIFT) || (keyNum == K_RSHIFT))
		keyShiftDown = isDown;
	keyCapslockOn = In_GetKeyState (K_CAPSLOCK);
	...
	if (keyNum == K_ESCAPE) {
		...
		if (cls.connectState != CA_ACTIVE) {
			if (Key_KeyDest (KD_GAME) || Key_KeyDest (KD_MENU)) {
				if (cls.connectState != CA_DISCONNECTED)
					Cbuf_AddText ("disconnect\n");
				else if (Key_KeyDest (KD_MENU))
	...
	*/
	...
	if (
		...
		(Key_KeyDest (KD_GAME) && ((cls.connectState == CA_ACTIVE) || (!keyShiftDown && !keyConsoleKeys[keyNum])))
		) {
		kb = Key_FallBackKey (keyNum);
		if (kb) {
			if (kb[0] == '+') {
