...
static int CL_ParseEntityBits (uInt *bits)
{
	...
	if (total & U_NUMBER16)
		number = MSG_ReadShort (&netMessage) % MAX_ENTITIES;
	else
...
===================
*/
static void CL_ParsePlayerstate (int extraFlags)
{
	...
	if (extraFlags & EPS_GUNOFFSET) {
		state->gunOffset[0] = MSG_ReadChar (&netMessage)*0.25f;
		state->gunOffset[1] = MSG_ReadChar (&netMessage)*0.25f;
		state->gunOffset[2] = MSG_ReadChar (&netMessage)*0.25f;
	}
	if (extraFlags & EPS_GUNANGLES) {
		state->gunAngles[0] = MSG_ReadChar (&netMessage)*0.25f;
		state->gunAngles[1] = MSG_ReadChar (&netMessage)*0.25f;
		state->gunAngles[2] = MSG_ReadChar (&netMessage)*0.25f;
	}
	// protocol changes

	if (flags & PS_BLEND) {
		state->vBlend[0] = MSG_ReadByte (&netMessage) * (1.0f/255.0f);
		state->vBlend[1] = MSG_ReadByte (&netMessage) * (1.0f/255.0f);
		state->vBlend[2] = MSG_ReadByte (&netMessage) * (1.0f/255.0f);
		state->vBlend[3] = MSG_ReadByte (&netMessage) * (1.0f/255.0f);
	}
...
static void CL_ParsePacketEntities (void)
{
	...
	if (cl.oldFrame) {
		if (oldindex < cl.oldFrame->numEntities) {
			oldstate = &cl_parseEntities[(cl.oldFrame->parseEntities+oldindex) & (MAX_PARSEENTITIES_MASK)];
	...
	for ( ; ; ) {
		...
		if (oldnum > newnum) {
			...
			CL_DeltaEntity (newnum, &cl_baseLines[newnum], bits);
			continue;
		}
...
static void CL_ParseFrame (int extraBits)
{
	...
	// protocol updates
	serverFrame = MSG_ReadLong (&netMessage);
	if (cls.serverProtocol != ENHANCED_PROTOCOL_VERSION) {
	...
	if (cls.serverProtocol != ENHANCED_PROTOCOL_VERSION) {
		...
		SHOWNET (svcStrings[(cmd > SVC_MAX) ? 0 : cmd]);
		if (cmd != SVC_PLAYERINFO)
			Com_Error (ERR_DROP, "CL_ParseFrame: not playerinfo");
	}
	CL_ParsePlayerstate (extraFlags);
...
qBool CL_CheckOrDownloadFile (char *fileName)
{
	...
	Q_FixPathName (cls.download.name, sizeof (cls.download.name), 3, qFalse);

	/*
	** download to a temp name, and only rename to the real name when done, so if interrupted
	** a runt file wont be left
	** copy, normalize the path, strip the extension, and replace with ".tmp"
	*/
	Q_strncpyz (cls.download.tempName, fileName, sizeof (cls.download.tempName));
	Q_FixPathName (cls.download.tempName, sizeof (cls.download.tempName), 3, qTrue);
...
static void CL_ParseDownload (qBool compressed)
{
	...
	// insert block
 	if (compressed) {
 		...
 		FS_ZLibDecompress (netMessage.data + netMessage.readCount, size, uncompressed, uncompressedLen, -15);
...
static qBool CL_ParseServerData (void)
{
	...
	if (cls.serverProtocol == ENHANCED_PROTOCOL_VERSION) {
		newVersion = MSG_ReadShort (&netMessage);
...
static void CL_ParseStartSoundPacket (void)
{
	...
	volume = (flags & SND_VOLUME) ? MSG_ReadByte (&netMessage) * (1.0f/255.0f) : DEFAULT_SOUND_PACKET_VOLUME;
...
void CL_ParseServerMessage (void)
{
	...
	for ( ; ; ) {
		...
		if (cl_shownet->value >= 2) {
			if (!svcStrings[(cmd > SVC_MAX) ? 0 : cmd])
		...
		default:
			if (!CL_CGModule_ParseServerMessage (cmd)) {
				if (!cls.moduleCGameActive) {
					Com_Printf (0, S_COLOR_YELLOW "WARNING: CGame not initialized to parse message, forcing init\n");
					CL_CGameAPI_Init ();
				...
				Com_Error (ERR_DROP, "CL_ParseServerMessage: Illegible server message\n"
					...
					cmd, svcStrings[(cmd > SVC_MAX) ? 0 : cmd],
					lastCmd, (lastCmd == -2) ? "None" : svcStrings[(lastCmd > SVC_MAX) ? 0 : lastCmd]);
			}
			break;
		}
