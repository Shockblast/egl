...
float	palRedf		(int color) { return ((graphPal[color*3+0] > 0) ? graphPal[color*3+0] * (1.0f/255.0f) : 0); }
float	palGreenf	(int color) { return ((graphPal[color*3+1] > 0) ? graphPal[color*3+1] * (1.0f/255.0f) : 0); }
float	palBluef	(int color) { return ((graphPal[color*3+2] > 0) ? graphPal[color*3+2] * (1.0f/255.0f) : 0); }
...
void SCR_DrawDebugGraph (void)
{
	...
	R_DrawFill (0, vidDef.height-scr_graphheight->value, vidDef.width, scr_graphheight->value, color);
	...
	for (a=0 ; a<vidDef.width ; a++) {
		...
		R_DrawFill (vidDef.width-1-a, vidDef.height-h, 1, h, color);
	}
}
...
static void SCR_Benchmark_f (void)
{
	...
	VectorMA (cl.frame.playerState.viewOffset, (1.0f/8.0f), cl.frame.playerState.pMove.origin, refDef.viewOrigin);
...
void SCR_UpdateScreen (void)
{
	...
	for (i=0 ; i<numFrames ; i++) {
		...
		if (scrDrawLoading) {
			...
			CL_UIModule_DrawConnectScreen (qTrue, qFalse, scrDownloading);
		}
			...
			case CA_LOADING:
				// Loading screen is drawn in cgame, where map media is initialized
				SCR_RenderView (separation[i]);

				scrDownloading = qFalse;
				CL_UIModule_DrawConnectScreen (qFalse, qTrue, qFalse);
				break;
