...
{
	return CL_Trace (start, end, size, 1);
}

void calcPartVelocity (cparticle_t *p, double scale, double *time, vec3_t velocity, double gravscale)
{
	double time1 = *time;
...
void pBloodDripThink (struct cparticle_s *p, vec3_t org, vec3_t angle, dvec4_t color, double *size, double *orient, double *time)
{
	...
	calcPartVelocity (p, 0.33, time, angle, *orient);
...
void pBounceThink (struct cparticle_s *p, vec3_t org, vec3_t angle, dvec4_t color, double *size, double *orient, double *time)
{
	...
	if (tr.fraction < 1)
	{
		calcPartVelocity (p, 0.9, time, velocity, 1);
		...
		VectorIdentity (p->accel);
		VectorCopy (tr.endPos, p->org);
		VectorCopy (p->org, org);
		VectorCopy (p->org, p->oldOrigin);

		if ((tr.plane.normal[2] > 0.6) && (VectorLength (p->vel) < 2))
		{
			if (p->flags & PF_GRAVITY)
				p->flags &= ~PF_GRAVITY;
			VectorIdentity (p->vel);
...
{
	R_AddLight (org, 375 * ((color[3] / p->color[3]) + (frand () * 0.05)), 1, 0.8, 0.6);

	if (color[3] > (p->color[3] * 0.95))
		p->image = clMedia.mediaTable[PT_EXPLO1];
	else if (color[3] > (p->color[3] * 0.9))
		p->image = clMedia.mediaTable[PT_EXPLO2];
	else if (color[3] > (p->color[3] * 0.8))
		p->image = clMedia.mediaTable[PT_EXPLO3];
	else if (color[3] > (p->color[3] * 0.65))
		p->image = clMedia.mediaTable[PT_EXPLO4];
	else if (color[3] > (p->color[3] * 0.3))
		p->image = clMedia.mediaTable[PT_EXPLO5];
	else if (color[3] > (p->color[3] * 0.15))
		p->image = clMedia.mediaTable[PT_EXPLO6];
	else
		p->image = clMedia.mediaTable[PT_EXPLO7];
...
void pExploWaveThink (struct cparticle_s *p, vec3_t org, vec3_t angle, dvec4_t color, double *size, double *orient, double *time)
{
	...
	calcPartVelocity (p, *orient, time, angle, *orient);
...
void pFireTrailThink (struct cparticle_s *p, vec3_t org, vec3_t angle, dvec4_t color, double *size, double *orient, double *time)
{
	...
	calcPartVelocity (p, 10, time, angle, *orient);
...
void pRicochetSparkThink (struct cparticle_s *p, vec3_t org, vec3_t angle, dvec4_t color, double *size, double *orient, double *time)
{
	...
	calcPartVelocity (p, 6, time, angle, *orient);
...
void pSplashThink (struct cparticle_s *p, vec3_t org, vec3_t angle, dvec4_t color, double *size, double *orient, double *time)
{
	...
	calcPartVelocity (p, 0.7, time, angle, *orient);
...
void pSplashPlumeThink (struct cparticle_s *p, vec3_t org, vec3_t angle, dvec4_t color, double *size, double *orient, double *time)
{
	...
	calcPartVelocity (p, 2, time, angle, *orient);
