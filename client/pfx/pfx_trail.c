...
void FASTCALL CL_BeamTrail (vec3_t start, vec3_t end, int color, double size, double alpha, double alphaVel)
{
	...
	for (; len>0 ; VectorAdd (move, vec, move))
	{
		...
		for (i=0 ; i<3 ; i++)
			dest[i] += ((rand () % ((int)(size*32) + 1) - (int)(size*16))/32);
...
void CL_BfgTrail (entity_t *ent)
{
	...
	/* core */
	CL_FlareEffect (ent->origin, PT_FLARE, 0 + (frand () * 32), 45, 45, 0xd0, 0xd0, 0.66, PART_INSTANT);
	CL_FlareEffect (ent->origin, PT_FLARE, 180 + (frand () * 32), 55, 55, 0xd0, 0xd0, 0.66, PART_INSTANT);

	CL_FlareEffect (ent->origin, PT_FLARE, 0 + (frand () * 32), 35, 35, 0xd7, 0xd7, 0.66, PART_INSTANT);
	CL_FlareEffect (ent->origin, PT_FLARE, 180 + (frand () * 32), 45, 45, 0xd7, 0xd7, 0.66, PART_INSTANT);
	...
	for (i=0 ; i<(pIncDegree (ent->origin, NUMVERTEXNORMALS/2.0, 0.5, qTrue)) ; i++)
	{
		...
		dist = VectorDistance (org, ent->origin) / 90.0;
...
void CL_BlasterGoldTrail (vec3_t start, vec3_t end)
{
	...
	orgscale = (FS_CurrGame ("gloom")) ? 0.5 : 1;
	velscale = (FS_CurrGame ("gloom")) ? 3 : 5;
...
lets you control the number of bubbles by
	...
	for (i=0 ; i<(clamp (cl_gore->value + 1, 1, 11) / 5.0) ; i++)
	{
		if (!(rand () % 26))
		{
			...
			makePart (
				...
				g ? 30 : 235+(frand ()*5),		g ? 70+(rand ()%91) : 245+(frand()*10),	g ? 30 : 245 + (frand ()*10),
				0,									0,									0,
				1.0,								-0.5 / (0.6 + (frand () * 0.3)),
				1.5 + frand (),						2 + frand (),
				PT_BLDDRIP,							PF_GRAVITY|PF_DIRECTION|PF_ALPHACOLOR,
				g ? GL_SRC_ALPHA : GL_ZERO,			g ? GL_ONE : GL_ONE_MINUS_SRC_COLOR,
...
void CL_RocketTrail (vec3_t start, vec3_t end)
{
	...
	dec = pDecDegree (start, 3.5, 0.25, qTrue);
	VectorScale (vec, dec, vec);

	i = inwater ? PT_BLUEFIRE : PT_FIRE;
	for (; len>0 ; VectorAdd (move, vec, move))
	{
		len -= dec;
		...
		makePart (
			move[0],							move[1],							move[2],
			0,									0,									0,
			vec[0] + (crand () * 0.25),			vec[1] + (crand () * 0.25),			vec[2] + (crand () * 0.25),
			0,									0,									0,
			235 + (frand () * 20),				235 + (frand () * 20),				255,
			235 + (frand () * 20),				235 + (frand () * 20),				255,
		...
		if (!(rand () % 4))
			makePart (
				move[0] + (crand () * 4),		move[1] + (crand () * 4),		move[2] + (crand () * 4),
				0,								0,								0,
				-vec[0] * (2 + (frand () * 3)) + (crand () * 2),
				-vec[1] * (2 + (frand () * 3)) + (crand () * 2),
				-vec[2] * (2 + (frand () * 3)) + (crand () * 2),
				0,								0,								0,
				255,							255,							100,
				255,							255,							50,
				1.0,							-1.0 / (1.5 + (frand () * 0.1)),
				0.5 + (frand () * 0.25),		0.4 + (frand () * 0.25),
				PT_RED,							PF_SCALED|PF_DIRECTION,
				GL_SRC_ALPHA,					GL_ONE,
				pSplashThink,					qTrue,
				PMAXSPLASHLEN);
	}
