...
void CL_LoadLoc (char *mapName)
{
	...
	if (!fileBuffer || fileLen <= 0) {
		Com_DevPrintf (0, "CL_LoadLoc: %s not found\n", cl_locFileName);
		return;
	}

	buf = (char *)Mem_PoolAllocExt (fileLen+1, qFalse, MEMPOOL_LOCSYS, 0);
	...
	while (token) {
		...
		Com_DevPrintf (0, "CL_AddLoc: adding location '%s'\n", loc->name);
	...
	if (!finished) {
		Com_Printf (PRNT_ERROR, "CL_LoadLoc: Bad loc file '%s'\n", cl_locFileName);
		CL_FreeLoc ();
	}
		...
		while (*sayText && *(sayText + 1)) {
			if ((*sayText == '@') && (toupper(*(sayText + 1)) == 'L')) {
				...
				if (cmdLen + locLen >= MAX_STRING_CHARS) {
					Com_DevPrintf (0, "CL_Say_Preprocessor: location expansion aborted, not enough space\n");
					break;
				}
