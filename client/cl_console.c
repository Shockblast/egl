...
================
*/
static void CL_MessageModeX_f (void)
{
	Com_Printf (0, "%s\n", Cmd_Argv(0));
	if (Com_ClientState () != CA_ACTIVE)
		return;
...
void CL_ConsoleInit (void)
{
	...
	Cmd_AddCommand ("messagemode",		CL_MessageMode_f,		"Public message");
	Cmd_AddCommand ("messagemode2",		CL_MessageMode2_f,		"Team/group message");
	Cmd_AddCommand ("messagemodex",		CL_MessageModeX_f,		"Custom prompt");
...
static void CL_DrawNotify (void)
{
	...
	if (con_notifylines->intVal) {
		...
		for (v=charSize[1]*(totalLines-1), notifyLines=0, i=cl_console.currentLine ; i>cl_console.currentLine-CON_MAXTIMES+1 ; i--) {
			...
			if (con_notifyfade->intVal)
				color[3] = 0.5f + 0.5f * (con_notifytime->floatVal - (time * 0.001)) / con_notifytime->floatVal;
	...
	if (Key_GetDest () == KD_MESSAGE) {
		...
		case CHATDEST_CUSTOM:
			skip = R_DrawString (NULL, 4, v, newScale, newScale, 0, key_customPrompt, Q_colorWhite) + 1;
			break;
		default:
			assert (0);
			break;
