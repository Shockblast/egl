...
void Con_DrawConsole (void) {
	...
	Draw_String (NULL, vidDef.width - (Q_strlen (version) * FT_SIZE) - (FT_SHAOFFSET * 2),
	...
	if (con_clock->integer) {
		...
		if (clockStr[0] == '0') {
			for (i=0 ; i<(Q_strlen (clockStr) + 1) ; i++)
				clockStr[i] = clockStr[i+1];
			clockStr[i+1] = '\0';
		}

		Draw_String (NULL, vidDef.width - (Q_strlen (clockStr) * FT_SIZE) - (FT_SHAOFFSET * 2),
	...
	if (cls.download.file) {
		...
		Q_snprintfz (dlbar + Q_strlen(dlbar), sizeof (dlbar), " %02d%%", cls.download.percent);
