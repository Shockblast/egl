...
void CL_WriteDemoPacketEntities (const frame_t *from, frame_t *to, netMsg_t *msg)
{
	...
	while (newIndex < to->numEntities || oldIndex < from_numEntities) {
		...
		if (newNum == oldNum) {
			...
			*/
			MSG_WriteDeltaEntity (oldEnt, newEnt, msg, qFalse, newEnt->number <= cl.maxClients);
			oldIndex++;
			newIndex++;
			continue;
		}
		...
		if (newNum < oldNum) {
			...
			MSG_WriteDeltaEntity ((entityStateOld_t *)&cl_baseLines[newNum], newEnt, msg, qTrue, qTrue);
			newIndex++;
			continue;
		}
		...
		if (newNum > oldNum) {
			...
			MSG_WriteDeltaEntity (oldEnt, NULL, msg, qTrue, qFalse);
			oldIndex++;
			continue;
		}
	}
...
void CL_WriteDemoMessageChunk (byte *buffer, int length, qBool forceFlush)
{
	...
	if (length)
		MSG_Write (&cl.demoBuffer, buffer, length);
}
...
qBool CL_StartDemoRecording (char *name)
{
	...
	for (i=0; i<MAX_CS_EDICTS ; i++) {
		...
		MSG_WriteDeltaEntity (&nullstate, ent, &buf, qTrue, qTrue);
	}
