...
extern unsigned int	sys_FrameTime;
unsigned int		sys_OldFrameTime;
unsigned int		in_FrameMSec;
...
void KeyDown (kButton_t *b) {
	...
	if (!b->down[0])
		...
		Com_Printf (PRINT_ALL, S_COLOR_YELLOW "Three keys down for a button!\n");
		return;
	}
...
void IN_UpDown (void)		{ KeyDown (&in_MoveUp); }
void IN_UpUp (void)			{ KeyUp (&in_MoveUp); }
void IN_DownDown (void)		{ KeyDown (&in_MoveDown); }
void IN_DownUp (void)		{ KeyUp (&in_MoveDown); }
void IN_LeftDown(void)		{ KeyDown (&in_LookLeft); }
void IN_LeftUp (void)		{ KeyUp (&in_LookLeft); }
void IN_RightDown (void)	{ KeyDown (&in_LookRight); }
void IN_RightUp (void)		{ KeyUp (&in_LookRight); }
...
void CL_FinishMove (userCmd_t *cmd) {
	...
	if (key_AnyDown && Key_KeyDest (KD_GAME))
...
userCmd_t CL_CreateCmd (void) {
	...
	in_FrameMSec = clamp (sys_FrameTime - sys_OldFrameTime, 1, 200);
...
void CL_SendCmd (void) {
	...
	cmd = &cl.cmds[i];
	cl.cmdTime[i] = cls.realTime;	// for netgraph ping calculation
	...
	*cmd = CL_CreateCmd ();
	...
	if (CL_ConnectionState (CA_CONNECTED)) {
		if (cls.netChan.message.curSize	|| ((g_CurTime - cls.netChan.lastSent) > 1000))
