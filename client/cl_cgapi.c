...
static void CL_CGModule_Init (void) {
	...
	R_MediaInit (); // because we can't register models before map load
...
void CL_CGameAPI_Init (void) {
	...
	CL_CGameAPI_Shutdown (qFalse);	// shutdown if already active
	...
	cgi.Img_RegisterImage			= Img_RegisterImage;
