...
static void CL_CGModule_Init (void)
{
	...
	KeepAlive ();
	cge->Init (cl.playerNum, cls.serverProtocol, cl.attractLoop, vidDef.width, vidDef.height);
	KeepAlive ();

	CL_UIModule_RegisterMedia ();	// Make sure UI media is touched before registration ends

	R_MediaInit ();	// Touch before registration ends
...
{
	if (cge)
		cge->RenderView (vidDef.width, vidDef.height, cls.realTime, cls.trueNetFrameTime, cls.trueRefreshFrameTime, stereoSeparation, qFalse);
}
...
void CL_CGameAPI_Init (void)
{
	...
	oldState = cls.connectState;
	cls.connectState = CA_LOADING;
