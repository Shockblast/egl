...
void CL_CGModule_LoadMap (void)
{
	...
	cge->LoadMap (cl.playerNum, cls.serverProtocol, cl.attractLoop, cl.strafeHack, &cls.glConfig);
	Netchan_Transmit (&cls.netChan, 0, NULL);
...
void CL_CGModule_UpdateConnectInfo (void)
{
	...
	if (cls.download.file) {
		...
		cge->UpdateConnectInfo (cls.serverName, cls.connectCount, cls.download.name, cls.download.percent, size);
		return;
	}

	// Nothing downloading
	cge->UpdateConnectInfo (cls.serverName, cls.connectCount, NULL, -1, -1);
}
...
{
	if (!cg_developer->integer && !developer->integer)
		return;
...
{
	return _Mem_Alloc (size, zeroFill, MEMPOOL_CGAMESYS, tagNum, fileName, fileLine);
}
...
===============
*/
int CGI_ChangeTag (const int tagFrom, const int tagTo)
{
	return _Mem_ChangeTag (MEMPOOL_CGAMESYS, tagFrom, tagTo);
}
...
===============
*/
static void CGI_Free (const void *ptr, const char *fileName, const int fileLine)
{
...
===============
*/
static void CGI_FreeTag (const int tagNum, const char *fileName, const int fileLine)
{
	_Mem_FreeTag (MEMPOOL_CGAMESYS, tagNum, fileName, fileLine);
}
...
{
	return _Mem_PoolStrDup (in, MEMPOOL_CGAMESYS, tagNum, fileName, fileLine);
}
...
===============
*/
static int CGI_TagSize (const int tagNum)
{
	return _Mem_TagSize (MEMPOOL_CGAMESYS, tagNum);
}
...
void CL_CGameAPI_Init (void)
{
	...
	cgi.R_GetGLConfig				= R_GetGLConfig;
	...
	cgi.R_RegisterTexture			= R_RegisterTexture;
	...
	if (!cge)
		Com_Error (ERR_FATAL, "CGameAPI_Init: Find/load of CGame library failed!");

	// Check the api version
	if (cge->apiVersion != CGAME_APIVERSION) {
		Sys_UnloadLibrary (LIB_CGAME);
		Com_Error (ERR_FATAL, "CGameAPI_Init: incompatible apiVersion (%i != %i)", cge->apiVersion, CGAME_APIVERSION);
	}
	...
	|| !cge->Pmove || !cge->RegisterSounds || !cge->RenderView || !cge->SetGLConfig
	|| !cge->MainMenu || !cge->ForceMenuOff || !cge->MoveMouse || !cge->KeyEvent
	|| !cge->ParseServerInfo || !cge->ParseServerStatus)
		Com_Error (ERR_FATAL, "CGameAPI_Init: Failed to find all CGame exports!");
...
void CL_CGameAPI_Shutdown (void)
{
	...
	size = Mem_PoolSize (MEMPOOL_CGAMESYS);
	if (size > 0)
		Com_Printf (PRNT_WARNING, "WARNING: CGame memory leak (%i bytes)\n", size);
