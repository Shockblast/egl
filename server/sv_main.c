...
static void SVC_Ack (void)
{
	...
	for (i=0 ; i<MAX_MASTERS ; i++) {
		if (svMasterAddr[i].port && NET_CompareBaseAdr (svMasterAddr[i], netFrom)) {
...
static void SVC_GetChallenge (void)
{
	...
	if (i == MAX_CHALLENGES) {
		...
		svs.challenges[oldest].time = sys_currTime;
		i = oldest;
	}
...
void SV_Frame (int msec)
{
	...
	if (!sv_timedemo->value && (uInt)svs.realTime < sv.time) {
...
void SV_Init (void)
{
	...
	Cvar_Get ("protocol",		Q_VarArgs ("%i", ORIGINAL_PROTOCOL_VERSION),	CVAR_SERVERINFO|CVAR_NOSET);
