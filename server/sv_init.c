...
================
*/
int SV_FindIndex (char *name, int start, int max, qBool create) {
	int		i;
	...
	if (sv.state != SS_LOADING) {
		...
		SV_Multicast (vec3Identity, MULTICAST_ALL_R);
	}
...
================
*/
void SV_SpawnServer (char *server, char *spawnpoint, serverState_t serverstate, qBool attractloop, qBool loadgame) {
	int			i;
	unsigned	checksum;
	...
	Com_Printf (PRINT_ALL, "--------- Server Initialization --------\n");

	Com_Printf (PRINT_DEV, "SpawnServer: %s\n",server);
	if (sv.demo_File)
		fclose (sv.demo_File);
...
void SV_InitGame (void) {
	...
	if (Cvar_VariableValue ("coop") && Cvar_VariableValue ("deathmatch")) {
		Com_Printf (PRINT_ALL, "Deathmatch and Coop both set, disabling Coop\n");
	...
	*/
	...
	svs.num_ClientEntities = maxclients->integer*UPDATE_BACKUP*64;
	svs.client_Entities = Z_Malloc (sizeof (entityState_t)*svs.num_ClientEntities);
