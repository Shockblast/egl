...
static void *GI_TagAlloc (int size, int tagNum)
{
	...
	return _Mem_Alloc (size, qTrue, MEMPOOL_GAMESYS, tagNum, "GAME DLL", -1);
}
...
static void GI_FreeTags (int tagNum)
{
	...
	_Mem_FreeTag (MEMPOOL_GAMESYS, tagNum, "GAME DLL", -1);
}
...
static void GI_Pmove (pMove_t *pMove)
{
	...
	if (!dedicated->integer && !CL_CGModule_Pmove (&epm, sv_airAcceleration))
#endif
...
void SV_GameAPI_Init (void)
{
	...
	|| !ge->ClientThink || !ge->RunFrame || !ge->ServerCommand)
		Com_Error (ERR_FATAL, "GameAPI_Init: Failed to find all CGame exports!");
...
void SV_GameAPI_Shutdown (void)
{
	...
	size = Mem_PoolSize (MEMPOOL_GAMESYS);
	if (size > 0)
		Com_Printf (PRNT_WARNING, "WARNING: Game memory leak (%i bytes)\n", size);
}
