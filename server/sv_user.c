...
static void SV_New_f (void) {
	...
	*/
	...
	MSG_WriteLong (&svCurrClient->netChan.message, PROTOCOL_VERSION);
...
static void SV_Baselines_f (void) {
	...
	while ((svCurrClient->netChan.message.curSize <  MAX_MSGLEN/2) && (start < MAX_CS_EDICTS)) {
		...
		if (base->modelIndex || base->sound || base->effects) {
			...
			MSG_WriteDeltaEntity (&nullstate, base, &svCurrClient->netChan.message, qTrue, qTrue);
		}
...
void SV_ExecuteClientMessage (svClient_t *cl) {
	...
	for ( ; ; ) {
		if (netMessage.readCount > netMessage.curSize) {
			Com_Printf (0, "SV_ReadClientMessage: badread\n");
			SV_DropClient (cl);
			return;
		}	
		...
		case CLC_USERINFO:
			strncpy (cl->userInfo, MSG_ReadString (&netMessage), sizeof (cl->userInfo)-1);
