...
static void SV_New_f (void)
{
	...
	if (sv_currentClient->state != SVCS_CONNECTED) {
		Com_Printf (PRNT_WARNING, "New not valid -- already spawned\n");
		return;
	}
	...
	if (Com_ServerState () == SS_CINEMATIC || Com_ServerState () == SS_PIC)
		playerNum = -1;
	else
...
static void SV_Baselines_f (void)
{
	...
	while (sv_currentClient->netChan.message.curSize < MAX_SV_MSGLEN/2 && start < MAX_CS_EDICTS) {
		...
		if (base->modelIndex || base->sound || base->effects) {
			...
			MSG_WriteDeltaEntity (&nullstate, base, &sv_currentClient->netChan.message, qTrue, qTrue);
		}
...
static void SV_NextDownload_f (void)
{
	...
	MSG_Write (&sv_currentClient->netChan.message,
		sv_currentClient->download + sv_currentClient->downloadCount - r, r);
...
static void SV_BeginDownload_f (void)
{
	...
	sv_currentClient->downloadSize = FS_LoadFile (name, (void **)&sv_currentClient->download);
