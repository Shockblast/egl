...
static void SV_ConfigStrings_f (void)
{
	...
	while ((svCurrClient->netChan.message.curSize < MAX_SV_MSGLEN/2) && (start < MAX_CONFIGSTRINGS)) {
...
static void SV_Baselines_f (void)
{
	...
	// write a packet full of data
	while ((svCurrClient->netChan.message.curSize < MAX_SV_MSGLEN/2) && (start < MAX_CS_EDICTS)) {
...
{
	if (svCurrClient->state == SVCS_SPAWNED)
		return;
