...
=============
*/
static void SV_EmitPacketEntities (svClient_t *cl, clientFrame_t *from, clientFrame_t *to, netMsg_t *msg)
{
	...
	while (newindex < to->numEntities || oldindex < from_numEntities) {
		...
		if (newnum == oldnum) {
			...
			*/
			MSG_WriteDeltaEntity (oldent, newent, msg, qFalse, newent->number <= maxclients->integer, 0,
		...
		if (newnum < oldnum) {
			...
			MSG_WriteDeltaEntity (&sv.baseLines[newnum], newent, msg, qTrue, qTrue, 0,
#ifdef ENHANCED_SERVER
				cl->protocol);
#else
				ORIGINAL_PROTOCOL_VERSION);
...
void SV_BuildClientFrame (svClient_t *client)
{
	...
	for (i=0 ; i<3 ; i++) {
		org[i] = clent->client->ps.pMove.origin[i]*(1.0f/8.0f) + clent->client->ps.viewOffset[i];
	}
	...
	for (e=1 ; e<ge->numEdicts ; e++) {
		...
		if (ent->s.number != e) {
			Com_Printf (PRNT_DEV, "FIXING ENT->S.NUMBER!!!\n");
			ent->s.number = e;
		}
...
void SV_RecordDemoMessage (void)
{
	...
	while (e < ge->numEdicts) {
		...
		if (ent->inUse && ent->s.number &&
			...
			MSG_WriteDeltaEntity (&nostate, &ent->s, &buf, qFalse, qTrue, 0,
#ifdef ENHANCED_SERVER
				ent->client->protocol);
#else
				ORIGINAL_PROTOCOL_VERSION);
	...
	MSG_Write (&buf, svs.demoMultiCast.data, svs.demoMultiCast.curSize);
