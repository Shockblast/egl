...
static void SV_WriteServerFile (qBool autoSave) {
	...
	if (!autoSave) {
		...
		strncat (comment, sv.configStrings[CS_NAME], sizeof (comment) - 1 - Q_strlen (comment));
	}
	else {
		// autosaved
	...
	for (var=cvarVariables ; var ; var=var->next) {
		...
		if ((Q_strlen (var->name) >= sizeof (name)-1) || (Q_strlen (var->string) >= sizeof (string)-1)) {
...
static void SV_DemoMap_f (void) {
	...
	map = Cmd_Argv (1);
	if (!strstr (map, ".")) {
		Q_snprintfz (expanded, sizeof (expanded), "demos/%s.dm2", map);
		if (FS_LoadFile (expanded, NULL) == -1) {
			Com_Printf (0, "Can't find %s\n", expanded);
			return;
		}
		map = strstr (expanded, "/")+1;
	}

	SV_Map (qTrue, map, qFalse);
}
	...
	for (i=0 ; i<MAX_CONFIGSTRINGS ; i++) {
		if (sv.configStrings[i][0]) {
			...
			if (buf.curSize + 67 >= buf.maxSize) {
				Com_Printf (0, S_COLOR_RED "Not enough buffer space available.\n");
				fclose (svs.demoFile);
				return;
			}
		}
