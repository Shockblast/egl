...
static libList_t sys_libList[LIB_MAX] = {
	{ "LIB_CGAME",	NULL,	EGLCGAME_NAME,	"GetCGameAPI"	},
	{ "LIB_GAME",	NULL,	GAME_NAME,	"GetGameAPI"	},
};
...
void *Sys_LoadLibrary (libType_t libType, void *parms)
{
	...
	while (1) {
		...
		*lib = dlopen (name, RTLD_LAZY);
		if (*lib) {
			Com_Printf (0, "dlopen (%s)\n", name);
			break;
		}
		else {
			Q_snprintfz (name, sizeof(name), "%s", libName);
...
static void OS_Info(void) 
{
#include <sys/utsname.h>
	struct utsname info;
	
	uname (&info);
	Com_Printf(0, "OS Info\n");
	Com_Printf(0, "System:   %s\n", info.sysname);
	Com_Printf(0, "Kernel:   %s\n", info.release);
	Com_Printf(0, "Machine:  %s\n\n", info.machine);
