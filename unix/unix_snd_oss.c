...
qBool OSS_Init (void)
{
	...
	if (!Q_stricmp (s_oss_device->string, "default"))
		...
		oss_curDevice = Mem_PoolStrDup ("/dev/dsp0.0", cl_soundSysPool, 0);
#endif
	else
	...
	Com_Printf (0, "Initializing OSS Sound System\n");
...
{
	if (oss_initialized) {
		munmap (snd_audioDMA.buffer, snd_audioDMA.samples *snd_audioDMA.sampleBits / 8);
		snd_audioDMA.buffer = 0L;
		close(oss_audioFD);
		oss_audioFD = -1;
