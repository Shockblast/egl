...
int X11_LateKey (XKeyEvent *ev)
{
	...
	case 92:		return '~'; /* Italy */
	case 94:		return '~'; /* Deutschland */
	case 178:		return '~'; /* France */
	case 186:		return '~'; /* Spain */
...
char *Sys_GetClipboardData (void)
{
	...
	if (sowner != None) {
		property = XInternAtom(x11display.dpy,
				       "GETCLIPBOARDDATA_PROP",
				       False);
				
		XConvertSelection(x11display.dpy,
				  XA_PRIMARY, XA_STRING,
				  property, x11display.win, myxtime); /* myxtime == time of last X event */
		XFlush(x11display.dpy);

		XGetWindowProperty(x11display.dpy,
				   x11display.win, property,
