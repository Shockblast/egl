...
#define USE_BYTESWAP
#include "snd_loc.h"
#include "../win32/winquake.h" // FIXME
...
static uLong	sndRegistrationFrame;
...
static sfx_t *Snd_FindName (char *name, qBool create) {
	...
	for (p=outName, flipped=qFalse ; p=strstr (outName, "\\") ; ) {
		flipped = qTrue;
		*p = '/';
	}
	if (flipped)
		Com_Printf (0, S_COLOR_CYAN "Snd_FindName: flipped: %s -> %s", name, outName);
	// orly test
...
static void Snd_AddLoopSounds (void) {
	if (cl_paused->integer || !cls.soundPrepped)
		return;
...
void Snd_Update (vec3_t viewOrigin, vec3_t forward, vec3_t right, vec3_t up, qBool underWater) {
	...
	if (sndALActive) {
		...
		qalListenerf (AL_GAIN, sysActiveApp ? s_volume->value : 0.0);
		...
		if (alConfig.extEAX) {
			...
			if ((cls.connectState != CA_ACTIVE) || cl.attractLoop)
