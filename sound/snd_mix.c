...
static __declspec (naked) void Snd_WriteLinearBlastStereo16 (void)
{
	__asm {
		push edi
		push ebx
...
================
*/
static void Snd_TransferStereo16 (uLong *pbuf, int endTime)
{
	int		lpos;
	int		lpaintedtime;
	...
	while (lpaintedtime < endTime) {
		...
		sndLinearCount = (snd_audioDMA.samples>>1) - lpos;
...
===================
*/
static void Snd_TransferPaintBuffer (int endTime)
{
	int		out_idx;
	int		count;
	int		out_mask;
	...
	pbuf = (uLong *)snd_audioDMA.buffer;
	...
	if (s_testsound->value) {
		...
		for (i=0 ; i<count ; i++)
			sndPaintBuffer[i].left = sndPaintBuffer[i].right = sin ((snd_paintedTime+i)*0.1)*20000*256;
	}
	...
	if (snd_audioDMA.sampleBits == 16 && snd_audioDMA.channels == 2) {
		...
		count = (endTime - snd_paintedTime) * snd_audioDMA.channels;
		out_mask = snd_audioDMA.samples - 1; 
		out_idx = snd_paintedTime * snd_audioDMA.channels & out_mask;
...
static void Snd_PaintChannelFrom8 (channel_t *ch, sfxCache_t *sc, int count, int offset)
{
	int		data;
	int		*lScale, *rScale;
	byte	*sfx;
...
static __declspec (naked) void Snd_PaintChannelFrom8 (channel_t *ch, sfxCache_t *sc, int count, int offset)
{
	__asm {
		push esi
		push edi
...
================
*/
static void Snd_PaintChannelFrom16 (channel_t *ch, sfxCache_t *sc, int count, int offset)
{
	int		data, i;
	int		left, right;
...
void Snd_PaintChannels (int endTime)
{
	...
	while (snd_paintedTime < endTime) {
		...
		for ( ; ; ) {
			...
			if ((int)ps->begin <= snd_paintedTime) {
		...
		if (snd_rawEnd < snd_paintedTime)
			memset (sndPaintBuffer, 0, (end - snd_paintedTime) * sizeof (sfxSamplePair_t));
		else {
			...
			for (i=snd_paintedTime ; i<stop ; i++) {
				...
				sndPaintBuffer[i-snd_paintedTime] = snd_rawSamples[s];
			}

			for ( ; i<end ; i++) {
				sndPaintBuffer[i-snd_paintedTime].left =
				sndPaintBuffer[i-snd_paintedTime].right = 0;
			}
		}
