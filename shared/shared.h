...
vec_t	VectorNormalize (const vec3_t in, vec3_t out);	// returns vector length
...
float	InterpolateAngle (const float a1, float a2, const float frac);
...
extern inline float	BigFloat	(float f);
extern inline int	BigLong		(int l);
extern inline short	BigShort	(short s);
...
extern inline float	LittleFloat	(float f);
extern inline int	LittleLong	(int l);
extern inline short	LittleShort	(short s);
...
enum {
	...
	CVAR_LATCH_SERVER	= 1 << 4,	// save changes until server restart
	CVAR_LATCH_VIDEO	= 1 << 5,	// save changes until video restart
	CVAR_LATCH_AUDIO	= 1 << 6,	// save changes until audio restart
	CVAR_RESET_GAME		= 1 << 7	// reset game dir when this cvar is modified
};
