...
#define DEFFUNCS		// use constants for mathlib functions
...
# define VectorDistanceFast(v1,v2)	(Q_FastSqrt((((v1)[0]-(v2)[0])*((v1)[0]-(v2)[0])+((v1)[1]-(v2)[1])*((v1)[1]-(v2)[1])+((v1)[2]-(v2)[2])*((v1)[2]-(v2)[2]))))
...
void	_Vector2Clear (vec2_t in);
void	_Vector2Copy (vec2_t in, vec2_t out);
void	_Vector2Scale (const vec2_t in, const vec_t scale, vec2_t out);
void	_Vector2Set (vec2_t out, const vec_t vec0, const vec_t vec1);
...
# define Vector2Clear(in)		_Vector2Clear (in);
# define Vector2Copy(a,b)		_Vector2Copy (a, b);
# define Vector2Scale(in,s,out)	_Vector2Scale (in, s, out);
# define Vector2Set(v,x,y)		_Vector2Set (v, x, y);

# define VectorAdd(a,b,out)		_VectorAdd (a, b, out);
...
# define VectorDistanceFast(v1,v2)	_VectorDistanceFast (v1, v2);
# define VectorLengthFast(v)		_VectorLengthFast (v);
...
vec_t	VectorNormalize (vec3_t in, vec3_t out);	// returns vector length
...
#ifdef id386
size_t __cdecl Q_strlen (const char *s);
void __cdecl Q_strlwr (char *s);
...
================
*/
static inline uLong CalcHash (char *name, const int maxSize) {
	uLong	hashValue;
	uInt	ch;
...
==============================================================================
	...
	STRING FIXUP FUNCTIONS
 
==============================================================================
*/

#ifdef USE_FIXPATHNAME
static inline char *Q_FixPathName (const char *name, qBool stripExtension) {
	static char	tempName[2][MAX_QPATH];
	static int	index = 0;
	int			i, len, lastDot;
	...
	//
	lastDot = -1;
	memset (&tempName[index], 0, sizeof (tempName[index]));
	for (i=((name[0] == '\\') || (name[0] == '/'))?1:0, len=0 ; name[i] && (len < sizeof (tempName[index])-5) ; i++) {
		switch (name[i]) {
		case '\\':
			tempName[index][len++] = '/';
			continue;
	...
	// trim the extension
	if (stripExtension)
		tempName[index][len] = '\0';

	return tempName[index];
}
#endif // USE_FIXPATHNAME
...
#define SURF_HINT				0x100	// these aren't known to the engine I believe
#define SURF_SKIP				0x200	// only the compiler uses them
