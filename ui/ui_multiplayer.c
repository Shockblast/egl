...
{
	UI_StartFramework (&m_multiPlayerMenu.frameWork);
...
static void MultiplayerMenu_Draw (void)
{
	...
	m_multiPlayerMenu.frameWork.x			= uiState.vidWidth * 0.5f;
	...
	// Render
	UI_CenterMenuHeight (&m_multiPlayerMenu.frameWork);
	UI_SetupMenuBounds (&m_multiPlayerMenu.frameWork);

	UI_AdjustCursor (&m_multiPlayerMenu.frameWork, 1);
	UI_Draw (&m_multiPlayerMenu.frameWork);
}
...
=============
*/
static struct sfx_s *MultiplayerMenu_Key (int key)
{
	return DefaultMenu_Key (&m_multiPlayerMenu.frameWork, key);
}
...
{
	MultiplayerMenu_Init ();
	UI_PushMenu (&m_multiPlayerMenu.frameWork, MultiplayerMenu_Draw, MultiplayerMenu_Close, MultiplayerMenu_Key);
}
