...
static void OverbrightBitsFunc (void *unused) {
	Com_sprintf(m_glexts_menu.overbrights_amount.generic.name,
				sizeof (char) * 2,
...
static void AnisoAmountFunc (void *unused) {
	Com_sprintf(m_glexts_menu.anisoamount_amount.generic.name,
				sizeof (char) * 3,
...
static void MultisamplesFunc (void *unused) {
	Com_sprintf(m_glexts_menu.multisamples_amount.generic.name,
				sizeof (char) * 2,
				"%.0f", m_glexts_menu.multisamples_slider.curvalue * 2);
}
#endif

void GLExtsMenu_SetValues (void); // kthx
...
//	m_glexts_menu.multisamples_amount.generic.name	= uii.Cvar_VariableString ("gl_ext_samples");
	...
	if (!strcmp (uii.Cvar_VariableString ("gl_nv_multisample_filter_hint"), "fastest"))
...
void GLExtsMenu_Init (void) {
	...
	m_glexts_menu.framework.nitems	= 0;
...
=============
*/
const char *GLExtsMenu_Key (int key) {
