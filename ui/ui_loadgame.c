...
static void LoadGameMenu_Init (void)
{
	...
	UI_StartFramework (&m_loadGameMenu.frameWork);
...
static void LoadGameMenu_Draw (void)
{
	...
	m_loadGameMenu.frameWork.x			= uiState.vidWidth * 0.5f;
	...
	UI_CenterMenuHeight (&m_loadGameMenu.frameWork);
	UI_SetupMenuBounds (&m_loadGameMenu.frameWork);

	UI_AdjustCursor (&m_loadGameMenu.frameWork, 1);
	UI_Draw (&m_loadGameMenu.frameWork);
}
...
=============
*/
static struct sfx_s *LoadGameMenu_Key (int key)
{
	return DefaultMenu_Key (&m_loadGameMenu.frameWork, key);
}
...
{
	LoadGameMenu_Init ();
	UI_PushMenu (&m_loadGameMenu.frameWork, LoadGameMenu_Draw, LoadGameMenu_Close, LoadGameMenu_Key);
}
