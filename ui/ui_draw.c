...
void Action_Draw (uiAction_t *a) {
	...
	if (a->generic.flags & UIF_LEFT_JUSTIFY)
		...
		xoffset = (((Q_ColorCharCount (a->generic.name, Q_strlen (a->generic.name))) * UISIZE_TYPE (a->generic.flags))*0.5);
	else
		xoffset = ((Q_ColorCharCount (a->generic.name, Q_strlen (a->generic.name))) * UISIZE_TYPE (a->generic.flags)) + RCOLUMN_OFFSET;
...
void Field_Draw (uiField_t *f) {
	...
	if (f->generic.flags & UIF_CENTERED) {
		...
		if (f->generic.name)
			x -= (Q_ColorCharCount (f->generic.name, Q_strlen (f->generic.name)) * UISIZE_TYPE (f->generic.flags)) * 0.5;
	}

	// title
	if (f->generic.name)
		UI_DrawString (x - ((Q_ColorCharCount (f->generic.name, Q_strlen (f->generic.name)) + 1) * UISIZE_TYPE (f->generic.flags)),
...
void Slider_Draw (uiSlider_t *s) {
	...
	// name
	if (s->generic.name) {
		UI_DrawString (s->generic.x + s->generic.parent->x + LCOLUMN_OFFSET - ((Q_ColorCharCount (s->generic.name, Q_strlen (s->generic.name)) - 1)*UISIZE_TYPE (s->generic.flags)),
...
void Selection_Draw (uiCommon_t *i) {
	if (i->flags & UIF_NOSELBAR)
		...
		if ((i->flags & UIF_FORCESELBAR) || ((uiCursorItem && (uiCursorItem == i)) || (uiSelItem && (uiSelItem == i))))
	...
	*/
	...
	if (item && item->cursorDraw)
		...
		if (item->flags & UIF_LEFT_JUSTIFY)
			UI_DrawChar (menu->x + item->x + LCOLUMN_OFFSET + item->cursorOffset, menu->y + item->y, 0, UISCALE_TYPE (item->flags), 12 + ((int)(uis.time/250) & 1), colorWhite);
		else if (item->flags & UIF_CENTERED)
			UI_DrawChar (menu->x + item->x - (((Q_ColorCharCount (item->name, Q_strlen (item->name)) + 4) * UISIZE_TYPE (item->flags))*0.5) + item->cursorOffset, menu->y + item->y, 0, UISCALE_TYPE (item->flags), 12 + ((int)(uis.time/250) & 1), colorWhite);
		else
			UI_DrawChar (menu->x + item->x + item->cursorOffset, menu->y + item->y, 0, UISCALE_TYPE (item->flags), 12 + ((int)(uis.time/250) & 1), colorWhite);
	}
	...
	if (selitem && (selitem->type != UITYPE_FIELD) && (selitem->type != UITYPE_IMAGE) && (!(selitem->flags & UIF_NOSELECT))) {
		if (selitem->flags & UIF_LEFT_JUSTIFY)
			UI_DrawChar (menu->x + selitem->x + LCOLUMN_OFFSET + selitem->cursorOffset, menu->y + selitem->y, 0, UISCALE_TYPE (selitem->flags), 12 + ((int)(uis.time/250) & 1), colorWhite);
		else if (selitem->flags & UIF_CENTERED)
			UI_DrawChar (menu->x + selitem->x - (((Q_ColorCharCount (selitem->name, Q_strlen (selitem->name)) + 4) * UISIZE_TYPE (selitem->flags))*0.5) + selitem->cursorOffset, menu->y + selitem->y, 0, UISCALE_TYPE (selitem->flags), 12 + ((int)(uis.time/250) & 1), colorWhite);
		else
			UI_DrawChar (menu->x + selitem->x + selitem->cursorOffset, menu->y + selitem->y, 0, UISCALE_TYPE (selitem->flags), 12 + ((int)(uis.time/250) & 1), colorWhite);
	}
...
void UI_DrawStatusBar (char *string) {
	if (string) {
		int col = (uis.vidWidth*0.5) - (((Q_ColorCharCount (string, Q_strlen (string))) * 0.5) * UIFT_SIZE);
