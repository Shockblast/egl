...
static void MainCursorDrawFunc (uiFrameWork_t *menu)
{
	...
	m_mainMenu.cursorNum = ((int)(uiState.time / 100)) % MAINMENU_CURSOR_NUMFRAMES;

	uii.R_GetImageSize (uiMedia.menus.mainGame, NULL, &h);

	y = (((m_mainMenu.frameWork.cursor%MAIN_ITEMS) * (h + 5)));

	uii.R_GetImageSize (uiMedia.menus.mainCursors[m_mainMenu.cursorNum], &w, &h);

	UI_DrawPic (uiMedia.menus.mainCursors[m_mainMenu.cursorNum],
...
{
	UI_StartFramework (&m_mainMenu.frameWork);
...
static void MainMenu_Draw (void)
{
	...
	m_mainMenu.frameWork.x				= uiState.vidWidth * 0.5 - (40 * UIFT_SCALE);
	...
	uii.R_GetImageSize (uiMedia.menus.mainCursors[m_mainMenu.cursorNum], &curWidth, NULL);
	uii.R_GetImageSize (uiMedia.menus.mainPlaque, &plaqueWidth, NULL);
	uii.R_GetImageSize (uiMedia.menus.mainLogo, &logoWidth, NULL);

	m_mainMenu.plaque_pic.generic.x			= - ((curWidth + plaqueWidth + 6) * UIFT_SCALE);
	m_mainMenu.plaque_pic.generic.y			= y = - (15 * UIFT_SCALE);
	uii.R_GetImageSize (uiMedia.menus.mainPlaque, NULL, &height);
	y += height * UIFT_SCALE;
	...
	// Render
	UI_CenterMenuHeight (&m_mainMenu.frameWork);
	UI_SetupMenuBounds (&m_mainMenu.frameWork);

	UI_AdjustCursor (&m_mainMenu.frameWork, 1);
	UI_Draw (&m_mainMenu.frameWork);
}
...
=============
*/
static struct sfx_s *MainMenu_Key (int key)
{
	return DefaultMenu_Key (&m_mainMenu.frameWork, key);
}
...
{
	if (UI_ActiveUI == &m_mainMenu.frameWork)
		return;

	MainMenu_Init ();
	UI_PushMenu (&m_mainMenu.frameWork, MainMenu_Draw, MainMenu_Close, MainMenu_Key);
}
