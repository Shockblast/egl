...
qBool UI_ParseServerStatus (char *adr, char *info)
{
	...
	// Add net address
	server->netAddress = UI_StrDup (adr);
	server->mapName = UI_StrDup (Info_ValueForKey (info, "mapname"));
	server->maxPlayers = atoi (Info_ValueForKey (info, "maxclients"));
	server->gameName = UI_StrDup (Info_ValueForKey (info, "gamename"));
	server->hostName = UI_StrDup (Info_ValueForKey (info, "hostname"));
	if (server->hostName) {
...
static void JoinServerFunc (void *used)
{
	...
	if (ui_selectedItem)
		index = (uiAction_t *)ui_selectedItem - m_joinServerMenu.hostNames;
	else
...
//		uii.Cbuf_AddText ("pinglocal\n");	// send out info packet request
	...
	case JS_PAGE_ADDRBOOK:
		...
		uii.Cbuf_AddText ("ui_startSStatus\n");
...
static void JoinServerMenu_Init (qBool sort)
{
	...
	UI_StartFramework (&m_joinServerMenu.frameWork);
...
static void JoinServerMenu_Draw (void)
{
	...
	m_joinServerMenu.frameWork.x			= uiState.vidWidth * 0.5f;
	...
	// Render
	UI_CenterMenuHeight (&m_joinServerMenu.frameWork);
	UI_SetupMenuBounds (&m_joinServerMenu.frameWork);

	UI_AdjustCursor (&m_joinServerMenu.frameWork, 1);
	UI_Draw (&m_joinServerMenu.frameWork);
}
...
=============
*/
static struct sfx_s *JoinServerMenu_Key (int key)
{
	return DefaultMenu_Key (&m_joinServerMenu.frameWork, key);
}
...
{
	SearchLocalGamesFunc (NULL);
	UI_PushMenu (&m_joinServerMenu.frameWork, JoinServerMenu_Draw, JoinServerMenu_Close, JoinServerMenu_Key);
}
