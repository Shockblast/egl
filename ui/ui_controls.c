...
static void KeyCursorDrawFunc (uiFrameWork_t *menu)
{
	...
	cursor = (ui_cursorLock) ? '=' : ((int)(uiState.time/250)&1) + 12;
...
{
	// Clear items
	UI_StartFramework (&m_controlsMenu.frameWork);
...
static void ControlsMenu_Draw (void)
{
	...
	m_controlsMenu.frameWork.x				= uiState.vidWidth * 0.5f;
	...
	// Render
	UI_CenterMenuHeight (&m_controlsMenu.frameWork);
	UI_SetupMenuBounds (&m_controlsMenu.frameWork);

	UI_AdjustCursor (&m_controlsMenu.frameWork, 1);
	UI_Draw (&m_controlsMenu.frameWork);
}
...
static struct sfx_s *ControlsMenu_Key (int key)
{
	...
	uiAction_t *item = (uiAction_t *) UI_ItemAtCursor (&m_controlsMenu.frameWork);
	...
	if (ui_cursorLock) {
		...
		default:
			Q_snprintfz (cmd, sizeof (cmd), "bind \"%s\" \"%s\"\n", uii.Key_KeynumToString(key), m_bindNames[item->generic.localData[0]][0]);
	...
	return DefaultMenu_Key (&m_controlsMenu.frameWork, key);
}
...
void UI_ControlsMenu_f (void)
{
	...
	m_controlsMenu.frameWork.statusBar = "[ENTER] to change, [BACKSPACE] to clear";
	UI_PushMenu (&m_controlsMenu.frameWork, ControlsMenu_Draw, ControlsMenu_Close, ControlsMenu_Key);
}
