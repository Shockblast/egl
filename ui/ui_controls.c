...
static void KeyCursorDrawFunc (uiFrameWork_t *menu) {
	...
	cursor = (ui_CursorLock) ? '=' : ((int)(uii.Sys_Milliseconds()/250)&1) + 12;

	item = UI_ItemAtCursor (menu);
	if (item->flags & UIF_LEFT_JUSTIFY)
		UI_DrawChar (menu->x + item->x + LCOLUMN_OFFSET + item->cursor_offset, menu->y + item->y, 0, UISCALE_TYPE (item->flags), cursor, colorWhite);
	else if (item->flags & UIF_CENTERED)
		UI_DrawChar (menu->x + item->x - (((Q_ColorCharCount (item->name, strlen (item->name)) + 4) * UISIZE_TYPE (item->flags))*0.5) + item->cursor_offset, menu->y + item->y, 0, UISCALE_TYPE (item->flags), cursor, colorWhite);
	else
		UI_DrawChar (menu->x + item->x + item->cursor_offset, menu->y + item->y, 0, UISCALE_TYPE (item->flags), cursor, colorWhite);
}
...
static void DrawKeyBindingFunc (void *self) {
	...
	if (keys[0] == -1) {
		UI_DrawString (a->generic.x + a->generic.parent->x + (UISIZE_TYPE (a->generic.flags)*2), a->generic.y + a->generic.parent->y,
			0, UISCALE_TYPE (a->generic.flags), "???", color);
		...
		UI_DrawString (a->generic.x + a->generic.parent->x + (UISIZE_TYPE (a->generic.flags)*2), a->generic.y + a->generic.parent->y,
			0, UISCALE_TYPE (a->generic.flags), name, color);
...
static void KeyBindingFunc (void *self) {
	...
	UI_SetStatusBar (&m_controls_menu.framework, "Press a key or button for this action");
}
...
static void ControlsMenu_Init (void) {
	...
	m_controls_menu.framework.nitems		= 0;
...
=============
*/
static const char *ControlsMenu_Key (int key) {
