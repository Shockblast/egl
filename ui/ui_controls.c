...
static void KeyCursorDrawFunc (uiFrameWork_t *menu) {
	...
	cursor = (uiCursorLock) ? '=' : ((int)(uis.time/250)&1) + 12;
	...
	if (item->flags & UIF_LEFT_JUSTIFY)
		...
		UI_DrawChar (menu->x + item->x - (((Q_ColorCharCount (item->name, Q_strlen (item->name)) + 4) * UISIZE_TYPE (item->flags))*0.5) + item->cursorOffset, menu->y + item->y, 0, UISCALE_TYPE (item->flags), cursor, colorWhite);
	else
...
static void DrawKeyBindingFunc (void *self) {
	...
	if (keys[0] == -1) {
		...
		x = Q_ColorCharCount (name, Q_strlen (name)) * UISIZE_TYPE (a->generic.flags);
		...
		if (keys[1] != -1) {
			...
			a->generic.br[0] = a->generic.x + a->generic.parent->x + (Q_strlen (uii.Key_KeynumToString (keys[1])) * UISIZE_TYPE (a->generic.flags)) + (UISIZE_TYPE (a->generic.flags)*6) + x;
			a->generic.br[1] = a->generic.y + a->generic.parent->y + UISIZE_TYPE (a->generic.flags);
		}
	}
}
...
static void ControlsMenu_Init (void) {
	...
	UI_Banner (&m_controls_menu.banner, uiMedia.banners.options, &y);
	...
	m_controls_menu.back_action.generic.callBack	= Menu_Pop;
