...
typedef struct {
	...
	uiField_t		fields[MAX_LOCAL_SERVERS];
...
void AddressBook_MenuInit (void) {
	...
	m_addrbook_menu.framework.nitems	= 0;

	UI_Banner (&m_addrbook_menu.banner, uiMedia.addressBookBanner, &y);
	...
	for (i=0 ; i<MAX_LOCAL_SERVERS ; i++) {
		...
		m_addrbook_menu.fields[i].visible_length		= 30;

		strcpy (m_addrbook_menu.fields[i].buffer, adr->string);
