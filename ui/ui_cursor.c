...
void UI_DrawMouseCursor (void) {
	...
	UI_DrawPic (uiMedia.cursorImage, uis.cursorX, uis.cursorY,
				width * clamp (UIFT_SCALE, 0.5, 1) * uiCursorScale,
				height * clamp (UIFT_SCALE, 0.5, 1) * uiCursorScale,
				0, 0, 1, 1, colorWhite);
}
...
void UI_UpdateMousePos (void) {
	...
	if (uiMouseItem)
		uiCursorScale = CURSOR_ONITEMSCALE;
	else
		uiCursorScale = 1;
}
...
void UI_MoveMouse (int x, int y) {
	...
	uis.cursorX = clamp (uis.cursorX += (x * ui_sensitivity->value), 2, uis.vidWidth - 2);
	uis.cursorY = clamp (uis.cursorY += (y * ui_sensitivity->value), 2, uis.vidHeight - 2);
...
void Action_Setup (uiAction_t *s) {
	...
	if (s->generic.flags & UIF_CENTERED)
		s->generic.tl[0] -= ((Q_ColorCharCount (s->generic.name, Q_strlen (s->generic.name)) * UISIZE_TYPE (s->generic.flags))*0.5);
	else if (!(s->generic.flags & UIF_LEFT_JUSTIFY))
		s->generic.tl[0] += LCOLUMN_OFFSET - ((Q_ColorCharCount (s->generic.name, Q_strlen (s->generic.name))) * UISIZE_TYPE (s->generic.flags));

	s->generic.br[0] = s->generic.tl[0] + (Q_ColorCharCount (s->generic.name, Q_strlen (s->generic.name)) * UISIZE_TYPE (s->generic.flags));
}
...
void Field_Setup (uiField_t *s) {
	...
	if (s->generic.name)
		s->generic.tl[0] -= (Q_ColorCharCount (s->generic.name, Q_strlen (s->generic.name)) + 1) * UISIZE_TYPE (s->generic.flags);
...
void Slider_Setup (uiSlider_t *s) {
	...
	if (s->generic.name)
		offset = Q_ColorCharCount (s->generic.name, Q_strlen (s->generic.name)) * UISIZE_TYPE (s->generic.flags);
	else
		offset = 0;
...
void SpinControl_Setup (uiList_t *s) {
	...
	ysize = UISIZE_TYPE (s->generic.flags);
	if (s->generic.name) {
		s->generic.tl[0] = s->generic.x + s->generic.parent->x - (Q_ColorCharCount (s->generic.name, Q_strlen (s->generic.name)) * UISIZE_TYPE (s->generic.flags)) - UISIZE_TYPE (s->generic.flags);
		s->generic.tl[1] = s->generic.y + s->generic.parent->y;

		xsize = (Q_ColorCharCount (s->generic.name, Q_strlen (s->generic.name)) * UISIZE_TYPE (s->generic.flags)) + UISIZE_TYPE (s->generic.flags)*3;
	}
	...
	if (s->itemNames[s->curValue])
		xsize += (Q_ColorCharCount (s->itemNames[s->curValue], Q_strlen (s->itemNames[s->curValue])) * UISIZE_TYPE (s->generic.flags));
