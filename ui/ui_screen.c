...
{
	uii.Cvar_SetValue ("con_clock", m_screenMenu.con_clock_toggle.curValue);
}

static void ConsoleNotFadeFunc (void *unused)
{
	uii.Cvar_SetValue ("con_notifyfade", m_screenMenu.con_notfade_toggle.curValue);
}

static void ConsoleNotLargeFunc (void *unused)
{
	uii.Cvar_SetValue ("con_notifylarge", m_screenMenu.con_notlarge_toggle.curValue);
}

static void ConsoleTimeStampFunc (void *unused)
{
	uii.Cvar_SetValue ("cl_timestamp", m_screenMenu.con_timestamp_list.curValue);
}

static void ConsoleNotLinesFunc (void *unused)
{
	uii.Cvar_SetValue ("con_notifylines", m_screenMenu.con_notlines_slider.curValue);
	m_screenMenu.con_notlines_amount.generic.name = uii.Cvar_VariableString ("con_notifylines");
}

static void ConsoleAlphaFunc (void *unused)
{
	uii.Cvar_SetValue ("con_alpha", m_screenMenu.con_alpha_slider.curValue * 0.1);
	m_screenMenu.con_alpha_amount.generic.name = uii.Cvar_VariableString ("con_alpha");
}

static void ConsoleDropFunc (void *unused)
{
	uii.Cvar_SetValue ("con_drop", m_screenMenu.con_drop_slider.curValue * 0.1);
	m_screenMenu.con_drop_amount.generic.name = uii.Cvar_VariableString ("con_drop");
}

static void ConsoleScrollFunc (void *unused)
{
	uii.Cvar_SetValue ("con_scroll", m_screenMenu.con_scroll_slider.curValue);
	m_screenMenu.con_scroll_amount.generic.name = uii.Cvar_VariableString ("con_scroll");
}

static void ConsoleFontScaleFunc (void *unused)
{
	uii.Cvar_SetValue ("r_fontscale", m_screenMenu.con_fontscale_slider.curValue * 0.25);
	m_screenMenu.con_fontscale_amount.generic.name = uii.Cvar_VariableString ("r_fontscale");
}
...
static void CrosshairFunc (void *unused)
{
	if (m_screenMenu.ch_number_list.curValue < m_screenMenu.numCrosshairs)
		uii.Cvar_SetValue ("crosshair", m_screenMenu.crosshairs[m_screenMenu.ch_number_list.curValue].number);
}

static void CrosshairAlphaFunc (void *unused)
{
	uii.Cvar_SetValue ("ch_alpha", m_screenMenu.ch_alpha_slider.curValue * 0.1);
	m_screenMenu.ch_alpha_amount.generic.name = uii.Cvar_VariableString ("ch_alpha");
}

static void CrosshairPulseFunc (void *unused)
{
	uii.Cvar_SetValue ("ch_pulse", m_screenMenu.ch_pulse_slider.curValue * 0.1);
	m_screenMenu.ch_pulse_amount.generic.name = uii.Cvar_VariableString ("ch_pulse");
}

static void CrosshairScaleFunc (void *unused)
{
	uii.Cvar_SetValue ("ch_scale", m_screenMenu.ch_scale_slider.curValue * 0.1);
	m_screenMenu.ch_scale_amount.generic.name = uii.Cvar_VariableString ("ch_scale");
}

static void CrosshairRedFunc (void *unused)
{
	uii.Cvar_SetValue ("ch_red", m_screenMenu.ch_red_slider.curValue * 0.1);
	m_screenMenu.ch_red_amount.generic.name = uii.Cvar_VariableString ("ch_red");
}

static void CrosshairGreenFunc (void *unused)
{
	uii.Cvar_SetValue ("ch_green", m_screenMenu.ch_green_slider.curValue * 0.1);
	m_screenMenu.ch_green_amount.generic.name = uii.Cvar_VariableString ("ch_green");
}

static void CrosshairBlueFunc (void *unused)
{
	uii.Cvar_SetValue ("ch_blue", m_screenMenu.ch_blue_slider.curValue * 0.1);
	m_screenMenu.ch_blue_amount.generic.name = uii.Cvar_VariableString ("ch_blue");
}
...
static void ScreenMenu_SetValues (void)
{
	...
	m_screenMenu.con_clock_toggle.curValue			= uii.Cvar_VariableInteger ("con_clock");

	uii.Cvar_SetValue ("con_notifyfade",			clamp (uii.Cvar_VariableInteger ("con_notifyfade"), 0, 1));
	m_screenMenu.con_notfade_toggle.curValue		= uii.Cvar_VariableInteger ("con_notifyfade");

	uii.Cvar_SetValue ("con_notifylarge",			clamp (uii.Cvar_VariableInteger ("con_notifylarge"), 0, 1));
	m_screenMenu.con_notlarge_toggle.curValue		= uii.Cvar_VariableInteger ("con_notifylarge");

	uii.Cvar_SetValue ("cl_timestamp",				clamp (uii.Cvar_VariableInteger ("cl_timestamp"), 0, 2));
	m_screenMenu.con_timestamp_list.curValue		= uii.Cvar_VariableInteger ("cl_timestamp");

	m_screenMenu.con_notlines_slider.curValue		= uii.Cvar_VariableValue ("con_notifylines");
	m_screenMenu.con_notlines_amount.generic.name	= uii.Cvar_VariableString ("con_notifylines");
	m_screenMenu.con_alpha_slider.curValue			= uii.Cvar_VariableValue ("con_alpha") * 10;
	m_screenMenu.con_alpha_amount.generic.name		= uii.Cvar_VariableString ("con_alpha");
	m_screenMenu.con_drop_slider.curValue			= uii.Cvar_VariableValue ("con_drop") * 10;
	m_screenMenu.con_drop_amount.generic.name		= uii.Cvar_VariableString ("con_drop");
	m_screenMenu.con_scroll_slider.curValue			= uii.Cvar_VariableValue ("con_scroll");
	m_screenMenu.con_scroll_amount.generic.name		= uii.Cvar_VariableString ("con_scroll");

	m_screenMenu.con_fontscale_slider.curValue		= uii.Cvar_VariableValue ("r_fontscale") * 4;
	m_screenMenu.con_fontscale_amount.generic.name	= uii.Cvar_VariableString ("r_fontscale");
	...
	m_screenMenu.ch_number_list.curValue		= uii.Cvar_VariableValue ("crosshair");
	...
	if (m_screenMenu.numCrosshairs)
		uii.Cvar_ForceSetValue ("crosshair", m_screenMenu.crosshairs[m_screenMenu.ch_number_list.curValue].number);
	else
		uii.Cvar_ForceSetValue ("crosshair", 0);

	m_screenMenu.ch_alpha_slider.curValue		= uii.Cvar_VariableValue ("ch_alpha") * 10;
	m_screenMenu.ch_alpha_amount.generic.name	= uii.Cvar_VariableString ("ch_alpha");
	m_screenMenu.ch_pulse_slider.curValue		= uii.Cvar_VariableValue ("ch_pulse") * 10;
	m_screenMenu.ch_pulse_amount.generic.name	= uii.Cvar_VariableString ("ch_pulse");
	m_screenMenu.ch_scale_slider.curValue		= uii.Cvar_VariableValue ("ch_scale")* 10;
	m_screenMenu.ch_scale_amount.generic.name	= uii.Cvar_VariableString ("ch_scale");

	m_screenMenu.ch_red_slider.curValue			= uii.Cvar_VariableValue ("ch_red") * 10;
	m_screenMenu.ch_red_amount.generic.name		= uii.Cvar_VariableString ("ch_red");
	m_screenMenu.ch_green_slider.curValue		= uii.Cvar_VariableValue ("ch_green") * 10;
	m_screenMenu.ch_green_amount.generic.name	= uii.Cvar_VariableString ("ch_green");
	m_screenMenu.ch_blue_slider.curValue		= uii.Cvar_VariableValue ("ch_blue") * 10;
	m_screenMenu.ch_blue_amount.generic.name	= uii.Cvar_VariableString ("ch_blue");
}
...
=============
*/
static int stricmpSort (const char **a, const char **b)
{
	char	ch1[MAX_QPATH];
	char	ch2[MAX_QPATH];
	int		chNum1;
...
static void ScreenMenu_Init (void)
{
	...
	// get crosshair list
	numCrosshairs = uii.FS_FindFiles ("pics", "pics/ch*.*", NULL, crosshairList, MAX_CROSSHAIRS, qFalse, qFalse);
	if (!numCrosshairs)
	...
	if (m_screenMenu.crosshairsFound) {
		...
		for (i=0 ; i<numCrosshairs ; i++) {
			...
			m_screenMenu.crosshairs[m_screenMenu.numCrosshairs].shader = uii.R_RegisterPic (crosshairList[i]);
			m_screenMenu.crosshairNames[m_screenMenu.numCrosshairs] = UI_StrDup (crosshairList[i]);
	...
	UI_FS_FreeFileList (crosshairList, numCrosshairs);

	// create menu
	UI_StartFramework (&m_screenMenu.frameWork);
...
static struct sfx_s *ScreenMenu_Close (void)
{
	...
	// Free display names
	for (i=0 ; i<m_screenMenu.numCrosshairs ; i++) {
		UI_MemFree (m_screenMenu.crosshairNames[i]);
	}
...
static void ScreenMenu_Draw (void)
{
	...
	m_screenMenu.frameWork.x				= uiState.vidWidth * 0.5f;
	...
	if (m_screenMenu.crosshairsFound && m_screenMenu.ch_number_list.curValue < m_screenMenu.numCrosshairs) {
		...
		uii.R_GetImageSize (chShader, &width, &height);
	}
	else {
		chShader = uiMedia.whiteTexture;
		width = 32;
		height = 32;
	}
	...
	// Render
	UI_CenterMenuHeight (&m_screenMenu.frameWork);
	UI_SetupMenuBounds (&m_screenMenu.frameWork);

	UI_AdjustCursor (&m_screenMenu.frameWork, 1);
	UI_Draw (&m_screenMenu.frameWork);
}
...
=============
*/
static struct sfx_s *ScreenMenu_Key (int key)
{
	return DefaultMenu_Key (&m_screenMenu.frameWork, key);
}
...
{
	ScreenMenu_Init ();
	UI_PushMenu (&m_screenMenu.frameWork, ScreenMenu_Draw, ScreenMenu_Close, ScreenMenu_Key);
}
