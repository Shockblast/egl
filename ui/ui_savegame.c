...
static void SaveGameMenu_Init (void)
{
	...
	UI_StartFramework (&m_saveGameMenu.frameWork);
...
static void SaveGameMenu_Draw (void)
{
	...
	m_saveGameMenu.frameWork.x			= uiState.vidWidth * 0.5f;
	...
	UI_CenterMenuHeight (&m_saveGameMenu.frameWork);
	UI_SetupMenuBounds (&m_saveGameMenu.frameWork);

	UI_AdjustCursor (&m_saveGameMenu.frameWork, 1);
	UI_Draw (&m_saveGameMenu.frameWork);
}
...
=============
*/
static struct sfx_s *SaveGameMenu_Key (int key)
{
	return DefaultMenu_Key (&m_saveGameMenu.frameWork, key);
}
...
void UI_SaveGameMenu_f (void)
{
	...
	SaveGameMenu_Init ();
	UI_PushMenu (&m_saveGameMenu.frameWork, SaveGameMenu_Draw, SaveGameMenu_Close, SaveGameMenu_Key);
	Create_Savestrings ();
}
