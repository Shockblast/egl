...
void SaveGameMenu_Init (void) {
	...
	m_savegame_menu.framework.nitems	= 0;

	Create_Savestrings ();

	UI_Banner (&m_savegame_menu.banner, uiMedia.saveGameBanner, &y);
	...
	for (i=0 ; i<MAX_SAVEGAMES-1 ; i++) {
		...
		m_savegame_menu.actions[i].generic.name			= ui_SaveStrings[i+1];
	...
	m_savegame_menu.back_action.generic.callback	= Back_Menu;
...
=============
*/
const char *SaveGameMenu_Key (int key) {
