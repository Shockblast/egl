...
static void VIDSettingsMenu_ApplyValues (void *unused)
{
	...
	uii.Cvar_ForceSetValue ("gl_mode",			m_vidSettingsMenu.mode_list.curValue);

	if (m_vidSettingsMenu.tm_list.curValue == 1)		uii.Cvar_ForceSet ("gl_texturemode", "GL_LINEAR_MIPMAP_LINEAR");
	else if (m_vidSettingsMenu.tm_list.curValue == 0)	uii.Cvar_ForceSet ("gl_texturemode", "GL_LINEAR_MIPMAP_NEAREST");

	if (m_vidSettingsMenu.tb_list.curValue == 2)		uii.Cvar_ForceSet ("gl_texturebits", "32");
	else if (m_vidSettingsMenu.tb_list.curValue == 1)	uii.Cvar_ForceSet ("gl_texturebits", "16");
	else												uii.Cvar_ForceSet ("gl_texturebits", "default");

	if (m_vidSettingsMenu.bitd_list.curValue == 0)		uii.Cvar_ForceSet ("gl_bitdepth", "0");
	else if (m_vidSettingsMenu.bitd_list.curValue == 1)	uii.Cvar_ForceSet ("gl_bitdepth", "16");
	else if (m_vidSettingsMenu.bitd_list.curValue == 2)	uii.Cvar_ForceSet ("gl_bitdepth", "32");

	uii.Cvar_ForceSetValue ("gl_stencilbuffer",		m_vidSettingsMenu.stencilbuf_list.curValue);

	if (m_vidSettingsMenu.dispfreq_list.curValue == 1)			uii.Cvar_ForceSet ("r_displayFreq", "60");
	else if (m_vidSettingsMenu.dispfreq_list.curValue == 2)	uii.Cvar_ForceSet ("r_displayFreq", "65");
	else if (m_vidSettingsMenu.dispfreq_list.curValue == 3)	uii.Cvar_ForceSet ("r_displayFreq", "70");
	else if (m_vidSettingsMenu.dispfreq_list.curValue == 4)	uii.Cvar_ForceSet ("r_displayFreq", "75");
	else if (m_vidSettingsMenu.dispfreq_list.curValue == 5)	uii.Cvar_ForceSet ("r_displayFreq", "80");
	else if (m_vidSettingsMenu.dispfreq_list.curValue == 6)	uii.Cvar_ForceSet ("r_displayFreq", "85");
	else if (m_vidSettingsMenu.dispfreq_list.curValue == 7)	uii.Cvar_ForceSet ("r_displayFreq", "90");
	else if (m_vidSettingsMenu.dispfreq_list.curValue == 8)	uii.Cvar_ForceSet ("r_displayFreq", "95");
	else if (m_vidSettingsMenu.dispfreq_list.curValue == 9)	uii.Cvar_ForceSet ("r_displayFreq", "100");
	else if (m_vidSettingsMenu.dispfreq_list.curValue == 10)	uii.Cvar_ForceSet ("r_displayFreq", "105");
	else if (m_vidSettingsMenu.dispfreq_list.curValue == 11)	uii.Cvar_ForceSet ("r_displayFreq", "110");
	else if (m_vidSettingsMenu.dispfreq_list.curValue == 12)	uii.Cvar_ForceSet ("r_displayFreq", "115");
	else if (m_vidSettingsMenu.dispfreq_list.curValue == 13)	uii.Cvar_ForceSet ("r_displayFreq", "120");
	else														uii.Cvar_ForceSet ("r_displayFreq", "0");

	uii.Cvar_ForceSetValue ("vid_fullscreen",	m_vidSettingsMenu.fullscreen_toggle.curValue);
	uii.Cvar_ForceSetValue ("viewsize",			m_vidSettingsMenu.screensize_slider.curValue * 10);
	...
	*/
	...
	uii.Cvar_ForceSetValue ("vid_gammapics",	m_vidSettingsMenu.gammapics_toggle.curValue);

	uii.Cvar_ForceSetValue ("gl_picmip",		3 - m_vidSettingsMenu.texqual_slider.curValue);
	uii.Cvar_ForceSetValue ("gl_finish",		m_vidSettingsMenu.finish_toggle.curValue);
...
{
	if (Q_stricmp (uii.Cvar_VariableString ("gl_driver"), "3dfxgl") == 0)		m_vidSettingsMenu.ref_list.curValue = REF_3DFX;
	else if (Q_stricmp (uii.Cvar_VariableString ("gl_driver"), "pvrgl") == 0)	m_vidSettingsMenu.ref_list.curValue = REF_POWERVR;
	...
	m_vidSettingsMenu.mode_list.curValue	= uii.Cvar_VariableInteger ("gl_mode");

	if (!Q_stricmp (uii.Cvar_VariableString ("gl_texturemode"), "GL_LINEAR_MIPMAP_LINEAR"))			m_vidSettingsMenu.tm_list.curValue = 1;
	else if (!Q_stricmp (uii.Cvar_VariableString ("gl_texturemode"), "GL_LINEAR_MIPMAP_NEAREST"))	m_vidSettingsMenu.tm_list.curValue = 0;
	else																							m_vidSettingsMenu.tm_list.curValue = 2;

	if (uii.Cvar_VariableValue ("gl_texturebits") == 32)		m_vidSettingsMenu.tb_list.curValue = 2;
	else if (uii.Cvar_VariableValue ("gl_texturebits") == 16)	m_vidSettingsMenu.tb_list.curValue = 1;
	else														m_vidSettingsMenu.tb_list.curValue = 0;

	if (uii.Cvar_VariableValue ("gl_bitdepth") == 0)		m_vidSettingsMenu.bitd_list.curValue = 0;
	else if (uii.Cvar_VariableValue ("gl_bitdepth") == 16)	m_vidSettingsMenu.bitd_list.curValue = 1;
	else if (uii.Cvar_VariableValue ("gl_bitdepth") == 32)	m_vidSettingsMenu.bitd_list.curValue = 2;

	uii.Cvar_ForceSetValue ("gl_stencilbuffer",		clamp (uii.Cvar_VariableInteger ("gl_stencilbuffer"), 0, 1));
	m_vidSettingsMenu.stencilbuf_list.curValue	= uii.Cvar_VariableInteger ("gl_stencilbuffer");

	if (uii.Cvar_VariableValue ("r_displayFreq") == 60)			m_vidSettingsMenu.dispfreq_list.curValue = 1;
	else if (uii.Cvar_VariableValue ("r_displayFreq") == 65)	m_vidSettingsMenu.dispfreq_list.curValue = 2;
	else if (uii.Cvar_VariableValue ("r_displayFreq") == 70)	m_vidSettingsMenu.dispfreq_list.curValue = 3;
	else if (uii.Cvar_VariableValue ("r_displayFreq") == 75)	m_vidSettingsMenu.dispfreq_list.curValue = 4;
	else if (uii.Cvar_VariableValue ("r_displayFreq") == 80)	m_vidSettingsMenu.dispfreq_list.curValue = 5;
	else if (uii.Cvar_VariableValue ("r_displayFreq") == 85)	m_vidSettingsMenu.dispfreq_list.curValue = 6;
	else if (uii.Cvar_VariableValue ("r_displayFreq") == 90)	m_vidSettingsMenu.dispfreq_list.curValue = 7;
	else if (uii.Cvar_VariableValue ("r_displayFreq") == 95)	m_vidSettingsMenu.dispfreq_list.curValue = 8;
	else if (uii.Cvar_VariableValue ("r_displayFreq") == 100)	m_vidSettingsMenu.dispfreq_list.curValue = 9;
	else if (uii.Cvar_VariableValue ("r_displayFreq") == 105)	m_vidSettingsMenu.dispfreq_list.curValue = 10;
	else if (uii.Cvar_VariableValue ("r_displayFreq") == 110)	m_vidSettingsMenu.dispfreq_list.curValue = 11;
	else if (uii.Cvar_VariableValue ("r_displayFreq") == 115)	m_vidSettingsMenu.dispfreq_list.curValue = 12;
	else if (uii.Cvar_VariableValue ("r_displayFreq") == 120)	m_vidSettingsMenu.dispfreq_list.curValue = 13;
	...
	m_vidSettingsMenu.fullscreen_toggle.curValue	= uii.Cvar_VariableInteger ("vid_fullscreen");

	uii.Cvar_ForceSetValue ("viewsize",				clamp (uii.Cvar_VariableInteger ("viewsize"), 40, 100));
	m_vidSettingsMenu.screensize_slider.curValue	= uii.Cvar_VariableInteger ("viewsize") * 0.1; // kthx view menu?

	uii.Cvar_ForceSetValue ("vid_gamma",			clamp (uii.Cvar_VariableValue ("vid_gamma"), 0.5, 1.3));
	m_vidSettingsMenu.brightness_slider.curValue	= (1.3 - uii.Cvar_VariableValue ("vid_gamma") + 0.5) * 10;

	uii.Cvar_ForceSetValue ("vid_gammapics",		clamp (uii.Cvar_VariableInteger ("vid_gammapics"), 0, 1));
	m_vidSettingsMenu.gammapics_toggle.curValue	= uii.Cvar_VariableInteger ("vid_gammapics");

	m_vidSettingsMenu.texqual_slider.curValue		= 3 - uii.Cvar_VariableInteger ("gl_picmip");

	uii.Cvar_ForceSetValue ("gl_finish",		clamp (uii.Cvar_VariableInteger ("gl_finish"), 0, 1));
	m_vidSettingsMenu.finish_toggle.curValue	= uii.Cvar_VariableInteger ("gl_finish");
}
...
static void VIDSettingsMenu_Init (void)
{
	...
	UI_StartFramework (&m_vidSettingsMenu.frameWork);
	...
	m_vidSettingsMenu.tb_list.generic.type		= UITYPE_SPINCONTROL;
	m_vidSettingsMenu.tb_list.generic.name		= "Texture Bits";
	m_vidSettingsMenu.tb_list.itemNames			= tb_names;
	m_vidSettingsMenu.tb_list.generic.statusBar	= "Texture bits";
	...
	UI_AddItem (&m_vidSettingsMenu.frameWork,		&m_vidSettingsMenu.tb_list);
...
static void VIDSettingsMenu_Draw (void)
{
	...
	m_vidSettingsMenu.frameWork.x			= uiState.vidWidth * 0.5f;
	...
	m_vidSettingsMenu.tb_list.generic.x				= 0;
	m_vidSettingsMenu.tb_list.generic.y				= y += UIFT_SIZEINC;
	...
	// Render
	UI_CenterMenuHeight (&m_vidSettingsMenu.frameWork);
	UI_SetupMenuBounds (&m_vidSettingsMenu.frameWork);

	UI_AdjustCursor (&m_vidSettingsMenu.frameWork, 1);
	UI_Draw (&m_vidSettingsMenu.frameWork);
}
...
================
*/
static struct sfx_s *VIDSettingsMenu_Key (int key)
{
	return DefaultMenu_Key (&m_vidSettingsMenu.frameWork, key);
}
...
{
	VIDSettingsMenu_Init ();
	UI_PushMenu (&m_vidSettingsMenu.frameWork, VIDSettingsMenu_Draw, VIDSettingsMenu_Close, VIDSettingsMenu_Key);
}
