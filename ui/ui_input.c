...
{
	uii.Cvar_SetValue ("cl_run", m_inputMenu.always_run_toggle.curValue);
}

static void JoystickFunc (void *unused)
{
	uii.Cvar_SetValue ("in_joystick", m_inputMenu.joystick_toggle.curValue);
}

static void UISensFunc (void *unused)
{
	uii.Cvar_SetValue ("ui_sensitivity", m_inputMenu.ui_sensitivity_slider.curValue / 2.0F);
	m_inputMenu.ui_sensitivity_amount.generic.name = uii.Cvar_VariableString ("ui_sensitivity");
}

static void SensitivityFunc (void *unused)
{
	uii.Cvar_SetValue ("sensitivity", m_inputMenu.sensitivity_slider.curValue / 2.0F);
	m_inputMenu.sensitivity_amount.generic.name = uii.Cvar_VariableString ("sensitivity");
}

static void MouseAccelFunc (void *unused)
{
	uii.Cvar_SetValue ("m_accel", m_inputMenu.maccel_list.curValue);
}

static void InvertMouseFunc (void *unused)
{
	uii.Cvar_SetValue ("m_pitch", (uii.Cvar_VariableValue ("m_pitch")) * -1);
	m_inputMenu.invert_mouse_toggle.curValue		= (!!(uii.Cvar_VariableValue ("m_pitch") < 0));
}

static void AutoSensFunc (void *unused)
{
	uii.Cvar_SetValue ("autosensitivity", m_inputMenu.autosensitivity_toggle.curValue);
}

static void LookspringFunc (void *unused)
{
	uii.Cvar_SetValue ("lookspring", m_inputMenu.lookspring_toggle.curValue);
}

static void LookstrafeFunc (void *unused)
{
	uii.Cvar_SetValue ("lookstrafe", m_inputMenu.lookstrafe_toggle.curValue);
}

static void FreeLookFunc (void *unused)
{
	uii.Cvar_SetValue ("freelook", m_inputMenu.freelook_toggle.curValue);
}
...
static void InputMenu_SetValues (void)
{
	...
	m_inputMenu.always_run_toggle.curValue		= uii.Cvar_VariableInteger ("cl_run");

	uii.Cvar_SetValue ("in_joystick",			clamp (uii.Cvar_VariableInteger ("in_joystick"), 0, 1));
	m_inputMenu.joystick_toggle.curValue		= uii.Cvar_VariableInteger ("in_joystick");

	m_inputMenu.ui_sensitivity_slider.curValue		= (uii.Cvar_VariableValue ("ui_sensitivity")) * 2;
	m_inputMenu.ui_sensitivity_amount.generic.name = uii.Cvar_VariableString ("ui_sensitivity");
	m_inputMenu.sensitivity_slider.curValue		= (uii.Cvar_VariableValue ("sensitivity")) * 2;
	m_inputMenu.sensitivity_amount.generic.name	= uii.Cvar_VariableString ("sensitivity");

	uii.Cvar_SetValue ("m_accel",					clamp (uii.Cvar_VariableInteger ("m_accel"), 0, 2));
	m_inputMenu.maccel_list.curValue				= uii.Cvar_VariableInteger ("m_accel");

	m_inputMenu.invert_mouse_toggle.curValue		= (!!(uii.Cvar_VariableValue ("m_pitch") < 0));

	uii.Cvar_SetValue ("autosensitivity",			clamp (uii.Cvar_VariableInteger ("autosensitivity"), 0, 1));
	m_inputMenu.autosensitivity_toggle.curValue	= uii.Cvar_VariableInteger ("autosensitivity");

	uii.Cvar_SetValue ("lookspring",			clamp (uii.Cvar_VariableInteger ("lookspring"), 0, 1));
	m_inputMenu.lookspring_toggle.curValue		= uii.Cvar_VariableInteger ("lookspring");

	uii.Cvar_SetValue ("lookstrafe",			clamp (uii.Cvar_VariableInteger ("lookstrafe"), 0, 1));
	m_inputMenu.lookstrafe_toggle.curValue		= uii.Cvar_VariableInteger ("lookstrafe");

	uii.Cvar_SetValue ("freelook",				clamp (uii.Cvar_VariableInteger ("freelook"), 0, 1));
	m_inputMenu.freelook_toggle.curValue		= uii.Cvar_VariableInteger ("freelook");
}
...
static void InputMenu_Init (void)
{
	...
	UI_StartFramework (&m_inputMenu.frameWork);
...
static void InputMenu_Draw (void)
{
	...
	m_inputMenu.frameWork.x		= uiState.vidWidth * 0.5f;
	...
	// Render
	UI_CenterMenuHeight (&m_inputMenu.frameWork);
	UI_SetupMenuBounds (&m_inputMenu.frameWork);

	UI_AdjustCursor (&m_inputMenu.frameWork, 1);
	UI_Draw (&m_inputMenu.frameWork);
}
...
=============
*/
static struct sfx_s *InputMenu_Key (int key)
{
	return DefaultMenu_Key (&m_inputMenu.frameWork, key);
}
...
{
	InputMenu_Init ();
	UI_PushMenu (&m_inputMenu.frameWork, InputMenu_Draw, InputMenu_Close, InputMenu_Key);
}
