...
{
	UI_StartFramework (&m_optionsMenu.frameWork);
...
static void OptionsMenu_Draw (void)
{
	...
	m_optionsMenu.frameWork.x			= uiState.vidWidth * 0.5f;
	...
	// Render
	UI_CenterMenuHeight (&m_optionsMenu.frameWork);
	UI_SetupMenuBounds (&m_optionsMenu.frameWork);

	UI_AdjustCursor (&m_optionsMenu.frameWork, 1);
	UI_Draw (&m_optionsMenu.frameWork);
}
...
=============
*/
static struct sfx_s *OptionsMenu_Key (int key)
{
	return DefaultMenu_Key (&m_optionsMenu.frameWork, key);
}
...
{
	OptionsMenu_Init ();
	UI_PushMenu (&m_optionsMenu.frameWork, OptionsMenu_Draw, OptionsMenu_Close, OptionsMenu_Key);
}
