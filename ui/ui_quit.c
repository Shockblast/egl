...
void QuitMenu_Init (void) {
	...
	m_quit_menu.framework.numItems	= 0;

	UI_Banner (&m_quit_menu.banner, uiMedia.banners.quit, &y);
	...
	m_quit_menu.no_action.generic.callBack		= Menu_Pop;
...
=============
*/
struct sfx_s *QuitMenu_Key (int key) {
	switch (key) {
	case 'n':
	case 'N':
		UI_PopMenu ();
