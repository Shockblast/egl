...
static qBool IconOfSkinExists (char *skin, char **pcxfiles, int npcxfiles) {
	...
	*strrchr (scratch, '.') = 0;
	Q_strcat (scratch, "_i.pcx", sizeof (scratch));
...
static void PlayerConfig_ScanDirectories (void) {
	...
	for (i=0 ; i<numPlayerModels ; i++) {
		...
		Q_strncpyz (scratch, directoryNames[i], sizeof (scratch));
		Q_strcat (scratch, "/tris.md2", sizeof (scratch));
		...
		Q_strncpyz (scratch, directoryNames[i], sizeof (scratch));
		Q_strcat (scratch, "/*.pcx", sizeof (scratch));
		...
		Q_strncpyz (s_pmi[s_numplayermodels].displayName, c + 1, sizeof (s_pmi[s_numplayermodels].displayName));
...
qBool PlayerConfigMenu_Init (void) {
	...
	UI_Banner (&m_plyrcfg_menu.banner, uiMedia.banners.multiplayer, &y);
	...
	Q_strncpyz (m_plyrcfg_menu.name_field.buffer, uii.Cvar_VariableString ("name"), sizeof (m_plyrcfg_menu.name_field.buffer));
	m_plyrcfg_menu.name_field.cursor = Q_strlen (uii.Cvar_VariableString ("name"));
	...
	m_plyrcfg_menu.back_action.generic.callBack		= Menu_Pop;
...
void PlayerConfigMenu_Draw (void) {
	...
	if (s_pmi[m_plyrcfg_menu.model_list.curValue].skinDisplayNames) {
		...
		entity[0].model = uii.Mod_RegisterModel (Q_VarArgs ("players/%s/tris.md2", s_pmi[m_plyrcfg_menu.model_list.curValue].directory));

		if (entity[0].model) {
			entity[0].skin = uii.Img_RegisterImage (Q_VarArgs ("players/%s/%s.pcx", s_pmi[m_plyrcfg_menu.model_list.curValue].directory, s_pmi[m_plyrcfg_menu.model_list.curValue].skinDisplayNames[m_plyrcfg_menu.skin_list.curValue]), 0);
		...
		entity[1].model = uii.Mod_RegisterModel (Q_VarArgs ("players/%s/weapon.md2", s_pmi[m_plyrcfg_menu.model_list.curValue].directory));
		...
		// pic selection
		icon = uii.Img_RegisterImage (Q_VarArgs ("/players/%s/%s_i.pcx", 
			s_pmi[m_plyrcfg_menu.model_list.curValue].directory,
			s_pmi[m_plyrcfg_menu.model_list.curValue].skinDisplayNames[m_plyrcfg_menu.skin_list.curValue]), IF_NOMIPMAP);
		if (icon) {
			int		width;
			int		height;
