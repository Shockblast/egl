...
static void EGLCredits (void *unused)
{
	...
	m_creditsMenu.startTime = uiState.time;
}
...
static void CreditsMenu_Init (void)
{
	...
	count = uii.FS_LoadFile ("credits", &m_creditsMenu.fileBuffer);

	// kthx problem with using credits selection and having a file? test me!
	if (m_creditsMenu.fileBuffer && (count != -1)) {
		...
		m_creditsMenu.creditsBuffer = m_creditsMenu.creditsIndex;
	}
	else {
	...
	UI_StartFramework (&m_creditsMenu.frameWork);
...
static struct sfx_s *CreditsMenu_Close (void)
{
	if (m_creditsMenu.fileBuffer)
		UI_FS_FreeFile (m_creditsMenu.fileBuffer);
...
static void CreditsMenu_Draw (void)
{
	...
	// Draw the credits
	for (i=0, y=uiState.vidHeight - ((uiState.time - m_creditsMenu.startTime) / 40.0F) ; m_creditsMenu.creditsBuffer[i] && (y < uiState.vidHeight) ; y += (UIFT_SIZEINCMED), i++) {
		if (y <= -UIFT_SIZEMED)
			continue;

		UI_DrawString ((uiState.vidWidth - (Q_ColorCharCount (m_creditsMenu.creditsBuffer[i], strlen (m_creditsMenu.creditsBuffer[i])) * UIFT_SIZEMED)) * 0.5,
			y, FS_SHADOW, UIFT_SCALEMED, (char *)m_creditsMenu.creditsBuffer[i], colorWhite);
	}
	...
	// Render
	UI_CenterMenuHeight (&m_creditsMenu.frameWork);
	UI_SetupMenuBounds (&m_creditsMenu.frameWork);

	UI_AdjustCursor (&m_creditsMenu.frameWork, 1);
	UI_Draw (&m_creditsMenu.frameWork);
}
...
=============
*/
static struct sfx_s *CreditsMenu_Key (int key)
{
	switch (key) {
	case K_ESCAPE:
		if (m_creditsMenu.fileBuffer)
	...
	return DefaultMenu_Key (&m_creditsMenu.frameWork, key);
}
...
{
	CreditsMenu_Init ();
	UI_PushMenu (&m_creditsMenu.frameWork, CreditsMenu_Draw, CreditsMenu_Close, CreditsMenu_Key);
}
