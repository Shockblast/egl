...
typedef struct uiExport_s {
	...
	void		(*Init) (int vidWidth, int vidHeight, int oldCursorX, int oldCursorY);
	void		(*Shutdown) (qBool full);
	void		(*DrawConnectScreen) (char *serverName, int connectCount, qBool backGround, qBool cGameOn);
...
typedef struct uiImport_s {
	...
	void		(*StoreCursorPos) (int *x, int *y);
