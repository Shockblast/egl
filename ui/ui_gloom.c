...
{
	uii.Cvar_SetValue ("glm_advgas", m_gloomOptionsMenu.advgas_toggle.curValue);
}

static void GlmAdvStingerFireFunc (void *unused)
{
	uii.Cvar_SetValue ("glm_advstingfire", m_gloomOptionsMenu.advstingfire_toggle.curValue);
}

static void GlmBlueStingerFireFunc (void *unused)
{
	uii.Cvar_SetValue ("glm_bluestingfire", m_gloomOptionsMenu.bluestingfire_toggle.curValue);
}

static void GlmBlobTypeFunc (void *unused)
{
	uii.Cvar_SetValue ("glm_blobtype", m_gloomOptionsMenu.blobtype_list.curValue);
}

static void GlmFlashPredFunc (void *unused)
{
	uii.Cvar_SetValue ("glm_flashpred", m_gloomOptionsMenu.flashpred_toggle.curValue);
}

static void GlmFlWhiteFunc (void *unused)
{
	uii.Cvar_SetValue ("glm_flwhite", m_gloomOptionsMenu.flashwhite_toggle.curValue);
}

static void GlmForceCacheFunc (void *unused)
{
	uii.Cvar_SetValue ("glm_forcecache", m_gloomOptionsMenu.forcecache_toggle.curValue);
}

static void GlmJumpPredFunc (void *unused)
{
	uii.Cvar_SetValue ("glm_jumppred", m_gloomOptionsMenu.jumppred_toggle.curValue);
}

static void GlmClassDispFunc (void *unused)
{
	uii.Cvar_SetValue ("glm_showclass", m_gloomOptionsMenu.classdisplay_toggle.curValue);
}
...
static void GloomMenu_SetValues (void)
{
	...
	m_gloomOptionsMenu.advgas_toggle.curValue	= uii.Cvar_VariableInteger ("glm_advgas");

	uii.Cvar_SetValue ("glm_advstingfire",		clamp (uii.Cvar_VariableInteger ("glm_advstingfire"), 0, 1));
	m_gloomOptionsMenu.advstingfire_toggle.curValue	= uii.Cvar_VariableInteger ("glm_advstingfire");

	uii.Cvar_SetValue ("glm_bluestingfire",		clamp (uii.Cvar_VariableInteger ("glm_bluestingfire"), 0, 1));
	m_gloomOptionsMenu.bluestingfire_toggle.curValue	= uii.Cvar_VariableInteger ("glm_bluestingfire");

	uii.Cvar_SetValue ("glm_blobtype",			clamp (uii.Cvar_VariableInteger ("glm_blobtype"), 0, 1));
	m_gloomOptionsMenu.blobtype_list.curValue		= uii.Cvar_VariableInteger ("glm_blobtype");

	uii.Cvar_SetValue ("glm_flashpred",			clamp (uii.Cvar_VariableInteger ("glm_flashpred"), 0, 1));
	m_gloomOptionsMenu.flashpred_toggle.curValue	= uii.Cvar_VariableInteger ("glm_flashpred");

	uii.Cvar_SetValue ("glm_flwhite",			clamp (uii.Cvar_VariableInteger ("glm_flwhite"), 0, 1));
	m_gloomOptionsMenu.flashwhite_toggle.curValue	= uii.Cvar_VariableInteger ("glm_flwhite");

	uii.Cvar_SetValue ("glm_forcecache",		clamp (uii.Cvar_VariableInteger ("glm_forcecache"), 0, 1));
	m_gloomOptionsMenu.forcecache_toggle.curValue	= uii.Cvar_VariableInteger ("glm_forcecache");

	uii.Cvar_SetValue ("glm_jumppred",			clamp (uii.Cvar_VariableInteger ("glm_jumppred"), 0, 1));
	m_gloomOptionsMenu.jumppred_toggle.curValue		= uii.Cvar_VariableInteger ("glm_jumppred");

	uii.Cvar_SetValue ("glm_showclass",			clamp (uii.Cvar_VariableInteger ("glm_showclass"), 0, 1));
	m_gloomOptionsMenu.classdisplay_toggle.curValue	= uii.Cvar_VariableInteger ("glm_showclass");
}
...
static void GloomMenu_Init (void)
{
	...
	UI_StartFramework (&m_gloomOptionsMenu.frameWork);
...
static void GloomMenu_Draw (void)
{
	...
	m_gloomOptionsMenu.frameWork.x			= uiState.vidWidth * 0.5;
	...
	// Render
	UI_CenterMenuHeight (&m_gloomOptionsMenu.frameWork);
	UI_SetupMenuBounds (&m_gloomOptionsMenu.frameWork);

	UI_AdjustCursor (&m_gloomOptionsMenu.frameWork, 1);
	UI_Draw (&m_gloomOptionsMenu.frameWork);
}
...
=============
*/
static struct sfx_s *GloomMenu_Key (int key)
{
	return DefaultMenu_Key (&m_gloomOptionsMenu.frameWork, key);
}
...
{
	GloomMenu_Init ();
	UI_PushMenu (&m_gloomOptionsMenu.frameWork, GloomMenu_Draw, GloomMenu_Close, GloomMenu_Key);
}
