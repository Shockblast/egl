...
static void StartGame (void)
{
	...
	uii.Cbuf_AddText ("killserver ; wait ; newgame\n");
...
{
	UI_StartFramework (&m_gameMenu.frameWork);
...
static void GameMenu_Draw (void)
{
	...
	m_gameMenu.frameWork.x			= uiState.vidWidth * 0.5f;
	...
	// Render
	UI_CenterMenuHeight (&m_gameMenu.frameWork);
	UI_SetupMenuBounds (&m_gameMenu.frameWork);

	UI_AdjustCursor (&m_gameMenu.frameWork, 1);
	UI_Draw (&m_gameMenu.frameWork);
}
...
=============
*/
static struct sfx_s *GameMenu_Key (int key)
{
	return DefaultMenu_Key (&m_gameMenu.frameWork, key);
}
...
{
	GameMenu_Init ();
	UI_PushMenu (&m_gameMenu.frameWork, GameMenu_Draw, GameMenu_Close, GameMenu_Key);
}
