...
static void DMFlagsMenu_Init (void)
{
	...
	UI_StartFramework (&m_dMFlagsMenu.frameWork);
...
static void DMFlagsMenu_Draw (void)
{
	...
	m_dMFlagsMenu.frameWork.x			= uiState.vidWidth * 0.50f;
	...
	// Render
	UI_CenterMenuHeight (&m_dMFlagsMenu.frameWork);
	UI_SetupMenuBounds (&m_dMFlagsMenu.frameWork);

	UI_AdjustCursor (&m_dMFlagsMenu.frameWork, 1);
	UI_Draw (&m_dMFlagsMenu.frameWork);
}
...
=============
*/
static struct sfx_s *DMFlagsMenu_Key (int key)
{
	return DefaultMenu_Key (&m_dMFlagsMenu.frameWork, key);
}
...
{
	DMFlagsMenu_Init();
	UI_PushMenu (&m_dMFlagsMenu.frameWork, DMFlagsMenu_Draw, DMFlagsMenu_Close, DMFlagsMenu_Key);
}
