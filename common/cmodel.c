...
void CM_LoadVisibility (dBspLump_t *l) {
	...
	memcpy (cmMapVisibility, cm_mapBuffer + l->fileOfs, l->fileLen);
...
void CM_LoadEntityString (dBspLump_t *l) {
	...
	memcpy (cmMapEntityString, cm_mapBuffer + l->fileOfs, l->fileLen);
}
...
void CM_WritePortalState (int fileNum) {
	FS_Write (cmPortalOpen, sizeof (cmPortalOpen), fileNum);
}
...
void CM_ReadPortalState (int fileNum) {
	FS_Read (cmPortalOpen, sizeof (cmPortalOpen), fileNum);
	FloodAreaConnections ();
}
