...
int FS_LoadFile (char *path, void **buffer, char *terminate)
{
	...
	if (terminate) {
		...
		termLen = 0;
		buf = Mem_PoolAlloc (fileLen, com_fileSysPool, 0);
	}
	*buffer = buf;
...
#if defined (DATADIR) && defined (__unix__)	
...
#if defined (__unix__) && defined (DATADIR)
	fs_basedir		= Cvar_Register ("basedir",			DATADIR, CVAR_READONLY);
#else
...
#if defined (__unix__) && defined (LIBDIR)
