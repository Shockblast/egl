...
void *MSG_GetSpace (netMsg_t *buf, int length) {
	...
	if (buf->curSize + length > buf->maxSize) {
		...
		if (buf->curSize + length >= buf->bufferSize) {
			MSG_Clear (buf);
			Com_Printf (0, S_COLOR_YELLOW "MSG_GetSpace: overflow\n");
		}
		else {
			Com_Printf (0, S_COLOR_YELLOW "MSG_GetSpace: overflowed maxSize\n");
		}
	}
...
void MSG_WriteDeltaEntity (entityStateOld_t *from, entityStateOld_t *to, netMsg_t *msg, qBool force, qBool newEntity, int mask, int clientProtocol) {
	...
	if (to->skinNum != from->skinNum) {
		...
		else if ((uInt)to->skinNum < 0x10000)	bits |= U_SKIN16;
	...
	// take a baseline
	if (clientProtocol == ENHANCED_PROTOCOL_VERSION) {
		if (mask == 1)
			to->event |= 32;
	...
	if ((newEntity || to->renderFx & RF_FRAMELERP || ((to->renderFx & RF_BEAM) && clientProtocol != ENHANCED_PROTOCOL_VERSION)) ||
		(to->renderFx & RF_BEAM && clientProtocol == ENHANCED_PROTOCOL_VERSION && !VectorCompare (to->oldOrigin, from->oldOrigin))) {
		bits |= U_OLDORIGIN;
	}

#ifdef ENHANCED_SERVER
	if (!VectorCompare (to->velocity, from->velocity) && clientProtocol == ENHANCED_PROTOCOL_VERSION)
		bits |= U_VELOCITY;
#endif
...
float MSG_ReadCoord (netMsg_t *msg_read) {
	return MSG_ReadShort (msg_read) * ONEDIV8;
}
...
void MSG_ReadPos (netMsg_t *msg_read, vec3_t pos) {
	pos[0] = MSG_ReadShort (msg_read) * ONEDIV8;
	pos[1] = MSG_ReadShort (msg_read) * ONEDIV8;
	pos[2] = MSG_ReadShort (msg_read) * ONEDIV8;
}
