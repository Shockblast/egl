...
// FIXME TODO:
// * other neat features like maximum size, hunk management?
// * PROPER memory footer sentinel checking (for proper underflow checking).
//

#include "common.h"

#define MEM_BLOCK_SENTINEL_TOP	0xFEBDFAED
#define MEM_BLOCK_SENTINEL_BOT	0xD0BAF0FF
...
{
	if (mem->topSentinel != MEM_BLOCK_SENTINEL_TOP) {
		Com_Error (ERR_FATAL,
			"Mem_Free: bad memory block top sentinel\n"
			"check: %s:#%i",
			fileName, fileLine);
	}
	else if (mem->botSentinel != MEM_BLOCK_SENTINEL_BOT) {
		Com_Error (ERR_FATAL,
			"Mem_Free: bad memory block bottom sentinel\n"
			"check: %s:#%i",
			fileName, fileLine);
	}
	else if (!mem->footer) {
		Com_Error (ERR_FATAL,
			"Mem_Free: corrupted memory footer [buffer overflow]\n"
			"pool: %s\n"
			"alloc: %s:#%i\n"
			"check: %s:#%i",
...
void *_Mem_Alloc (size_t size, qBool zeroFill, struct memPool_s *pool, const int tagNum, const char *fileName, const int fileLine)
{
	...
	// Fill in the header
	mem->topSentinel = MEM_BLOCK_SENTINEL_TOP;
	mem->tagNum = tagNum;
	mem->size = size;
	...
	mem->botSentinel = MEM_BLOCK_SENTINEL_BOT;
...
void Mem_Register (void)
{
	...
	Cmd_AddCommand ("memtouch",		Mem_Touch_f,		"Touches all allocated memory, forcing it out of VM");
}
