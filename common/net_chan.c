...
void Netchan_Transmit (netChan_t *chan, int length, byte *data) {
	...
	if (chan->message.overFlowed) {
		...
		Com_Printf (PRINT_ALL, S_COLOR_YELLOW "%s: Outgoing message overflow\n"
	...
	if (send.maxSize - send.curSize >= length)
		...
		Com_Printf (PRINT_ALL, S_COLOR_YELLOW "Netchan_Transmit: dumped unreliable\n");
	...
	if (showpackets->value) {
		if (send_reliable)
			Com_Printf (PRINT_ALL, "send %4i : s=%i reliable=%i ack=%i rack=%i\n"
				, send.curSize
				, chan->outgoingSequence - 1
			...
			Com_Printf (PRINT_ALL, "send %4i : s=%i ack=%i rack=%i\n"
				, send.curSize
				, chan->outgoingSequence - 1
				, chan->incomingSequence
				, chan->incomingReliableSequence);
...
called when the current net_message is from remoteAddress
...
qBool Netchan_Process (netChan_t *chan, netMsg_t *msg) {
	...
	if (showpackets->value) {
		if (reliable_message)
			Com_Printf (PRINT_ALL, "recv %4i : s=%i reliable=%i ack=%i rack=%i\n"
				, msg->curSize
				, sequence
			...
			Com_Printf (PRINT_ALL, "recv %4i : s=%i ack=%i rack=%i\n"
				, msg->curSize
				, sequence
				, sequence_ack
				, reliable_ack);
	...
	if (sequence <= chan->incomingSequence) {
		if (showdrop->value)
			Com_Printf (PRINT_ALL, "%s:Out of order packet %i at %i\n"
				, NET_AdrToString (chan->remoteAddress)
				,  sequence
				, chan->incomingSequence);
		return qFalse;
	...
	if (chan->dropped > 0) {
		if (showdrop->value)
			Com_Printf (PRINT_ALL, "%s:Dropped %i packets at %i\n"
			, NET_AdrToString (chan->remoteAddress)
			, chan->dropped
			, sequence);
	}
	...
	*/
	if (reliable_ack == chan->reliableSequence)
