...
================
*/
static qBool Netchan_NeedReliable (netChan_t *chan)
{
	qBool	sendReliable;
...
int Netchan_Transmit (netChan_t *chan, int length, byte *data)
{
	...
	if (chan->sock == NS_CLIENT) {
		if (chan->protocol != ENHANCED_PROTOCOL_VERSION)
			...
			MSG_WriteByte (&send, qport->integer);
	}
...
qBool Netchan_Process (netChan_t *chan, netMsg_t *msg)
{
	...
	if (chan->sock == NS_SERVER && (chan->protocol != ENHANCED_PROTOCOL_VERSION || chan->qPort > 0xff))
		MSG_ReadShort (msg);
