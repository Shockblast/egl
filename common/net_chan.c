...
void Netchan_Init (void)
{
	...
	qport		= Cvar_Get ("qport",		Q_VarArgs ("%i", port),	0);
}
...
void Netchan_Setup (int sock, netChan_t *chan, netAdr_t adr, int protocol, int qPort)
{
	...
	if (protocol == ENHANCED_PROTOCOL_VERSION) {
		...
		chan->message.bufferSize = MAX_SV_USABLEMSG;
	}

	chan->qPort = qPort;
	chan->protocol = protocol;
	chan->lastReceived = sys_currTime;
...
int Netchan_Transmit (netChan_t *chan, int length, byte *data)
{
	...
	if (chan->sock == NS_CLIENT) {
		if (chan->protocol != ENHANCED_PROTOCOL_VERSION)
			...
			MSG_WriteByte (&send, qport->value);
	}
