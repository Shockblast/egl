...
char *Cvar_BitInfo (int bit)
{
	...
	for (var=com_cvarList ; var ; var=var->next) {
		if (var->flags & bit) {
...
cVar_t *Cvar_Get (char *varName, char *varValue, int flags)
{
	...
	if (Cmd_Exists (varName)) {
		Com_Printf (0, S_COLOR_YELLOW "Cvar_Get: overwriting command \"%s\" with a cvar\n", varName);
		Cmd_RemoveCommand (varName, NULL);
	}

	// Overwrite aliases
	if (Alias_Exists (varName)) {
		Com_Printf (0, S_COLOR_YELLOW "Cvar_Get: overwriting alias \"%s\" with a cvar\n", varName);
	...
	if (flags & (CVAR_USERINFO|CVAR_SERVERINFO)) {
		if (!Cvar_InfoValidate (varName)) {
			Com_Printf (0, S_COLOR_YELLOW "Cvar_Get: invalid info cvar name\n");
			return NULL;
		}
	}
	...
	if (flags & (CVAR_USERINFO|CVAR_SERVERINFO)) {
		if (!Cvar_InfoValidate (varValue)) {
			Com_Printf (0, S_COLOR_YELLOW "Cvar_Get: invalid info cvar value\n");
			return NULL;
		}
	}

	// Allocate
	var = Mem_Alloc (sizeof (cVar_t));
...
static void Cvar_List_f (void)
{
	...
	// create a list
	for (matching=0, total=0, var=com_cvarList ; var ; var=var->next, total++) {
	...
	sortedList = Mem_Alloc (matching * sizeof (cVar_t));
	for (matching=0, var=com_cvarList ; var ; var=var->next) {
		if (!Q_WildcardMatch (wildCard, var->name, 1))
			continue;
