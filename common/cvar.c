...
============
*/
static cVar_t *Cvar_Exists (char *varName) {
	cVar_t	*var;
...
static cVar_t *Cvar_SetVariableValue (cVar_t *var, char *value, qBool force) {
	...
	if (!force) {
		...
		if (var->flags & CVAR_LATCH_SERVER) {
			if (var->latchedString) {
				if (Q_stricmp (value, var->latchedString) == 0)
					return var;
				Mem_Free (var->latchedString);
			} else {
				if (Q_stricmp (value, var->string) == 0)
					return var;
			}
...
cVar_t *Cvar_VariableSet (cVar_t *var, char *value) {
	...
	return Cvar_SetVariableValue (var, value, qFalse);
}
...
cVar_t *Cvar_VariableSetValue (cVar_t *var, float value) {
	...
	return Cvar_VariableSet (var, val);
}
...
static void Cvar_List_f (void) {
	...
	for (i=0, j=0, var=cvarVariables ; var ; var=var->next, i++) {
